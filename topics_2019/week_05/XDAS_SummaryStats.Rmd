...
[**Aho, Chapter 4, Section 4.3**](https://xdas.bio.nyu.edu/references/Aho_Ch4.3_Statistics.pdf)
...
* Top panel: boxplot showing IQR and limit of outliers (median +/- 1.5xIQR).
* Middle and bottom panels: a probability density function with mean $\mu$ = 0 and varance =  $\sigma^2$.
...
* If the data are normally distributed, then the limits of the IQR fall at +/- 0.67 standard deviations from the mean/median (these are the same for a normal distribution).


# Thinking about data visualization

There are many ways to represent your data. In choosing graphical displays, it is important to ask yourself:

* What kind of data do I want to show?
* What is the message that I want to get across?
* What kinds of summary statistics are important?
* What is the easiest way for someone to understand what I am trying to convey?

Different visualization methods are useful for making different points:

* Histograms - discrete distributions
* Bar charts, pie charts - relative distributions (counts, proportions)
* Box plots - median and IQR (range)
* Violin plots - comparative shape of distributions
* Line graphs - trends
* Scatter plots - correlations, trends
* (Kernel) density plots - shapes of continuous (or smoothened) distributions

## Background reading

We will encounter all of these throughout the semester and in the literature. A useful guide to effective visualization can be found here: 

[**Plots to Avoid**, from *Irizarry and Love*](http://genomicsclass.github.io/book/pages/plots_to_avoid.html)

**Irizzarry and Love, pp. 104-123: Plots to Avoid (same content as link above)**


<!--
What are statistics?
=> Estimates of population parameters based on random samples.

Key concepts:
* Population: every example of an item of interest
* Parameter: Greek, "beyond measurement"
* Estimate: a reasoned guess
* Samples: things you CAN measure
* Random: "representative"

Estimates of location or central tendency:
* Mean: sum(x.i)/x.total
    * An estimate of the population average
    * Ideally, representative of a "typical" individual in the population
    * Sensitive to outliers
    * Makes some assumptions about distribution in population
    * Minimizes sum of squared distances: least squares method
* Median: the "middle" measure, 50% of samples above and below
    * A robust statistic (not sensitive to outliers)
    * Minimizes absolute deviation of all samples
* Trimmed mean: remove outliers (assuming measurement error)
    * Caveat: outliers may be real!

A word on notation:
* Mathematical descriptions
* Shorthand for important concepts, e.g.
    * sum(x.i)/x.total => x.bar
    * sum of squares
    * root mean squared deviation (SD, s)

Estimates of variation:
* Range: min / max
* Absolute deviation: IQR
* Variance: normalized sum of squares (s^2)
* Standard deviation: s
* Relative deviation: Deciles, quartiles
* Confidence intervals: 
    * e.g. 95% of time, our estimate of the mean will be within the 95% CI

Data visualization:
* Histograms: distribution
* Bar charts and pie charts: relative distributions
* Box plots: median, IQR; range
* Scatter plots: correlations, trends
* Kernel density plots: smoothened data => approximate population

Think about:
* What am I trying to show?
* What kind of illustration will best show it?
