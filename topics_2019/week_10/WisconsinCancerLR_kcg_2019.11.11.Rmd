...
cancer_data = read_csv("data.csv")  # creates a tibble
cancer_data  # M = malignant; B = benign
```
...
ggplot(cancer_data,aes(x=diagnosis, y=perimeter_mean)) +
  geom_violin(width=0.5) +
  geom_boxplot(width=0.05)
...
Using the glm() function we are going to create a model for each variable. Logistic regression works on a binary response so we have to first convert all the Benign to 0 and Malignant to 1.

1) Split the dataframe into two dataframes, call it **alldata**.

```{r, echo=T, eval=T}
# creates an object with two data frames
alldata = split(cancer_data, cancer_data$diagnosis)
dim(alldata$B) # 357 (rows) x 32 (cols)
dim(alldata$M) # 212 x 32
```
...
3) Before we create our models, let's first remove a fraction of our data for testing purposes.

First we will "train" our model on a large fraction of the data, and then we will test it on the remainder to see how well our model fits. This is called "leave-one-out validation". 

[ Another method would be to do this a bunch of times, each time leaving out a different subset of the data. That method is called "cross-validation". For example, for "10-fold cross-validation". ]
...
```{r, echo=T, eval=T}
# get a set of random row indices
benign_test_index = sample(1:nrow(alldata$B), 70)
malignant_test_index = sample(1:nrow(alldata$M), 40)

# create test set data
benign_test = alldata$B[benign_test_index,]
malginant_test = alldata$M[malignant_test_index,]

# create training set data (all except above)
benign_training = alldata$B[-benign_test_index,]
malignant_training = alldata$M[-malignant_test_index,]

# recombine these
cancer_test = rbind(benign_test, malginant_test)
cancer_training = rbind(benign_training, malignant_training)
dim(cancer_test)
dim(cancer_training)
class(cancer_training$diagnosis)
```
...
radius_prediction = predict(cancer_glm_radius, newdata = cancer_test, type="response")

radius_prediction_binary = round(radius_prediction)
radius_prediction_binary = 0
if (radius_prediction > 0.8) {
  radius_predition_binary = 1
}
