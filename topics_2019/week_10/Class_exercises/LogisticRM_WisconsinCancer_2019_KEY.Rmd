...
```{r, echo=T, eval=T}
par(mfrow=c(2,2))
boxplot(cancer_data$radius_mean ~ diagnosis, data=cancer_data, main="Radius")
boxplot(cancer_data$texture_mean ~ diagnosis, data=cancer_data, main="Texture")
boxplot(cancer_data$smoothness_mean ~ diagnosis, data=cancer_data, main="Smoothness")
boxplot(cancer_data$compactness_mean ~ diagnosis, data=cancer_data, main="Compactness")
```
...
```{r, echo=T, eval=T}
allMal = subset(cancer_data, diagnosis == "M")
allBen = subset(cancer_data, diagnosis == "B")
```
...
randomMal = sample(1:nrow(allMal),40)
randomBen = sample(1:nrow(allBen),70)
...
```{r, echo=T, eval=T}
cancer_glm_radius = glm(diagnosis ~ radius_mean, family="binomial", data=cancer_training)
cancer_glm_texture = glm(diagnosis ~ texture_mean, family="binomial", data=cancer_training)
cancer_glm_smoothness = glm(diagnosis ~ smoothness_mean, family="binomial", data=cancer_training)
cancer_glm_compactness = glm(diagnosis ~ compactness_mean, family="binomial", data=cancer_training)
