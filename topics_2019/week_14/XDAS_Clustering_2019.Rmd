...
* Step 1: Select the number of clusters k and rbitrarily assign each point to one of the k groups
* Step 2:
...
* Can end up with very different clusters depending on starting k (these are random)!
...
### Silhouette width and silhouette plots
...
[comment]: # (from https://de.wikipedia.org/wiki/Silhouettenkoeffizient)

![Hierarchical clustering with silhouette plots](Images/Norm_silhouette.png)
...
### Choosing *k* with silhouette widths

To choose the proper number of clusters, we can plot $k$ versus the average silhouette width for all clusters and pick the $k$ with the highest value (**Figure 8, left**). 
...
Hence another way to evaluate the right number of clusters is to plot the *total within-cluster sum of squares* versus the number of clusters (**Figure 8, right**). The optimal number of clusters may be taken as the "*elbow*" in the graph, where the improvement in WSS begins to level off with increasing $k$.
