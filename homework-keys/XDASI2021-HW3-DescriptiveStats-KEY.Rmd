...
```{r}
mice.pheno %>% group_by(Sex, Diet) %>% summarise(n_animals = n())
```
...
```{r}
# from "missing data" section above
...
```{r}
# mean bodyweight
mean_bw = mean(mice.pheno$Bodyweight)
mean_bw

# sem
sem_bw = sd(mice.pheno$Bodyweight) / sqrt(length(mice.pheno$Bodyweight))
sem_bw

# 95% CI
Q = qnorm(0.975)
c(mean_bw - Q*sem_bw, mean_bw + Q*sem_bw)
```

Describe in words what the 95% CI tells you.

```{r}
# the 95%CI is an estimated interval within which we expect to see the true 
# population mean ~95% of the time (i.e. we expect 19/20 95%CI's to contain 
...
mc = summary_stats %>% subset(Sex == "M" & Diet == "chow") %>%  pull("mean_weight")
mh = summary_stats %>% subset(Sex == "M" & Diet == "hf") %>%  pull("mean_weight")
...
c( ((fh - fc)/fc)*100 , ((mh - mc)/mc)*100 )
data.frame( F_pct_gain = as.numeric(((fh - fc)/fc)*100),
            M_pct_gain = as.numeric(((mh - mc)/mc)*100) )
```
...
```{r}
# it's hard to tell at this point whether these apparent differences are significant or just due to sample variation ... 
```
...
```{r}
# there is quite a lot of variation in these small samples!
```
...
# histogram of sample means from the `resample` data frame
ggplot(resample, aes(x=sample.mean)) + 
  geom_histogram(fill="lightblue", color="black", 
                 boundary=0, binwidth = 0.2) + # pick a color scheme
  
  # map the mean of `sample.mean` to the vertical line (copy syntax from previous histograms above)
...
cat(paste("Chow population mean = ", pop.mean, "\n",
          "Chow sample distribution mean = ", sample.dist.mean, "\n",
          "Difference (sample mean - population mean) = ", round(sample.dist.mean - pop.mean, 2), "\n",
          "SD of sample distribution: ", round(sd(resample$sample.mean),2), sep="") )
...
sem = sd(ctl.sample)/sqrt(length(ctl.sample))
ci = c(mean(ctl.sample) - Q*sem, mean(ctl.sample) + Q*sem)
...
```{r}
# around 5%, but often a few percent more (between 5-10%)
```


How much does the width of the 95% CI's seem to vary?
```{r}
# using additions to the loop above to record the size of the CI interval
# (commented out above)

#  Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
# 1.624   3.949   4.770   4.793   5.393   7.929 
```

About how often do your CI's contain the mean for the high-fat diet population (the dotted line)? What do you think this might mean?
```{r}
# 10-30% of the time; this prompts further hypothesis testing
# to determine how likely it is that high-fat and control samples are drawn from different distributions
```
