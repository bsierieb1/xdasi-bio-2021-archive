...
ggsave("my.ggplot_v1.png", plot = my.ggplot)
```
...
Now let's reproduce these plots using ggplot2. 
...
Notice that you get a warning about the number of bins - by default, the number of bins is set to 30, but you'll usually want to pick a better number for `bins` or set `binwidth`.
...
# data+aes+geom layers with additions specified above (don't forget the + sign!)
...
Here we will approximate the mosaic plot in Figure 3.2-2. Mosaic plots seem to be easier in base R than in ggplot, so we're just going to stick with this. Check out `ggmosaic()` if you want to try making a mosaic plot with ggplot. 
