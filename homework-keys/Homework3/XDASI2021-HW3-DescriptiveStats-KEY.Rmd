...
In a future homework, we will see how statistical inference can help us discern if there is 
...
mean.bw = mmw %>% group_by(Diet) %>% 
  summarize(avg = mean(Bodyweight), .groups = 'drop')
str(mean.bw)

# make a histogram of the distributions
# use the `mean.bw` dataframe as the data for the geom_vline() aesthetic mapping
a = ggplot(mmw, aes(x=Bodyweight, fill=Diet, color=Diet)) +
  geom_histogram(position="identity", alpha=0.5, binwidth=1) +
  geom_vline(data=mean.bw, 
             aes(xintercept=avg, color=Diet),
             linetype="dashed") + xlim(c(19,45)) 

## Violin plot
b = ggplot(mmw, aes(x=Diet, y=Bodyweight, fill=Diet, color=Diet)) +
  geom_violin(color="black", width=0.5) +
  geom_boxplot(fill="white", color="black", width=0.1)

## Display the plots together with `ggarrange()`
ggarrange(a,b, labels=c("A","B"), common.legend=TRUE, nrow=1)
```
...
pop.diff = mean(hf$Bodyweight) - mean(chow$Bodyweight)
pop.diff
```
...
+ Sample 12 individuals from the "chow" group and the same number from the "hf" group.
