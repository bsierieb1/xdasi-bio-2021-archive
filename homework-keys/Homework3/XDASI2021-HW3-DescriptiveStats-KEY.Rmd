...
+ Use the `summarise()` function to count up the number in each group
  + The argument to do this is `n()`

```{r}
mice.pheno %>% group_by(Sex, Diet) %>% summarise(n())
```
...
Try out this function on the `mice.pheno$Bodyweight`.
...
Now let's use dplyr to make a data frame containing all of the relevant statistics. You will use the same method you used in recitation on the COVID data:
  ...
  + Then, pipe to `summarise()` or `summarize()` (these are synonyms using British or American spelling).
...
+ Check out the documentation for `sample()` and use it to randomly sample 12 male individuals from each group *with replacement*.
...
s.ctl = sample(chow, 12, replace=TRUE)
s.trt = sample(hf, 12, replace=TRUE)
...
```{r warning=FALSE}
# ============================================================================ #
# simulate 100 random samples of size 12 drawn from the male control population
...
sem = function(pop.data, sample.size=12){
  ...
  random.sample = sample(pop.data, sample.size, replace = FALSE)
...
Now we can call this function and compare the SEM we compute from individual samples with the SD we got from the distribution of 1000 sample means taken from the male control ("chow") population.
```{r collapse=TRUE}
# repeat sampling from the given population 10 times (with default sample size = 12)
...
And we can also plot these data to visualize how much they vary:
```{r}
# ============================================================================ #
# plot the distribution of the SEM for 1000 samples of size 12
sem_dist = data.frame(sem = replicate(1000,sem(chow)))
ggplot(sem_dist, aes(x=sem)) + 
  geom_histogram(fill="lightblue", color="black", 
                 boundary=0, binwidth = 0.05) + # pick a color scheme
...
First, let's calculate the bounds of the 95% CI *by hand* for a single sample of 12 mice from the chow population, using the standard deviation of the sample and the sample size.

```{r}
# create a sample of 12 bodyweights from the chow population
sample.size = 12
ctl = sample(chow, sample.size, replace = FALSE)

# compute SEM and 95% CI
sem = sd(ctl)/sqrt(length(ctl))
cat("95% CI for an individual sample:\n",c(mean(ctl) - Q*sem, mean(ctl) + Q*sem))
```
...
# IV. Postscript: Hypothesis testing
