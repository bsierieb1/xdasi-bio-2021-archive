...
author: "Jacob Bernoulli [YOUR NAME HERE]"
date: "Due: 10/8/2021"
...
```{r}
pbinom(0,3,p$Aa, lower.tail=FALSE)
```
...
pW^2 * pI^4  # probability is the same independent of sequence
```
...
```{r}
1 - pbinom(0,6,pW, lower.tail = TRUE)
pbinom(0,6,pW, lower.tail = FALSE)  # upper tail probability for W>0
```
...
cat("P-value =",pbinom(35,50,0.87))

# is this significant?
# yes, the probability is <0.05, our standard cutoff for significance
```
...
library(binom)
moth.ci = binom.confint(35, 50, conf.level = 0.95, methods = "agresti-coull")
moth.ci

# P(white) and 95% CI
cat("Mean estimate = ", moth.ci$mean*50, "; 95%CI = ", moth.ci$lower*50, "-", moth.ci$upper*50)
```
...
par(mfrow=c(1,2))
x.range = 0:50 
plot(x=x.range, y=dbinom(x.range, size=50, prob=0.87), type="h", 
     main="PDF",
     xlab="expected # of white moths", 
     ylab="point probability")
plot(x=x.range, y=pbinom(x.range, size=50, prob=0.87), type="s", 
     main="CDF",
     xlab="expected # of white moths", 
     ylab="total probability")


## Option 2: step by step

# first specify the expected probability of success
pW = 0.87
...
# PDF and CDF
dbinom.W = dbinom(x = x.range, size = max(x.range), prob=pW)
#dbinom.W
pbinom.W = pbinom(q = x.range, size = max(x.range), prob=pW)
#pbinom.W

# make the plots
# par(mfrow=c(1,2))
# plot(x = x.range, y=dbinom.W, type="h", main="PDF",
#      xlab="expected # of white moths",ylab="point probability")
# plot(x = x.range, y=pbinom.W, type="s", main="CDF",
...
## You need to have a vector of the number of successes in each of the individual "observations" (replicates) drawn from a binomial distribution with n=sample size and p=probability of success.
## This is why Bogdan made you go to the trouble to massage your spinner data in recitation! ;-)
```
...
cdf <- function(x,n,p) {
  result = numeric()
  for (i in 0:x) {  # number of successes
    f = n-i         # number of failures
    q = 1-p         # probability of a failure (p = P(success))
#    result[i] = choose(n,i)*(p^i)*(q^f)  # using binomial formula
    result[i] = dbinom(i,n,p)
  }
  return(sum(result))
}
cdf(35,50,0.11)
...
ideal.sample = dbinom(0:sample.size, sample.size, p=p.success)
...
## 0.11*50  # 5.5
## expect 5 or 6 depending on rounding error, this is consistent with expectation

## 0.87*50 = 43.5, perfect
