...
#### d) Generate a table of expected counts.
...
#### f) Compute a $p$-value for the test statistic
...
#### g) Confirm your results using R's Chi-squared test function
...
#### h) What is the null hypothesis for this chi-squared test?
...
#### i) Interpret the result of the Chi-squared test between the Overweight trait and SNP1.
...
#### b) Calculate the RR for the occurence of SNP1 in overweight vs. non-overweight groups.
...
*Your use of Yates' correction should match whatever you did above.* ***Also, note that `riskratio()` is sensitive to how you arrange data in the contingency table!*** *Check out the help file to see the recommended way to arrange data. If your data are organized differently, you may want to either re-organize it manually, use `t()` to transform it, or play around with different `rev=` arguments.*
...
#### e) Confirm your results using the `oddsratio` function in the `epitools` package.
...
```{r eval=FALSE}
# None of them are, at least at alpha = 0.05, but SNP7 has the p-value that is closest to the threshold.
```
