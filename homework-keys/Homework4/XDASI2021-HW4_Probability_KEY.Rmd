...
set.seed(1)
n=1000
yeast.sample = rnorm(n, 1, 0.1)
y.mean = mean(yeast.sample)
y.sd = sd(yeast.sample)
c(y.mean,y.sd)
summary(yeast.sample)
boxplot(yeast.sample, horizontal = TRUE, xlab = "colony diameter (mm)")
```
...
SEM
Q = qnorm(0.975)
c(y.mean - Q*SEM, y.mean + Q*SEM)

# using gmodels package
ci(yeast.sample)


# SEM is the sample estimator for the population mean and is SD/sqrt(sample size)
# 95%CI is an interval within which we expect the true mean of the population to fall 95% of the time. 
# It is the the sample mean +/- Q*SEM, where Q = the 97.5th quantile of the data (z_{1-alpha/2}, alpha = 0.05)
```
...
# base R
hist(yeast.sample, prob=TRUE, col="skyblue", breaks=40)
lines(density(yeast.sample), col="darkblue", lwd=2)
curve(dnorm(x, mean=y.mean, sd=y.sd), 
      lwd=2, add=TRUE, yaxt="n")
abline(v=y.mean, col="red", lwd=3, lty=2)
...
ggplot(yeast.sample.df, aes(x=control)) +
  geom_histogram(aes(y = ..density..), 
                 color = "black", fill = "skyblue", binwidth=0.02) +
  geom_vline(xintercept = y.mean, linetype="dashed",
             color = "red", size=1) +
...
```{r}
pnorm(1.07, mean= y.mean, sd = y.sd) - pnorm(0.93, mean= y.mean, sd = y.sd)
```
...
```{r}
pnorm(y.mean - y.sd, y.mean, y.sd) + pnorm(y.mean + y.sd, y.mean, y.sd, lower.tail=FALSE)
```
...
```{r}
c(qnorm(0.1,y.mean,y.sd),qnorm(0.9,y.mean,y.sd))
```
...
# Independence: P(HPV AND Chlamydia) = P(HPV) * P(Chlamydia)
indep.test = p.hpv.tot * p.chl.tot
indep.test
indep.test == p.hpv.chl

# No - the joint probability is not the product of the individual probabilities.
```
...
```{r}
# P(A1 AND A4) = 2 * P(A1)*P(A4)  # since can inherit A1,A4 or A4,A1
2*A1*A4
```
...
```{r}
# P(A1,A1) = P(A1)*P(A1)          # prob of carrying two A1 alleles
# P(! (A1,A1)) = 1 - P(A1)*P(A1)  # prob of NOT having two A1 alleles

# P(2 individuals are NOT A1,A1) = (1 - P(A1)*P(A1)) * (1 - P(A1)*P(A1))
(1-A1*A1)^2
```
...
```{r}
# P(A3 or A4) = P(A3) + P(A4)        # prob of an allele being A3 or A4
# P(! (A3 or A4)) = 1 - P(A3 or A4)  # prob of an allele not being A3 or A4

# P(neither of 2 alleles is A3 or A4) = [ 1 - P(A3 or A4) ]^2
# P(3 people have no A3 or A4 alleles) = [ [ 1 - P(A3 or A4) ]^2 ]^3
p3or4 = A3 + A4
pNOT3or4 = 1 - p3or4
pNOT3or4^6 # 3 invididuals * 2 alleles each
...
# Aa (mother A, father a) and aA (mother a, father A) are the same, so multiply by 2
...
Pr$Caa = Pr$Caa.Faa_Maa * (Pr$aa * Pr$aa) +
         Pr$Caa.FAa_Maa * (Pr$Aa * Pr$aa) +
         Pr$Caa.FAA_Maa * (Pr$AA * Pr$aa) +
         Pr$Caa.Faa_MAa * (Pr$aa * Pr$Aa) +
         Pr$Caa.FAa_MAa * (Pr$Aa * Pr$Aa) +
         Pr$Caa.FAA_MAa * (Pr$AA * Pr$Aa) +
...
Pr$FAa_MAa.Caa = Pr$Caa.FAa_MAa * (Pr$Aa * Pr$Aa) / Pr$Caa
Pr$FAa_MAa.Caa
```
