...
A solution is to tranform the data in such a way that its transformation is normally distributed. Some transformations include: `log`, `square root` and `arcsine` transformation. The first two are much more common than the third. However any transformation is fine as long as you are able to work backwards to determine the original. Here are histograms of transformations:
...
We can now perform all our statistical methods that require data to be normally distributed. To compute confidence intervals, we just need to remember to back-transform into the original units before reporting them.
