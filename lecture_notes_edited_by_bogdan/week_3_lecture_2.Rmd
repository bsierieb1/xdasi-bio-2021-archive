...
The following [link](https://rpsychologist.com/d3/ci/) contains a very intuitive visualization of what a CI of a sample is. Note that sample mean is **NOT** related to sample variance. So, the widths of the simulated CIs will vary a lot, and **narrow CIs are not necessarilly more accurate**, as they may still be far away from the true mean.
...
We can use the result of these functions to plot the distributions of outcomes for rolls of any number of dice at once:
...
shinyApp(
  ...
  server = function(input, output) {
    output$hist = renderPlot({
      min_max = 1:100
      any_dice = data.frame(newsum = min_max, newcount = ncomb.ndice(min_max,n_dice=input$n_dice))
...
$\Rightarrow\ $ _**What happens to the shape of the distributions if we do this 10, 100, or 1,000 times? Try it out yourself.**_
