...
```{r eval=FALSE}
The workspace is all of the code, images, and objects in your enviroment that you are working with at a given time. You can manually save and load workspaces using the save and load workspace as options from the Session dropdown menu from the top bar.
...
```{r eval=FALSE}
A working directory is the directory which you currently have access to files in. You can use getwd() to print the working directory. You can set the working directory with setwd() or by going to the Session dropdown menu from the top bar
```

I.3) Where does R save the history of your commands?

```{r eval=FALSE}
Under the history tab in the top right of the R Studio window
```
...
```{r eval=FALSE}
R Projects create a file which saves the R workspace automatically into the R Project. R Projects are associated with a particular working directory. This enables much tidier organization of files and scripts needed for a particular project into a single directory.
```

I.5) How do you start a new R script in RStudio?

```{r eval=FALSE}
You can either use the file menu or the keyboard shortcut shift+command+n
...
```{r eval=FALSE}
A + at the beginning of a line in an RMD file means that the line is part of a sublist of a larger heading as in lines 216-19 of this RMD file
```

I.7) What are three ways you can retrieve documentation related to a function and its usage in RStudio?

```{r eval=FALSE}
# 1) Type ?table in the console
# 2) Search for it in the help menu from the bottom right pane in RStudio
# 3) Using the help dropdown from the top bar
```

I.8) What are the three basic components of R Markdown files?

```{r eval=FALSE}
The three basic components of R Markdown files are text blocks, code blocks, and the header.
...
```{r eval=FALSE}
when you knit an R Markdown file it runs through all the code blocks and returns a file of your choice of type with the text blocks as text and the results of the code blocks shown where they belong relative to the text
...
```{r eval=FALSE}
Rbind has rownames for the matrix, the matrix method does not
```
...
```{r eval=FALSE}
A matrix can only have one type of data in it. If you are working with data that has multiple types (say character and numeric) you would prefer a dataframe
```
...
```{r}
# CSV file
write.csv(fly_pheno, file = 'Data/massaged_fly_pheno.csv')
# RDS file
saveRDS(fly_pheno, file='Data/massaged_fly_pheno_df.rds')
