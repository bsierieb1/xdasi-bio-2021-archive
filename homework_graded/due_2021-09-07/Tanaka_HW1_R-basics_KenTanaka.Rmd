...
```{r eval=FALSE}
"The R data workspace is a storage of the current execution environent. 
I listed below how to save and load work spaces manually. 
save.image("worksp.RData")
load("worksp.RData")"
```
...
```{r eval=FALSE}
"The working directory is the origion of the relative path. 
You can find the currenty directory with getwd(). 
Changes can be made with setwd(C:/users/___/Downloads")"
...
```{r eval=FALSE}
"Saves them in the R history file under the working directory"
...
```{r eval=FALSE}
"Since the RStudio Project is associated with R workspace and working directory, there is an advantage that you can create multiple workspaces and working directories by creating a project."
...
```{r eval=FALSE}
"In order to start a new R script in RStudio, Restart R and Run all Chunks."
```

I.6) What does it mean then R prompts you with a  "+" in the beginning of the line?

```{r eval=FALSE}
"It measn to input the continuation of the command."
...
Run All Chunk Above and Run All Chunk Below
...
```{r eval=FALSE}
"The rbind() is useful when adding data in row units. "
...
expvalues <- rbind()
expvalues <- rbind(expvalues, GeneA)
expvalues <- rbind(expvalues, GeneB)
expvalues <- rbind(expvalues, GeneC)
expvalues
...
```{r eval=FALSE}                                  
"A data frame can combine different vector types into a single variable"
...
```{r}
fly_pheno <- read.table(header=TRUE, sep="\t", stringsAsFactors = F, "data/hw1_fly_phenotypes.tsv")
```
...
```{r}
h <- tolower(colnames(fly_pheno))
colnames(fly_pheno) = h
colnames(fly_pheno)
```
...
n <- which(fly_pheno$eye_color == "WHI")
fly_pheno[n,]$eye_color <- "WHITE"
# check what happened
head(fly_pheno)$eye_color
```
...
sum(fly_pheno$sex == "M")
sum(fly_pheno$sex == "F")
fly_pheno_male <- subset(fly_pheno, fly_pheno$sex == "M")
fly_pheno_female <- subset(fly_pheno, fly_pheno$sex == "F")
sum(fly_pheno_male$eye_color == "RED")
sum(fly_pheno_male$eye_color == "WHITE")
sum(fly_pheno_female$eye_color == "RED")
sum(fly_pheno_female$eye_color == "WHITE")

sex <- c("Male", "Female")
red <- c(sum(fly_pheno_male$eye_color == "RED"),
         sum(fly_pheno_female$eye_color == "RED"))
white <- c(sum(fly_pheno_male$eye_color == "WHITE"),
           sum(fly_pheno_female$eye_color == "WHITE"))
tdata <- data.frame(SEX=sex, RED=red, WHITE=white)
tdata
```
...
```{r}
# CSV file
write.table(tdata, "data/fly_male_female_eyecolor.csv", sep=",", quote=F, col.names=c("SEX","RED","WHITE"),row.names=F)
# RDS file
saveRDS(tdata, "data/fly_male_female_eyecolor.rds")
