...
```{r eval=FALSE}
# R workspace is the working environment containing all the objects. 
# To save the workspace select file > save as
# to load select file > open file
```

I.2) What is a working directory? How do you set it and how do you determine which directory is the working directory?

```{r eval=FALSE}
# Working directory is the directory in which all the project is saved in your computer
# to set it use setwd(dir). to determine it use getwd()
```

I.3) Where does R save the history of your commands?

```{r eval=FALSE}
#in the History pane and also is saved in the working directory in a .Rhistory file
```
...
```{r eval=FALSE}
# into the working directosy there is the R project. each project contains its own workspace, history, and source documents,so all the environment is set up and ready to use, everything is in the same place.
```
...
```{r eval=FALSE}
#the markdown file is converted into an HTML, PDF or Word document
```
...
```{r}
Experiments=c("control","nitrogen","potassium","phosphate") #create vector
Experiments #display the contents
```
...
```{r eval=FALSE}
#in a matrix all the values have to be the same type, while in a data frame the values can be a combination of different types. In case of different mixed-type of data the data frame would be preferred.
```
...
expvalues.df[1,1:2] #this gives only the actual values in the data frame


# b) alternatively, you can exclude unwanted columns using -c()
expvalues.df[1,-c(3:4)] #this gives only the actual values in the data frame
...
interesting_genes = which(expvalues.df$potassium > 7) #potassium column > than value 7
interesting_genes #this gives only the positions in the data frame ->which function gives positions only
expvalues.df[interesting_genes,] #this gives only the actual values in the data frame
...
```{r collapse=TRUE}
getwd()
dir.create("/Users/Sofia/Desktop/R week 1/data")
dir("data") #also can use all path
list.files("/Users/Sofia/Desktop/R week 1/data")
```

b. Download the data file called "hw1_fly_phenotypes.tsv" and put it into the data directory you just created. (Don't try to use RStudio for this, just use your filesytem.) # also can use file.copy function into R
...
```{r}
?read.table
fly_pheno=read.delim("data/hw1_fly_phenotypes.tsv", 
                     stringsAsFactors = TRUE) #Reads a file in table format and creates a data frame from it,                                               with cases corresponding to lines and variables to fields in the file.
...
# CSV file
write.csv(fly_pheno,
          file="data/fly_pheno.csv", #naming the file
          row.names = FALSE) #the row have non names
# RDS file
saveRDS(fly_pheno,
