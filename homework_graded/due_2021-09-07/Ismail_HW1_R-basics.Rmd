...
```{r eval=FALSE}
#It is our working environment where we assign objects e.g dataframe, vector, list, Matric. From the file menu we can open a file or save it or make new R script

```

I.2) What is a working directory? How do you set it and how do you determine dataframe directory is the working directory?

```{r eval=FALSE}
#we can get our current working directory from getwd() or set a new one from setwd(). Working directory is the whole path for the R file we are working on our PC
```

I.3) Where does R save the history of your commands?

```{r eval=FALSE}
#In a separate file called R history file that is autimatically generated , we can retrieve it by clicking on the history tab beside the Environment tab or just write history()
```
...
```{r eval=FALSE}
#Creating new R project will allow us to decide on the directory location or use existing directory, name the project and ( could be extended to different directories (different scripts in different locations). R workspace is the R working environment where we have our objects saved. 
```

I.5) How do you start a new R script in RStudio?

```{r eval=FALSE}
# by clicking on the file menu then new file, new R script
```

I.6) What does it mean then R prompts you with a  "+" in the beginning of the line?

```{r eval=FALSE}
#when the command is incomplete, the prompt is still in continuation and not closed yet, the + sign will show up
```
...
# 1) using Help tab 
# 2) help command, help()
# 3) using Vignettes but needs certain package(s) to be installed
```
...
```{r eval=FALSE}
#it generates new file that contains text, code and results. The generated file could be in different formats.
```
...
#alt + enter ( after putting the cursor on the line of interest) or just click on Run tab then run selected lines
# what are two ways you can run an entire code block at once?
#ctrl + shift + enter ( after putting the cursor on the line of interest) or just click on Run tab then run current chunk
# how do you prevent a code block from being executed when you knit the document?
#if we indexed them and used -ve like echo = -2 
# how do you prevent a code block from being displayed in a rendered document?
# by writing echo = false 
```
...
matrix(c(Gene_A,Gene_B,Gene_C), nrow = 3 , ncol = 4, byrow = TRUE)

# method 2: use rbind()
rbind(Gene_A,Gene_B,Gene_C)
```

d. What's the difference between the output when you use these two different methods to create the matrix?
```{r eval=FALSE}
#when we use rbind row names are there, but using matrix function row names not found
```
...
expvalues <- rbind(Gene_A,Gene_B,Gene_C)
# assign column names
...
```{r eval=FALSE}
#dataframe give us the flexibility to have different kind of data. In matrix all data has to be same type. If I have mixed data then I would go for dataframe
```
...
e. You can also select rows from data frames based on their values using the `dataframe()` function with logical expressions!
```{r collapse=TRUE}
...
```{r}
fly_pheno <- read.table(file='E:/NYU/Statistics_course/class_1/assigment/data/hw1_fly_phenotypes.tsv', sep = '\t', header = TRUE )

fly_pheno.df <- data.frame(fly_pheno,stringsAsFactors=FALSE)
fly_pheno.df
```
...
names(fly_pheno.df) <- tolower(names(fly_pheno.df))
head(fly_pheno.df)
```
...
fly_pheno.df$eye_color[fly_pheno.df$eye_color == "WHI"] <- "WHITE"
...
fly_pheno.df$sex <- as.factor(fly_pheno.df$sex)
# check your work
class(fly_pheno.df$sex)
#make a table
table(fly_pheno.df$eye_color)
```
...
```{r}
# CSV file
write.csv(fly_pheno.df,
            file="fly_pheno.df.csv", 
            row.names=F)
# RDS file
saveRDS(fly_pheno.df,
        file="fly_pheno.df.RDS")
