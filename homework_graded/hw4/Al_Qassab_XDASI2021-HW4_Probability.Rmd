...
author: "Sir Ronald Fisher [Omar Qassab]"
date: "Due: 9/30/2021"
output:
  html_document:
    toc: 3
...
# sample
n=1000
colonies = rnorm(n, 1, 0.1)

# mean and sd
mean_colonies=mean(colonies)
sd_colonies=sd(colonies)
paste("The mean is", round(mean_colonies,3), "and the SD is", round(sd_colonies,4) )

# IQR and boxplot
summary(colonies)
IQR(colonies)
boxplot(colonies, horizontal = TRUE)
...
Q=qnorm(0.975)
sem_colonies= sd_colonies/sqrt(n)
ci_colonies = c(mean_colonies - Q*sem_colonies, mean_colonies + Q*sem_colonies)

# Option 2: using gmodels package
ci(colonies)

# Explanation
# The SEM is computed by dividing the SD by the root of the sample size. The SEM measures how much discrepancy there is likely to be in a sample's mean compared to the population mean. The 95% CI is a range of values surrounding a sample estimate in which the true population parameter is expected to be found 95% of the time. The True population mean lies in 95/100 independent samples
```
...
colonies_df=data.frame(colonies)

ggplot(data=colonies_df,
       mapping=aes(x=colonies)) +
  geom_histogram( fill="firebrick",
                   color="black",
                  binwidth = 0.01) +
  geom_vline(aes(xintercept = mean_colonies,color="mean"),
             show.legend = TRUE,
             linetype="dashed",
             size=1) +
  geom_vline(aes(xintercept = median(colonies),color="median"),
             show.legend = TRUE,
             size=1) +
...
```{r}
# your answer here
pnorm(1.07, mean(colonies), sd(colonies)) - pnorm(0.93, mean(colonies), sd(colonies))
```
...
```{r}
# your answer here
pnorm(mean(colonies)-sd(colonies), mean(colonies),sd(colonies)) *2 #Portion of data greater than 1 SD away
1- (pnorm(mean(colonies)-sd(colonies), mean(colonies),sd(colonies)) *2 ) #Portion of data less than 1 SD away
...
```{r}
# your answer here
qnorm(c(0.10,0.90),mean(colonies), sd(colonies))
```
...
```{r}
# P(Pos)= P(HPV+) + P(CHL+) - P(HPV+ AND CHL+)
p.pos = p.chl + p.hpv + p.hpv.chl
p.pos
```
...
```{r}
# Two events are independent if the occurrence of one does not inform us about the occurrence of the other. This mean if events are independent the chance of both co-occurring should equal to the probability of one occurring multiplied by the other.
#Since P(CHL+ AND HPV+) != P(CHL+) * P(HPV+) then these results are not independent
```
...
# Probability of either A2 or A5 = P(A2 or A5)
A2+A5
...
```{r}
# prob of carrying two A1 alleles = P(A1 and A1)
A1*A1

# prob of NOT having two A1 alleles=P(!A1 and !A1)= 1 - P(A1 and A1)
1-(A1*A1)
...
```{r}
# prob of an allele being A3 or A4 = P(A3 or A4)
A3 + A4
# prob of an allele not being A3 or A4 = P(!A3 or !A4)
1 - (A3 + A4)
# P(neither of 2 alleles is A3 or A4)
not_A3orA4=(1 -(A3 + A4))*(1 -(A3 + A4))
