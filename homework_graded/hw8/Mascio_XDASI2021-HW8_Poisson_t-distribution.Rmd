...
```{r eval=FALSE}
# yes, because 95% do not contain a cell, so it is more of a rare event to see a cell in a droplet
```
...
range = c(0:10)
cell.droplet.pmf = dpois(range, 0.05)
...
ggplot(cell.droplet.pmf.df, aes(Cell_count, Density)) +
  geom_histogram(stat = "identity", fill = "thistle") +
  scale_x_continuous(breaks = pretty_breaks()) +
  labs(title = "Cell Droplet PMF", x = "Cell Count", y = "Probability Density")
```
...
cell.droplet.pmf.n1000 = dpois(range, 10*mu)   # 1k
cell.droplet.pmf.n2000 = dpois(range, 20*mu)   # 2k

# Create a data frame with one column for the range vector and one for each dataset
pdf.df <- data.frame("cell_count" = range, "n100" = cell.droplet.pmf.n100, "n1000" = cell.droplet.pmf.n1000, "n2000" = cell.droplet.pmf.n2000)
...
pdf.long.df <- melt(pdf.df, id.vars = "cell_count", measure.vars = c("n100", "n1000", "n2000"), variable.name = "concentration", value.name = "density")
# look at the first few lines of the melted data frame
head(pdf.long.df)

# Make the plot
ggplot(pdf.long.df, aes(cell_count, density, fill = concentration)) +
  geom_histogram(stat = "identity", position = "dodge2", alpha = 0.5) +
  labs(title = "Poisson Distribution at \nDifferent Concentrations", x = "Cell Count", y = "density")
...
```{r}
# Make the plot
ggplot(pdf.long.df, aes(cell_count, 1e6*density, fill = concentration)) +
  geom_histogram(stat = "identity", position = "dodge2", alpha = 0.5) +
  labs(title = "Poisson Distribution at \nDifferent Concentrations", x = "Cell Count", y = "number of droplets")
...
pdf.long.df.drops <- pdf.long.df %>% mutate("droplets" = 1e6*density)
head(pdf.long.df.drops)
# check sum of counts
pdf.long.df.drops %>% group_by(concentration) %>% summarise("sums" = sum(droplets))
```
...
```{r}
# filter the data
pdf.long.df.drops.single <- pdf.long.df.drops %>% filter(cell_count == 1) %>% mutate(cell_count = as.character(cell_count))
head(pdf.long.df.drops.single)
# Plot the data as a bar plot
ggplot(pdf.long.df.drops.single, aes(x = concentration, y = droplets)) +
  geom_col(aes(fill = concentration)) + 
  labs(title = "Number of Droplets with One Cell", x = "Concentration of cells per ul", y = "number of droplets")
```
...
```{r eval=FALSE}
# 2000 cells/ul produces the most droplets with 1 cell
```
...
```{r}
# filter the data
pdf.long.df.drops.doublets <- pdf.long.df.drops %>% filter(cell_count >= 2) #filtering for 2 or more in cell count
head(pdf.long.df.drops.doublets)
# sum up the totals by group using the aggregate function
pdf.long.df.drops.doublets.sums <- aggregate(formula = droplets ~ concentration, data = pdf.long.df.drops.doublets, FUN = "sum") %>% mutate(cell_count = "2+") #using aggregate to sum based on concentration and then mutate to add the cell_count column
head(pdf.long.df.drops.doublets.sums)
# combine the 1 and 2+ data
droplets.combined <- bind_rows(pdf.long.df.drops.single,pdf.long.df.drops.doublets.sums) #combining the singlets and doublets dataframes

#dropping the density column that is only in the singlets dataframe
droplets.combined <- droplets.combined[,c(1:2,4)]
head(droplets.combined)
# Plot the combined data as a bar plot
ggplot(droplets.combined, aes(x = concentration, y = droplets, fill = cell_count)) +
  geom_col(position = "dodge2") +
  labs(title = "Singlet vs doublet droplets per cell concentration", x = "concentration (cells/ul)", y = "number of droplets")
```

Which experimental concentration has the lowest doublet rate? Which concentration would you choose for your experiment?

```{r eval=FALSE}
# 100 cells/ul has the lowest doublet rate. I'd choose the 1000 cells/ul because it has a much higher singlet rate than 100 cells/ul and a much lower doublet rate than 2000 cells/ul
```
...
sample.110 <- rpois(1e6, 110/1000*0.5)

# make a frequency table from the sampled data
sample.110.table <- table(sample.110)
sample.110.table
# filter the n100 data from the ideal distributions
filtered.100 <- pdf.long.df.drops %>% filter(concentration == "n100") %>% select(cell_count, droplets)
filtered.100

# use xtabs to make a table of the ideal n100 data
filtered.100.table <- xtabs(formula = droplets ~ cell_count, data = filtered.100)
filtered.100.table
...
Go ahead and do this, and then join them together into a single contingency table. You should end up with a table containing two rows and around 3-4 columns.

```{r}
# truncate the table with the ideal data to the correct length
filtered.100.table.truncated <- filtered.100.table[1:length(sample.110.table)]
filtered.100.table.truncated
# bind the data together into a single table
combined.chi.sq.table <- rbind(sample.110.table, filtered.100.table.truncated)
combined.chi.sq.table
```
...
barplot(height = combined.chi.sq.table[1:2,], beside = TRUE)
...
chisq.test(combined.chi.sq.table)
...
```{r eval=FALSE}
# the barplot looks similar but hte chi squared p value is super tiny, so there is a significant difference between the sample and ideal data.
```
...
chow <- mice.pheno %>% filter(Sex == "M" & Diet == "chow") %>% select(Bodyweight) %>% pull()
```
...
```{r}
ci.fail.counter <- function(chowPop = chowPop, N = 12, n.samples = 100) {
...
ci.fail.counter.t <- function(chowPop = chowPop, N = 12, n.samples = 100) {
...
# take a peek at the first few rows of the vector
ci.fail.count.t[1:10]
# make a table to check the results
ci.table.t <- table(ci.fail.count.t)
ci.table.t
```
...
#the distribution looks slightly skewed right, but also the tails may seem just longer
...
```{r eval=FALSE}
# these results have fatter tails
```
