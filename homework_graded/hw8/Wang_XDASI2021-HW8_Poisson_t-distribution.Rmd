...
po.df=data.frame("Cell_count"=range,"Density"=cell.droplet.pmf)
...
cell.droplet.pmf.n1000 =   dpois(range,lambda = 0.5) # 1k
...
droplet.df=data.frame("Cell_count"=range,"pdf100"=cell.droplet.pmf.n100,"pdf1000"=cell.droplet.pmf.n1000,
                      "pdf2000"=cell.droplet.pmf.n2000)
...
droplet.melt=melt(droplet.df,id.vars="Cell_count",value.name="pdf",variable.name=cbind("concentration"))

# look at the first few lines of the melted data frame
head(droplet.melt)

# Make the plot
ggplot(data = droplet.melt,
       mapping=aes(x=Cell_count,y=pdf,fill=concentration))+
...
# Make the plot
ggplot(data = droplet.melt,
       mapping=aes(x=Cell_count,y=10^6*pdf,fill=concentration))+
  geom_histogram(stat="identity",position="dodge2",alpha=0.5)+
...
droplet.melt$number=droplet.melt$pdf*(10^6)

# check sum of counts
sum(droplet.melt$number)
```
...
```{r}
# filter the data
droplet.1=filter(droplet.melt,Cell_count==1)

# Plot the data as a bar plot
ggplot(data=droplet.1,
       mapping=aes(x=concentration,y=number))+
  geom_col()
```
...
```{r}
# filter the data
two.plus=filter(droplet.melt,Cell_count>=2)

# sum up the totals by group using the aggregate function
two.sum=aggregate(. ~ concentration,two.plus,sum)

# relabel the cell counts as "2+"
two.sum$Cell_count="2+"
# combine the 1 and 2+ data
num.df=rbind(two.sum,filter(droplet.melt,Cell_count==1))
# Plot the combined data as a bar plot
ggplot(num.df,
       mapping=aes(x=concentration,y=number,fill=Cell_count))+
...
```{r eval=FALSE}
# 100 has the lowest doublet rate. I would chose 100 regardless of the cost, and if the cost matters, I will choose 1000.
```
...
poi.sample=rpois(10^6,0.055)

# make a frequency table from the sampled data
sample.table=table(poi.sample)

# filter the n100 data from the ideal distributions
sample100=filter(droplet.melt,concentration=="pdf100")

# use xtabs to make a table of the ideal n100 data
ideal100=xtabs(number ~ Cell_count,sample100)
...
ideal100=ideal100[1:4]

# bind the data together into a single table
com.table=rbind(sample.table,ideal100)
```
...
barplot(com.table,beside=TRUE,legend.text=c("sample","ideal"))
...
```{r eval=FALSE}
# p value is very small, indicating sample follows the ideal distribution
```
...
```{r}
ci.fail.counter = function(chowPop) {
...
```{r eval=FALSE}
# the average failure is smaller
```
