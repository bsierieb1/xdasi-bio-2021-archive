...
author: "Salvador Luria [Omar Qassab]"
...
```{r eval=FALSE}
# Yes this is a good model for this data. We use Poisson when looking at a distribution of a rare event. The chance of a cell being in a droplet is really low.
```
...
cell_droplet_df_full = data.frame(cbind(range, cell.droplet.pmf.n100, cell.droplet.pmf.n1000, cell.droplet.pmf.n2000))
colnames(cell_droplet_df_full) = c("Cell_count", "100 cell/uL", "1,000 cell/uL", "2,000 cell/uL")

# look at the first few lines of the data.frame
head(cell_droplet_df)
...
melt_df = melt(cell_droplet_df_full, 
               id.vars = "Cell_count",
               value.name =  "Density",
...
melt_df_1e6 = melt_df %>% mutate( "Droplets" = Density * 1e6)
head(melt_df_1e6)

# check sum of counts
melt_df_1e6 %>% group_by(Concentration) %>% summarise("totals" = sum(Droplets))
```
...
```{r}
# filter the data
melt_filter_df = melt_df_1e6 %>% filter(Cell_count == 1) %>% mutate(Cell_count = as.character(Cell_count))

# Plot the data as a bar plot
ggplot(data=melt_filter_df,
...
```{r}
# filter the data
melt_df_2 = melt_df_1e6 %>% filter(Cell_count > 1)

# sum up the totals by group using the aggregate function
melt_df_2_aggr = aggregate(formula = Droplets ~ Concentration, data = melt_df_2, FUN = "sum")

# relabel the cell counts as "2+"
melt_df_2_aggr = melt_df_2_aggr  %>% mutate("Cell_count" = "2+")

# combine the 1 and 2+ data
combined_df = bind_rows(melt_filter_df, melt_df_2_aggr)
...
```{r eval=FALSE}
# #The 100cell/uL has the lowest doublet rate. I would be more inclined to pick the 1000 cell/uL to maximize singles, and minimize multiples. 
```
...
freq_table_obs= table(sample)

# filter the n100 data from the ideal distributions
n100 = melt_df_1e6 %>% filter(Concentration == "100 cell/uL")

# use xtabs to make a table of the ideal n100 data
xtabs_table = xtabs(formula = Droplets ~ Cell_count, data = n100)
xtabs_table
...
trunc_table= xtabs_table[1:4]

# bind the data together into a single table
combined_table = rbind(freq_table_obs, trunc_table)
combined_table
```
...
combined_freq_df = data.frame(t(combined_table))

combined_freq_df$cell_count = c(0:3)
combined_freq_df

ideal_obs = melt(combined_freq_df,
                 id.vars = "cell_count",
                 value.name = "frequency",
                 variable.name = "observation")

ggplot(data = ideal_obs,
       mapping = aes(x = cell_count,
                     y = frequency,
                     fill = observation)) +
...
```{r eval=FALSE}
# The sampled data is different than the ideal data according to the chi-squared test
```
...
```{r}
ci.fail.counter = function( chowPop = NA, N=12, n.samples=100 ) {
...
ci_fail_table = table(ci.fail.count)
ci_fail_table
```
...
ci_fail_df= data.frame(ci_fail_table)
ggplot(data=ci_fail_df,
...
# make a table to check the results
ci_fail_table.t = table(ci.fail.count.t)
ci_fail_table.t
```
...
ci_fail_df.t= data.frame(ci_fail_table.t)
ggplot(data=ci_fail_df.t,
...
```{r eval=FALSE}
# #The t-distribution has a failed count closer to 5, which is what we expect
# in a CI of 95%. The z-distribution produces a failed count closer to 7. 
# This means that the t-distribution is better job at estimating the CIs.
```
