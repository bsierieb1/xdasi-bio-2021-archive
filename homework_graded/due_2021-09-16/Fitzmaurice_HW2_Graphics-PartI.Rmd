...
# ============================================================================ ## 
# load the file
tigers.df = read.csv("Ex2.2A_tigers.csv", stringsAsFactors = FALSE)
...
colnames(tiger.table.df)[1] <- "Activity"
colnames(tiger.table.df)[2] <- "Frequency"
...
write.csv(tiger.table.df, file = "Tigers.csv")
...
pdf("tigers_freq_plot.pdf") # save bar graph as pdf, must end chunck with dev.off()
...
tiger.barplot = barplot(tiger.table.df$Frequency, ylim = c(0,50))
...
```{r, error=TRUE}
Activity=tiger.table.df$Activity
Frequency=tiger.table.df$Frequency
# ============================================================================ #
...
ggsave("Tiger_freq_ggplot.png",my.ggplot)
```
...
```{r, collapse=TRUE, error=TRUE}
# load the data
salmonSizeData <- read.csv("Fig2.2-5_SalmonBodySize.csv", stringsAsFactors = FALSE)
...
avianMalaria <- read.csv("Ex2.3A_malaria.csv", stringsAsFactors = FALSE)
...
ggplot(data = avianMalaria, (aes(x = avianMalaria[,2],
                     fill = avianMalaria[,3]))) +
...
ggplot(data = salmonSizeData, (aes(x = salmonSizeData[,4],
                                   y = salmonSizeData[,5]))) +
            geom_point(size = 1,
                       col = "firebrick",
                       shape = 1) +
