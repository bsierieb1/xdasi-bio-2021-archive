...
# ============================================================================ ## 
# load the file
tigers.df = read.table(header=TRUE, sep=",", stringsAsFactors = F, "data/Ex2.2A_tigers.csv")
...
str(tiger.table.df)
colnames(tiger.table.df)
tiger.table.df$Activity
tiger.table.df$Frequency


# ============================================================================ #
# write the table to a file
write.table(tiger.table.df, "data/tiger_attack_freq.csv", sep=",", quote = F, row.names = F)
...
```{r, error=TRUE}
barplot(height = tiger.table, col = "firebrick")
...
tiger.barplot = barplot(height = tiger.table, col = "firebrick", ylim = c(0,50), axisnames=F)
...
ggsave("data/fig_2_2-1.png", device = "png", my.ggplot)
```
...
```{r, collapse=TRUE, error=TRUE}
# load the data
salmonSizeData <- read.table("data/Fig2.2-5_SalmonBodySize.csv", header=TRUE, sep=",",  stringsAsFactors = F )
...
hist(salmonSizeData$massKg, col = "firebrick", xlab = "Body mass(kg)" )


# ============================================================================ #
# b. Replot with bin size (interval width) = 0.1
hist(salmonSizeData$massKg, col = "firebrick", xlab = "Body mass(kg)",
     breaks = 40, xlim = c(1,4), ylim = c(0,30), main = "Salmon Size Distribution")
...
qplot(massKg, data = salmonSizeData, col=I("black"), fill=I("firebrick"),
      bins = 40 )
...
avianMalaria <- read.table("data/Ex2.3A_malaria.csv", header = T, sep=",",
                           stringsAsFactors = F)
avianMalaria
...
# ============================================================================ #
# side-by-side
barplot(avianMalaria.table, beside = T)
...
plot(data = salmonSizeData, massKg ~ lengthMm, col = "firebrick", 
     xlab = "Salmon length", ylab = "Salmon mass")
...
ggplot(data = salmonSizeData, aes(x = lengthMm, y = massKg)) +
  geom_point(size = 1, col = "firebrick", shape = 1) +
  labs(x = "Salmon length", y = "Salmon mass") +
