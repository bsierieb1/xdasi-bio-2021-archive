...
a. Load the data in the `Ex2.2A_tigers.salm` file into a data frame called `tigers.df` and take a look at it. 
...
tigers <- read.csv(file='Ex2.2A_tigers.csv')
tigers.df <- data.frame(tigers, stringsAsFactors = TRUE)
...
b. i) First, create a **frequency table** containing the counts for each category, arranged in descending order (you may choose to do these operations sequentially, or all in one line):
  ...
  iii) Finally, save this table as a new file  in the `data` folder called `tiger_attack_freq.salm`. Make sure to include column names but not row names. Check the docs on how to specify the right column names.
...
tiger.table = sort(table(tigers.df$activity), decreasing = TRUE)
...
write.csv(tiger.table.df, file = 'tiger_attack_freq.csv', row.names = FALSE)
```


## I.2) Bar chart (1-dimensional categorical data)

Let's reproduce Figure 2.2-1. If you still have `tiger.table.df` in your environment, you can use that directly, otherwise load the `.salm` file you saved above into a data frame with the same name.
...
```{r, error=TRUE}
barplot(height=tiger.table.df$Frequency, col = "firebrick")
```
...
ggsave("my.file.png", plot = my.ggplot)
```
...
Let's reproduce the histogram shown on the left in Figure 2.2-5, which shows the frequency distribution of body mass for 228 female sockeye salmon from Pick Creek, Alaska in 1996, using the data in the corresponding `.salm` file.

Read the file into a data frame called `salmonSizeData` and take a look at it.

```{r, collapse=TRUE, error=TRUE}
# load the data
salmonSizeData <- data.frame(read.csv(file='Fig2.2-5_SalmonBodySize.csv',stringsAsFactors = TRUE))
...
hist(salmonSizeData$massKg, col= "firebrick", xlab='As in book :D')

# ============================================================================ #
# b. Replot with bin size (interval width) = 0.1
hist(salmonSizeData$massKg, col= "firebrick", xlab='As in book :D', bin = 0.1, ylim = c(0,30), xlim = c(1,4), breaks = 5)
...
ggplot(data=salmonSizeData,
        mapping=aes(x=salmonSizeData$massKg)) +
...
avianMalaria <- read.csv(file = "Ex2.3A_malaria.csv")
...
# ============================================================================ #
# add "marginal" totals
x = addmargins(avianMalaria.table, margin = c(1,2), FUN = sum, quiet = TRUE)
```
...
transposed <- t(avianMalaria.table)
mosaicplot(transposed)
```
...
plot(x=salmonSizeData$lengthMm,
     ...
     col="firebrick",xlab='Salmon size, mm',
      ylab='Salmon weight, kg')
# ============================================================================ #
# add the best-fit regression line for these data
# (execute the entire code block at once to see the result)
abline(lm(salmonSizeData$massKg ~ salmonSizeData$lengthMm, data = salmonSizeData))
```
...
ggplot(data=salmonSizeData,
       mapping=aes(x=salmonSizeData$lengthMm,
                   y=salmonSizeData$massKg, color="fire red")) +
  geom_point(color = "red" , size = 3 , shape = 1) +
   labs(x = "salmon size", y = "salmon weight") +
  stat_smooth(method=lm, se=FALSE, color="black", size=0.5)+
  theme_classic() +
geom_smooth()   #it came as an error, so I added this info to remove the error



```

***Now you're cooking with gas!(yes!!!) Hand in your homework and go relax!***
