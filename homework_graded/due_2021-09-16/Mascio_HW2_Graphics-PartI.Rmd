...
tiger.table = table(tigers.df[,2])
tiger.table = sort(tiger.table, decreasing = TRUE)
tiger.table.df = as.data.frame(tiger.table)

# Option 2: all at once ---------------------------------------------------- #
# tiger.table.df = as.data.frame(sort(table(tigers.df[,2]), decreasing = TRUE))
...
pdf("tigers_freq_plot.pdf") #opening device to save as pdf
...
# 6. close the device
#    (skip for part b; we will add this in part c)
dev.off()   #turning device off to save pdf
...
my.ggplot = ggplot(tiger.table.df, aes(x = Activity, y = Frequency)) +
...
```{r, collapse=TRUE, error=TRUE}
# load the data
salmonSizeData <- read.csv("./data/Fig2.2-5_SalmonBodySize.csv", header = TRUE, stringsAsFactors = TRUE)
...
hist_1 <- hist(salmonSizeData$massKg, col = "firebrick", xlab = "Body mass (kg)")
hist_1
# ============================================================================ #
# b. Replot with bin size (interval width) = 0.1
break_num = round((max(salmonSizeData$massKg) - min(salmonSizeData$massKg))/0.1)
hist_2 <- hist(salmonSizeData$massKg, col = "firebrick", xlab = "Body mass (kg)",
     breaks = break_num,
     xlim = c(1,4),
     ylim = c(0,30),
...
qplot(x = massKg, data = salmonSizeData, col = I("black"), fill = I("firebrick"))

# ============================================================================ #
# b. body mass histogram with ggplot
ggplot(salmonSizeData, aes(x = massKg)) + 
...
avianMalaria <- read.csv("./data/Ex2.3A_malaria.csv", header = TRUE, stringsAsFactors = FALSE)
head(avianMalaria)
...
avianMalaria.table = table(avianMalaria[,-1])
...
a. First, make a basic `mosaicplot()` using the malaria table.
...
ggplot(salmonSizeData, aes(lengthMm, massKg)) +
  geom_point(size = 3, col = "salmon", shape = 1) +
  labs(x = "Size (mm)", y = "Mass(kg)") +
