...
# ============================================================================ ## 
# load the file
tigers.df = read.csv("./data/Ex2.2A_tigers.csv", stringsAsFactors = TRUE, header=TRUE)
...
ggsave("act_fre.png",plot=my.ggplot)
```
...
hist(x=salmonSizeData$massKg,col="firebrick",xlab = "Body mass(kg)",ylab = "Frequency")
...
hist(x=salmonSizeData$massKg,col="firebrick",xlim=c(1,4),ylim=c(0,30),breaks=30,xlab = "Body mass(kg)",ylab = "Frequency",main="Salmon Size Distribution")
...
qplot(salmonSizeData$massKg,data=salmonSizeData,col=I("black"), fill=I("firebrick"))
...
ggplot(data=salmonSizeData,
                   mapping=aes(x=massKg))+
  geom_histogram(col="black",fill="firebrick",breaks=seq(1,4,by=0.1))+
    xlim(c(1,4))+
  ylim(c(0,30))+
labs(x = "Body mass(kg)", y = "Frequency", title="Salmon Size Distribution") 
...
avianMalaria <- read.csv(
  "./data/Ex2.3A_malaria.csv",stringsAsFactors = FALSE)
  avianMalaria
...
avianMalaria.table = table(avianMalaria)
avianMalaria.table
# ============================================================================ #
# add "marginal" totals
avianMalaria.table=addmargins(avianMalaria.table, margin = c(1,2), FUN = sum, quiet = TRUE)
```
...
# ============================================================================ #
# stacked
barplot(t(avianMalaria.table["sum",,]))

# ============================================================================ #
# side-by-side
barplot(t(avianMalaria.table["sum",,]),beside=TRUE)
```
...
```{r, error=TRUE}
mosaicplot(avianMalaria.table["sum",1:2,1:2])
```
...
mosaicplot(t(avianMalaria.table["sum",1:2,1:2]))
```
...
plot(salmonSizeData$lengthMm,salmonSizeData$massKg,col="firebrick",xlab="size",ylab="mass")
...
ggplot(data=salmonSizeData,
  ...
  geom_point(size=1,col="red",shape=1)+
   xlab("size") +      
  ylab("mass")+
