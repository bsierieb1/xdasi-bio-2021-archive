...
```{r eval=FALSE}
"An R workspace is all the data elements created in a current coding session or 
loaded from a previous session. You can save and load workspaces by using the save()
and load() functions. 
```
...
```{r eval=FALSE}
"A working directory is a file path which holds the files currently being worked with. You can set the directory by using the setwd function.  The getwd will print the current working directory."
```

I.3) Where does R save the history of your commands?

```{r eval=FALSE}
"The history of commands used is in the History tab of the Envirnoment/Histroy window." 
```
...
```{r eval=FALSE}
"Rstudio projects is a file you save a working directory in.  The R workspace contains all of the data elements defined in the current session or loaded from previous sessions. The Rstudio project file opens a saved workspace.  The big advantage of using R projects is all the files and data belonging to a particular project are located in one place."
```

I.5) How do you start a new R script in RStudio?

```{r eval=FALSE}
"You can start a new R script by going to File -> New File -> R Script."
```

I.6) What does it mean when R prompts you with a  "+" in the beginning of the line?

```{r eval=FALSE}
"The + prompt means R is expecting more code from the user to finish a command."
```

I.7) What are three ways you can retrieve documentation related to a function and its usage in RStudio?

```{r eval=FALSE}
"1)Go to the Help tab within the  Files / Plots / Packages / Help window and search for the function of interest" 
"2) Within the Console you can type ? before the function of interest"
"3)Also, you can use the help() function"
```

I.8) What are the three basic components of R Markdown files?

```{r eval=FALSE}
"YAML metadata, Text, and Codechunks"
```

I.9) What happens when you knit an R Markdown file?

```{r eval=FALSE}
"The code is embedded into a finished document"
```
...
"You can click Run -> Run Selected Line(s) or use the shorcut Cmd + return"

# what are two ways you can run an entire code block at once?
"You can click Run -> Run Current Chunk or use the shorcut Up + Cmd + return"

# how do you prevent a code block from being executed when you knit the document?
"By setting eval=FALSE"
...
```{r eval=FALSE}
# rbind() sets names the row after the variable used while matrix does not.
```
...
```{r eval=FALSE}
"Data frames are matrices that can contain both numeric and character columns 
while a matrix can only can numeric or character columns." 
```
...
```{r}
fly_pheno = read.table("data/hw1_fly_phenotypes.tsv",
            header = TRUE,
...
levels(fly_pheno$eye_color)[2] = "WHITE"
