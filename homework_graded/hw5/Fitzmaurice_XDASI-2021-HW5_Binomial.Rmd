...
#Yes, because if you only have 4 Wnt-insensitiveout of 6 cells total you would 
#need two Wnt-responsive cells
```
...
```{r}
# with choose
choose(n = 6, k = 4)*(pI^4)*((1-pI)^(6-4))

choose(n = 6, k = 2)*(pW^2)*((1-pW)^(6-2))
...
choose(n = 6, k = 2)*(pW^2)*((1-pW)^(6-2)) +
choose(n = 6, k = 1)*(pW^1)*((1-pW)^(6-1)) + 
choose(n = 6, k = 0)*(pW^0)*((1-pW)^(6-0))
...
```{r}
1-choose(n = 6, k = 0)*(pW^0)*((1-pW)^(6-0))
...
library(binom)
moth.ci = binom.confint(x = 35 , n = 50, conf.level = 0.95)
moth.ci
...
cat("Mean estimate = ", moth.ci$mean  , 
    "; 95%CI = ",moth.ci$lower  * 50 ,
    "-", moth.ci$upper * 50)
```
...
plot(x= x.range, y= dbinom(0:50, 50, p=0.11), type= "h",
     main="PDF",
     xlab="expected # of white moths", ylab="point probability")
plot(x= x.range, y= pbinom(0:50, 50, p=0.11) , type= "h",
     main="CDF",
...
cdf(35,50,0.11)#not sure why its not working  
sum(dbinom(0:35,50,0.11))
...
ideal.sample = c(dbinom(0:50, sample.size, p.success))
ideal.sample

# convert probabilities to frequencies
# (multiply by number of replicate experiments)
replicates = 20
ideal.freq = round(replicates * ideal.sample)
ideal.freq
...
#Makes sense since P(W) is small, 0.11
