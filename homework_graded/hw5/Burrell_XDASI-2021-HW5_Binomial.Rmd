...
```{r}
sum(dbinom(1:3, size = 3, prob = 0.5))
```
...
```{r}
sum(dbinom(1:3, size = 3, prob = 0.66))
```
...
choose(6,2) * pW^2 * pI^4 + choose(6,1) * pW^1 * pI^5 + choose(6,0) * pI^6
# with a binom function
sum(dbinom(0:2, 6, 0.11))
```
...
```{r}
pbinom(6, 6, prob = 0.11) - pbinom(0, 6, prob = 0.11)
```
...
sum(dbinom(0:35,50,0.87))
# is this significant?
# Given that the probability of seeing only 35 moths is less than 1%, it does seem like a significant result. Also, p<<0.05.
```
...
plot(x= x.range, y= dbinom(x.range, 50, 0.11), type= "histogram-like vertical lines",
     main="PDF",
     xlab="expected # of white moths", ylab="point probability")
plot(x= x.range, y= pbinom(x.range, 50, 0.11), type= "stair-steps",
     main="CDF",
...
plot(x= x.range, y= dbinom.W, type= "histogram-like vertical lines",
     main="PDF",
     xlab="expected # of white moths", ylab="point probability")
plot(x= x.range, y= pbinom.W, type= "stair-steps",
     main="CDF",
     xlab="expected # of white moths", ylab="total probability")
...
# The most frequent number of W observed is 5 out of 50.
# How does this gibe with the given probability P(W)?
# This makes sense given that P(W) is 0.11, and 0.11*50 = 5.5.
# What is the total height of all the bars if you sum them all up?
# The total height of all the bars will be 1.
# What does this number correspond to?
# This number corresponds to the total probability or the probability of observing any combination of white and black moths.
# Where would you expect to the peak of the distribution to be if P(W)  = 0.35?
# The meak distribution for P(W) = 0.35 should be between 17 and 18 (0.35*50=17.5).
```
