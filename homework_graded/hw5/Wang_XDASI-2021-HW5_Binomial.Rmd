...
```{r}
choose(6,4)
"When 4 insensitive cells are choosen, those 2 cells left automatically are choosen to be responsive"
```
...
choose(6,2)*(pI)^4*(pW)^2+choose(6,1)*(pI)^5*(pW)^1+choose(6,0)*(pI)^6
# with a binom function
pbinom(2,6,p=pW)
```
...
```{r}
pbinom(0,6,p=pW,lower.tail=FALSE)
```
...
library(binom)
moth.ci = binom.confint(x=43.5,n=50)
moth.ci
...
plot(x= x.range, y= dbinom(x.range,50,p=0.11), type="histogram-like vertical lines" ,
     main="PDF",
     xlab="expected # of white moths", ylab="point probability")
plot(x=x.range , y= pbinom(x.range,50,p=0.11), type="stair-steps" ,
     main="CDF",
...
# PDF
dbinom.W = dbinom(x.range,50,p=0.11)

# CDF
pbinom.W = pbinom(x.range,50,p=0.11)
