...
```{r}
# I setted pA = 0.5 and pa = 0.5 for the sake of calculations. 
...
```{r}
#dbinom(1, size=1, prob=paa)
#dbinom(1:3, size=3, prob=paa)
dbinom(1, size=3, prob=paa)
```
...
```{r}
#dbinom(1:3, size=3, prob=paa)
#sum(dbinom(1:3, size=3, prob=paa))
# 1 - sum(dbinom(1:3, size=3, prob=paa))
dbinom(3, size=3, prob=pAa)
```
...
```{r}
#6C4 for I probability
n = 6
k = 4
choose(n,k) * pI^k * (1-pI)^(n-k)
#6C2 for W probability
n = 6
k = 2
choose(n,k) * pW^k * (1-pW)^(n-k)
"The probability of appearance depends on the product of the number of times the item appears, so if the number of times is the same, the probability is the same."
...
```{r}
sum(dbinom(x=1:6, size=6, prob=pW))
```

***Method 4:*** Use the `pbinom()` function

```{r}
pbinom(5, size=6, prob=pI)
```
...
pbinom(q=k, size=n, prob=q)
pbinom(q=k, size=n, prob=q, lower.tail = F)
...
#cat("Mean estimate = ", <mean>  , 
#    "; 95%CI = ",       <lower ci * 50> ,
#    "-",                <upper ci * 50> )
cat("Mean estimate = ", sum(moth.ci$mean)/length(moth.ci$mean)  , 
    "; 95%CI = ", sum(moth.ci$lower)/length(moth.ci$lower) * 50 ,
    "-", sum(moth.ci$upper)/length(moth.ci$upper) * 50 )
print("")
cat("Mean estimate = ", moth.ci$mean[moth.ci$method == "agresti-coull"] , 
    "; 95%CI = ", moth.ci$lower[moth.ci$method == "agresti-coull"] * 50 ,
    "-", moth.ci$upper[moth.ci$method == "agresti-coull"] * 50 )
```
...
plot(x=x.range , y=dbinom(x.range, size=50, prob=0.11), type="h" ,
     main="PDF",
     xlab="expected # of white moths", ylab="point probability")
plot(x=x.range , y=pbinom(x.range, size=50, prob=0.11), type="s" ,
     main="CDF",
     xlab="expected # of white moths", ylab="total probability")
...
# PDF
dbinom.W = dbinom(x.range, size=50, prob=pW)

# CDF
pbinom.W = pbinom(x.range, size=50, prob=pW)
...
```{r}
"The ggplot needs to be a data frame."
```
...
cdf <- function(x,n,p) {
  ...
  result = 0
  for (i in 0:x) { 
    #result[i] <- choose(n, i) * p^i * (1 - p)^(n - i)
...
ideal.sample = dbinom(0:sample.size, size=sample.size, prob=p.success)
ideal.sample

# convert probabilities to frequencies
# (multiply by number of replicate experiments)
replicates = 20
ideal.freq = round(replicates * ideal.sample, digits = 0)
ideal.freq
...
for (i in 0:sample.size) {
  ideal.counts <- c(ideal.counts, rep(i, ideal.freq[i + 1]))
}
ideal.counts
