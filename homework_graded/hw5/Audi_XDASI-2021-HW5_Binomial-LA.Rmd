...
Many disease genes have autosomal recessive phenotypes. Below, assume that both parents are heterozygous for two alleles, A and a, and the aa genotype displays the disease phenotype.
...
#prob 1 will have it and the other two won't
0.25 * 0.75 * 0.75
...
```{r}
# all three children will have it so 0.5 * 0.5 *0.5 
...
```{r}
#1 - the probability that they will be either a AA or AA
1- (0.5 * 0.5 * 0.5)
...
AA = 1/3 # changes because aa doesn't survive
Aa = 2/3
AA
#1 - minus the prob you are not a carrier three times 
1- (AA)^3
...
#prob you are a carrier 
1-0.3333
#prob you are carrier three times 
0.666^3
```
...
```{r}
# combinations - doesn't matter the order: n!/(n-k)!*k!
#  6!/((6-4)!(4)!)
factorial(6)/(factorial(6-4) * factorial(4))
#or with choose - does the work for you!!!
choose(6,4)
...
# Becasue the denominatoris the same in both instances (8) -- its a binomial -- only two options 
...
#calculate total possible ways to get 4 of the same then multiply by probability of each 
choose(6, 4) * pI^4 * pW^2
#can run either -- is the same 
choose(6,2)* pI^4 * pW^2
# with a binom function 
#dbinom is a probability mass function of binomial distribution
#dbinom = probability of observing value equal to x
dbinom(x= 4, size = 6, prob= pI) #prob of getting 4 I out of 6 
dbinom(2, 6, pW) # porb of getting 2W of of 6 
...
#give all the possible options for pW = 2, 1 & 0
(choose(6, 2) * pW^2 * pI^4) + (choose(6, 1) * pW^1 * pI^5) + (choose(6, 0) * pW^0 * pI^6)

# with a binom function
#pbinom is a cumulative distribution function of this distribution
#pbinom = probability of observing value smaller or equal to x
pbinom(q =2 , size =6, prob = pW)
```
...
# your answer here
#sum all options 1-6 instead of writing it out 
sum(choose(6,1:6))
```
...
```{r}
#use dbinom because its exact
dbinom(x=6, size=6, prob = pI)
...
```{r}
#prob of i and w = 1 so prob of not I = 1 - prob of I 
1 - (pI^6)
...
```{r}
#pbinom gives the prob of less than, so prob of 0 W - 1 = prob of 1-6 
1 - pbinom(0, 6, pW)
```
...
# is this significant?
# yes it is significant , it is less than 0.05%
```
...
plot(x= x.range, y= dbinom(0:50, 50, 0.11), type="h" , #histogram like lines 
     main="PDF",
     xlab="expected # of white moths", ylab="point probability")
plot( x.range, y = pbinom(0:50, 50, 0.11),  type= "s" , #stair steps 
     main="CDF",
