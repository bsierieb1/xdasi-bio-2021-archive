...
```{r}
# Probability of AA progeny is 0.25
# Probability of Aa (or aA) progeny is 0.5
# Probability of aa progeny is 0.25
```
...
```{r}
# The probability will be 0.25 * 0.75 * 0.75, which is roughly 0.14
0.25 * 0.75 * 0.75
```
...
```{r}
# The probability will be (0.5)^3, so 0.125
...
```{r}
# The probability is 1 - (0.5)^3 which is 0.875.
1 - (0.5)^3
...
```{r}
# The probability of at one child being AA would be 0.3333 instead of 0.25 and the probability of one child being heterozygous would not be 0.6666 instead of 0.5, so the probability that at least one child is a carrier is 0.963
...
```{r}
# That probability that all 3 children will be carriers will be (0.6666)^3, which is 0.296.
0.6666^3
...
```{r}
# combinations 4 out of 6 cells= 6!/((6-4)!(4)!)
factorial(6)/(factorial(6-4) * factorial(4))


# combinations 2 out of 6 cells = 6!/((6-2)!(2)!)
factorial(6)/(factorial(6-2) * factorial(2))

# These both result to the same value because of the denominator becomes the same whether you want two cells or four
...
# PDF
dbinom.W = dbinom(0:50, 50, 0.11)
dbinom.W

# CDF
pbinom.W = pbinom(0:50, 50, 0.11)
pbinom.W
...
```{r}
# We would need a dataframe to use ggplot or the hist() function.
```
