...
```{r}
p.one_of_three = (p.AA + p.Aa) * (p.AA + p.Aa) * p.aa
p.one_of_three
```
...
```{r}
# p.atleast_one = 1 - p.none
p.none_Aa = 0.5 * 0.5 * 0.5
p.atleast_one = 1 - p.none_Aa
p.atleast_one
```
...
p.AA_lethal = 1/3
p.Aa_lethal = 2/3
p.none_Aa_lethal = 1/3 * 1/3 * 1/3
p.atleast_one_lethal = 1 - p.none_Aa_lethal
p.atleast_one_lethal
```

f. What is the probability that all 3 children will be carriers?

```{r}
p.all_three_lethal = p.Aa_lethal * p.Aa_lethal * p.Aa_lethal
```
...
p.wwiiii = (pW*pW*pI*pI*pI*pI)
p.wwiiii

# 2. A sequence of IIWWII
p.iiwwii = pI*pI*pW*pW*pI*pI
p.iiwwii
...
```{r}
c.four_i = factorial(6) / (factorial(4) * 2)
c.four_i
```
...
```{r}
# with choose
p.4_I = choose(6, 2) * p.iiwwii
p.4_I

# with a binom function
pbinom(2, 6, pW, lower.tail = TRUE) - pbinom(1, 6, pW)
...
p.two_or_less_1 = (choose(6, 2) * pW^2 * pI^4) + (choose(6, 1) * pW * pI^5) + choose(6, 0) * pI^6
p.two_or_less_1

# with a binom function
p.two_or_less_2 = pbinom(2, 6, pW, lower.tail = TRUE)
p.two_or_less_2
...
p.obs_moth = choose(50, 35) * p^35 * q^15
p.obs_moth
...
# is this significant?
# Yes. The probability of observing 35 white moths or less in a sample population of 50
# is 0.12%. A p-value cutoff of 5% is much higher.
...
cat("Mean estimate = ", 0.69607  ,      # taken from Bayes method
    "; 95%CI = ",       0.5700 * 50 ,
    "-",                0.8178 * 50 )
```
...
```{r}
# I would need a 50 x 2 data frame that has the number of observed white moths in the first column
# and their respective probabilities of occurence in the second column.
```
...
p_w_11 = cdf(35, 50, 0.11)
p_w_87 = cdf(35, 50, 0.87)
...
replicates = 20
ideal.freq = round(ideal.sample * replicates)
ideal.freq
...
for (i in 1:sample.size) {
  ideal.counts <- c(ideal.counts, rep(i, ideal.freq[i]))
