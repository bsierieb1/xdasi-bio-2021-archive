...
# p.healthy = p.AA + p.Aa
p.healthy = 0.75
p.1of3disease = choose(3,1)*(p.disease^1)*(p.healthy^2)
p.1of3disease
```
...
# p.carrier = p.Aa
p.carrier = 0.5
p.3of3carrier = choose(3,3)*(p.carrier^3)*(p.healthy^0)
p.3of3carrier
```
...
```{r}
pbinom(1,3,0.5,lower.tail=FALSE)
```
...
p.AA.new = 0.33
p.carrier.new = 0.67
pbinom(1,3,0.67,lower.tail=FALSE)
```

f. What is the probability that all 3 children will be carriers?

```{r}
p.3of3carrier.new = choose(3,3)*(p.carrier.new^3)*(p.AA.new^0)
p.3of3carrier.new
```
...
```{r}
choose(6,4)
# Because choose() function, or combination has the denominator n-k! and k!. 6-4 = 2 and this is multiplied by 4!. Its the exact same calculation when you multiply 6-2 = 4! with 2!.
```
...
```{r}
(factorial(6)/(factorial(5)*factorial(1)))*(0.11^1)*(0.89^5)+
  (factorial(6)/(factorial(4)*factorial(2)))*(0.11^2)*(0.89^4)+
  (factorial(6)/(factorial(3)*factorial(3)))*(0.11^3)*(0.89^3)+
  (factorial(6)/(factorial(2)*factorial(4)))*(0.11^4)*(0.89^2)+
  (factorial(6)/(factorial(1)*factorial(5)))*(0.11^5)*(0.89^1)+
  (factorial(6)/(factorial(0)*factorial(6)))*(0.11^6)*(0.89^0)
```
...
choose(6,1)*(pW^1)*(pI^5)+
  choose(6,2)*(pW^2)*(pI^4)+
  choose(6,3)*(pW^3)*(pI^3)+
  choose(6,4)*(pW^4)*(pI^2)+
  choose(6,5)*(pW^5)*(pI^1)+
  choose(6,6)*(pW^6)*(pI^0)
```
...
```{r}
pbinom(0,6,0.11,lower.tail = FALSE)
```
...
pbinom(35,50,0.87)
# is this significant?
# 0.001285 is less than alpha = 0.05 so the null hypothesis that we would normally find no more than 35 white moths on average is rejected.
```
...
library(binom)
moth.ci = binom.confint(35,50,conf.level = 0.95)
moth.ci
...
# PDF
dbinom.W = dbinom(x.range,50,0.11)

# CDF
pbinom.W = pbinom(x.range,50,0.11)
