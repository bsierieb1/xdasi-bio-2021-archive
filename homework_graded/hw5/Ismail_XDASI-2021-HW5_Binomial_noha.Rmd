...
z <- factorial(6) / (factorial(4) * factorial(2))
z

#or

choose(6,4)
#since the total is 6 cells in both, so it will be the same when we take factorial of (6) and divided by
#factorial 2 * factorial 4 -- they are complimentary to each other
...
choose(6,1) * (pI)^5 * (pW)^1 + choose(6,0) * (pW)^0 * (pI)^6


# with a binom function

pbinom(2,6,0.11, lower.tail = TRUE)
...
pbinom(q = 35,size = 50, prob = p, lower.tail = TRUE)
# is this significant?

#yes it is significant, since 0.001 is less than 0.05 which is the threshold for p value
...
library(binom)
moth.ci = binom.confint(35,50,0.95)
moth.ci
...
#it is a vector and ggplot is a data frame
```
