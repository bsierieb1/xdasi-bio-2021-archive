...
author: "John Tukey [Luok Wen Yong]"
...
```{r}
# load the data
ce.df<-read.csv("/Users/lwyong/Desktop/LW Yong NYU/Biostat 2021 Fall/Week 12/XDASI2021-HW11_ANOVA_files/Ce_mel28_Emb_Ste.csv")
...
# they look like their normally distrubuted
# Shapiro.test is performed in section d) to test for normality
...
```{r eval=FALSE}
# ANOVA assumes that the variables compared are normally distributed and the variances of the variables compared are similar. 
```
...
```{r}
ce.wt<-ce.df$brood_size[ce.df$genotype == "WT"]
ce.A<-ce.df$brood_size[ce.df$genotype == "A"]
ce.AB<-ce.df$brood_size[ce.df$genotype == "A;B"]

#test for normality
# all normally distributed because p values are all above 0.05
shapiro.test(ce.wt)
shapiro.test(ce.A)
shapiro.test(ce.AB)
```
...
```{r}
# var.test #all variances are similar as all p values are above 0.05
var.test(ce.wt,ce.A)
var.test(ce.wt,ce.AB)
var.test(ce.A,ce.AB)
...
```{r eval=FALSE}
# The test statistics is the F statistics. If the null is true, it means the F stats is 1, which means there is no significance departure from 1. 
```
...
```{r eval=FALSE}
# Yes, because the variables are normally distributed as per shown by the shapiro test.
# And the variances are similar as shown by both the variance test and Bartlett's test. 
```
...
```{r}
ce.df.mean<- ce.df %>% group_by(genotype) %>% mutate(mean=mean(brood_size))

# SS_error
SS_error = sum((ce.df$brood_size-ce.df.mean$mean)^2)
SS_error

## SS_group
grand_mean = sum(ce.df$brood_size)/length(ce.df$brood_size)
#ce.df.mean2<- ce.df %>% group_by(genotype) %>% summarise(mean=mean(brood_size)) 

SS_group = sum((ce.df.mean$mean-grand_mean)^2)
SS_group
...
```{r}
# degrees of freedom for SS_error (within group)
df_sserror = length(ce.df.mean$brood_size) - nlevels(ce.df.mean$genotype)
df_sserror

# degrees of freedom for SS_groups (between groups)
df_ssgroups = nlevels(ce.df.mean$genotype)-1
df_ssgroups

# F-statistic
F.stat.ce.brood <- (SS_group/df_ssgroups)/(SS_error/df_sserror)
F.stat.ce.brood

# p-value using CDF for F-statistic
pf(F.stat.ce.brood,df_ssgroups,df_sserror, lower.tail = F)
...
```{r eval=FALSE}
# This is a planned dataset as we already have know the means we want to measure prior to actually obtaining the data. In this case, we already know we wanna measure the means of brood sizes for WT, A strain and A;B strains. If it was an unplanned dataset, we will still do an ANOVA test first, and then we perform additional Tukey Kramer test. 
```
...
ce.df.lm<-lm(ce.df$brood_size ~ ce.df$genotype)
anova(ce.df.lm)
anova(ce.df.lm)$"Pr(>F)"
```
...
```{r eval=FALSE}
# Call shows you the linear model you are testing. 
# Residuals is the deviation of the real data Y from the Y hat that lies on the regression line. This part shows us the dsitribution of the residuals. 
# Coefficient: The estimate column shows the relationship of the explanatory variable (genotype A, genotype A;B).the first row shows that intercept of the linear regression model. The other estimates show us the magnitude of change of the response variable (brood_size) in response to change in genotype. It is also the slope of the linear model. The standard error portion shows us the error of the estimate sampled slope. The T-value is the test statistics generated for testing if the estimated slope is significantly different from the null at alpha = 0.05. The Pr(>|t|) is the p-value that tell us if the slope is indeed significantly different from the null and thus the explanatory variable does impact the response variable. 
# Residual standard error is the standard error of the residuals
#R squared and Adjusted R squared and the F statistic in general tells use how well our model explains the variation of the data (the dependant variable). 
# R squared is the variation that is explained by the model in relation to the total variation. 
# Adjusted R-squared is just the R squared normalised to the amount of samples we have and the variables we are testing. 
# The F statistic is another test that shows us whether one of our group variation affect the variance of our data significantly. 
...
```{r eval=FALSE}
# Yes. Yes all the different groups are significantly different from the controls because all the p-values are lower than 0.05. 
```
...
```{r}
aov.ce.brood<-aov(ce.df$brood_size ~ ce.df$genotype)
aov.ce.brood
```
...
```{r}
TukeyHSD(aov.ce.brood)
```

What does Tukey's HSD test do? What do these results tell you? What is the biological interpretation of these results?

```{r eval=FALSE}
# Tukey HSD tell us if the means of each pair of (explanatory variable) combo is significant different. P adj = 0 means the pairs compared are significantly different. 
#biologically, A affects brood size negatively compared to WT. But somehow B will rescue the affects of A but not complete to WT level. 
```
...
```{r}
#plot
ggplot (aov.ce.brood, aes(x=aov.ce.brood$residuals))+
  geom_histogram(bins = 35, fill = "pink", col = "grey")

#normality
shapiro.test(aov.ce.brood$residuals)
...
```{r eval=FALSE}
# The p value here is different. The P value for the Welch's ANOVA is not as significant as the ANOVA performed above (though still lower than 0.05). 
```
...
ce.df.percent<- ce.df %>% group_by(genotype) %>% mutate(percent_fer=(fertilized/brood_size)*100)

# plot it

ggplot(ce.df.percent, mapping = aes(x = genotype,
                                    y = percent_fer,
                                    col = genotype)) + 
  geom_boxplot()+
  geom_jitter()
...
#subset groups
ce.fer.perc.WT<- ce.df.percent$percent_fer[ce.df.percent$genotype == "WT"]
ce.fer.perc.A<- ce.df.percent$percent_fer[ce.df.percent$genotype == "A"]
ce.fer.perc.AB<- ce.df.percent$percent_fer[ce.df.percent$genotype == "A;B"]

#shapiro.test (normal distributino)
shapiro.test(ce.fer.perc.WT)
shapiro.test(ce.fer.perc.A)
shapiro.test(ce.fer.perc.AB)

# histograms
ggplot(ce.df.percent, mapping = aes(x = percent_fer,
                                    fill = genotype)) +
  geom_histogram(alpha = 0.8)

#test for equal variances between groups
bartlett.test (percent_fer ~ genotype, ce.df.percent)
...
```{r eval=FALSE}
# No, since the distribution of the responding variable isn't normal amongst different groups and the variances between groups are also not normal, we have to do the Kruskal wallace test and the pairwise Wilcoxon Rank Sum test. Kruskal wallis test just tell us if there is a different between groups of treatments/explanatory variable buth the pairwise Wilcoxon Rank sum will show a table of the effects of each pair of "treatment" groups. 
```
...
```{r}
#pairwise wilcox test ## this gives more info
pairwise.wilcox.test(ce.df.percent$percent_fer, ce.df.percent$genotype,
                     p.adjust.method = "BH")
...
# compute the power
power.anova.test(groups= nlevels(ce.df.mean$genotype), 
                 n= 30, 
                 within.var= SS_error, 
...
```{r eval=FALSE}
# It feels weird that you can get such a big power from such small sample size. Its hard to believe that you can perform a "powerful" experiment with such little samples. 
```
