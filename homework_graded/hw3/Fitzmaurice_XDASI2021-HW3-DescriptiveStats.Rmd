...
getwd()
data = "/Users/dylanfitzmaurice/R/Biostats/Week_3/Homework3"
setwd(data)
list.files(data)
```
...
mice_pheno_summary = mice_pheno %>% group_by(Sex, Diet) %>% summarise(n_animals = n())
```
...
mice_pheno_complete = mice_pheno %>% filter(complete.cases(.))
```
...
mice_pheno_complete_summary = mice_pheno_complete %>% group_by(Sex, Diet) %>% summarise(n_animals = n())
```
...
```{r}
# your answer here
mice_pheno_complete_summary$n_difference <- mice_pheno_summary$n_animals - mice_pheno_complete_summary$n_animals
mice_pheno_complete_summary
```
...
+ Finally, use `facet_wrap( ~ Sex)` to draw side-by side plots for females and males.
...
ggplot(mice_pheno_complete,       
       mapping = aes(x = Bodyweight,
                     y = Diet,
                     )) +
   geom_violin() +  # violin
...
```{r}
# mean bodyweight
mean_bodyweight = mean(mice_pheno_complete$Bodyweight)
mean_bodyweight

# sem
std_bodyweight = sd(mice_pheno_complete$Bodyweight)
std_bodyweight

sem_bodyweight = std/(sqrt(length(mice_pheno_complete$Bodyweight)))
sem_bodyweight

# 95% CI
mean(mice_pheno_complete$Bodyweight)-1.96*sem 
1.96*sem+mean(mice_pheno_complete$Bodyweight)
...
```{r}
# your answer here
# It tells you the range of values you have 95% confidence the actual mean lies in
```
...
+ Note that you can refer to individual elements of the vector by index number or element name.
...
ci(mice_pheno_complete$Bodyweight)
```
...
summary_stats = mice_pheno_complete %>% group_by(Sex, Diet) %>% summarise(mean_wieght= round(ci(Bodyweight)["Estimate"], digits = 2),
...
fc = summary_stats %>% filter(Sex == "F", Diet == "chow") %>% pull(-4) 
fh = summary_stats %>% filter(Sex == "F", Diet == "hf") %>% pull(-4) 
  
mc = summary_stats %>% filter(Sex == "M", Diet == "chow") %>% pull(-4) 
mh = summary_stats %>% filter(Sex == "M", Diet == "hf") %>% pull(-4) 
...
```{r}
# your answer here
# Yes, males put on more weight than females.
```
...
```{r}
# subset the data
chow = mice_pheno_complete %>% filter(Sex == "M", Diet == "chow") %>% pull(-1)

hf   = mice_pheno_complete %>% filter(Sex == "M", Diet == "hf") %>% pull(-1)
```
...
s.ctl = sample(chow, size = 12)# control = "chow" group
s.trt =  sample(hf, size = 12) # treatment = "hf" group

# how different are the mean weights in the two samples?
obs.diff = abs(mean(s.ctl)-mean(s.trt))
obs.diff

# mean difference from full dataset
obs.diff.full.dataset = abs(mean(chow)-mean(hf))
obs.diff.full.dataset
```
...
```{r}
# your answer here
# In general they have a lot of variability and many times are 2 times greater or less
# than the  full dataset mean
```
...
replicate(6, mean( sample(chow , size = 12)))
```
...
ggplot(data = resample, 
  ...
  geom_histogram(fill="lightblue", color="black",  # pick a color scheme
                 boundary=0, binwidth = 0.05) +     #set binwidth
...
sample.size = sample(chow, 12)
ctl.sample =  sample(hf, 12)
