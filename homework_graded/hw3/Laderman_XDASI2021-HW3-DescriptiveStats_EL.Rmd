...
author: "Elizabeth Blackburn ERIC LADERMAN"
...
mice_data = read.csv("mice_pheno.csv")
...
mice_data_grouped = mice_data %>% group_by(Sex, Diet) %>% 
...
which(is.na(mice_data$Bodyweight))
```
...
cleaned_mice_data = mice_data[which(complete.cases(mice_data)),]
which(is.na(cleaned_mice_data$Bodyweight)) #This gives zero which is good
...
mice_data_cleaned_grouped = cleaned_mice_data %>% group_by(Sex, Diet) %>% 
  summarise(n_animals = n())

mice_data_cleaned_grouped
...
```{r}
# your answer here
# 4 were removed from m hf and 1 was removed from M chow
```
...
ggplot(data = cleaned_mice_data,
       mapping = aes(x = Diet, y = Bodyweight, fill = Diet)) +
  geom_violin() +
...
mean_bodyweight = mean(cleaned_mice_data$Bodyweight)


# sem

sem = mean_bodyweight / sqrt(length(cleaned_mice_data$Bodyweight))


# 95% CI

CI95 = c(mean_bodyweight - 2 * sem, mean_bodyweight + 2 * sem)
CI95
...
```{r}
# The 95% CI tells us an interval for which we have 95% certainty that the true mean of the population falls within
```
...
ci(cleaned_mice_data$Bodyweight)['CI lower']
...
summary_stats = cleaned_mice_data %>% group_by(Sex, Diet) %>% summarise(mean_weight = ci(Bodyweight)['Estimate'], SEM = ci(Bodyweight)['Std. Error'], CI_lower = ci(Bodyweight)['CI lower'], CI_upper = ci(Bodyweight)['CI upper'])
...
fc = summary_stats[1, 3]
fh = summary_stats[2, 3]

mc = summary_stats[3, 3]
mh = summary_stats[4, 3]

# average weight gain by sex

female_weight_gain = fh - fc
male_weight_gain = mh - mc


# percent weight gain

female_weight_gain_percent = female_weight_gain / fc
male_weight_gain_percent = male_weight_gain / mc
female_weight_gain_percent
male_weight_gain_percent
...
```{r}
# your answer here
#From just the proportion weight data it is hard to say. THe differences are quite small but this alone does not mean that they aren't significant. The violinplots do make me rather skeptical though as the IQRs of the control and treatment group seem to overlap and a rule of thumb I have learned is that if the IQRs of control and treatment overlap their is likely not a significant difference.
```
...
```{r}
# subset the data
chow = cleaned_mice_data[which(cleaned_mice_data$Sex == 'M' & cleaned_mice_data$Diet == 'chow'),]$Bodyweight
chow
hf   = cleaned_mice_data[which(cleaned_mice_data$Sex == 'M' & cleaned_mice_data$Diet == 'hf'),]$Bodyweight
```
...
```{r}
# There is a large range in the potneital samples you draw with some only having a difference of 0.5 while others have a difference closer to 6. The are centered around the value for the whole dataset but certainly display a lot of spread.
```
...
replicate(6  , mean( sample(chow   ,   12)))
```
...
ggplot(data = resample,
  ...
  geom_histogram(fill='aquamarine', color ='black', binwidth = 0.25) +
...
sem = function(pop.data, sample.size=12){
  ...
  return(sd(random.sample) / sqrt(sample.size))
}
# ============================================================================ #
```
...
ci = c(mean(ctl.sample) - 2 * sem, mean(ctl.sample) + 2 * sem)
...
```{r}
# Around 7% of the ci do not contain the true population mean. This varies somewhat but not very much the fewest I observed was 5 and the most I ovbserved was 10.
```


How much does the width of the 95% CI's seem to vary?
```{r}
# The width of the 95% CI's does not seem to vary too much
```

About how often do your CI's contain the mean for the high-fat diet population? What do you think this might mean?
```{r}
# Around 10-20% of the CI's contain the mean for the high fat diet population. I think this suggests that the two treatments are not significantly different with a p value of 0.05
```
