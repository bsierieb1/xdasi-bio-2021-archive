...
```{r}
# load the dataset
mice_pheno <- read.csv("./data/mice_pheno.csv", header = TRUE)
...
sum_n_mice_pheno <- mice_pheno %>% group_by(Sex, Diet) %>% summarise(n_animals = n())
sum_n_mice_pheno
```
...
which(is.na(mice_pheno$Bodyweight))
```
...
clean_mice_pheno <- mice_pheno[complete.cases(mice_pheno),]
clean_mice_pheno
```
...
sum_n_clean_mice_pheno <- clean_mice_pheno %>% group_by(Sex, Diet) %>% summarise(n_animals = n())
sum_n_clean_mice_pheno
sum_n_mice_pheno
```
...
```{r}
# 5 total, 1 from M,chow and 4 from M,hf
```
...
ggplot(clean_mice_pheno, aes(x = Diet, y = Bodyweight)) +
  geom_violin(aes(fill = Diet), width = 0.6) +
...
```{r}
# mean bodyweight
total <- unname(unlist(count(clean_mice_pheno)))
total
mean_mice <- sum(clean_mice_pheno$Bodyweight)/total
mean_mice
# sem
st_dev <- sqrt(sum((clean_mice_pheno$Bodyweight - mean_mice)^2)/(total-1))
st_dev

sem <- st_dev/sqrt(total)
sem

# 95% CI
lower_limit <- mean_mice-1.96*sem
upper_limit <- mean_mice+1.96*sem
lower_limit
upper_limit

# 28.42393 < mu < 29.27017
```

Describe in words what the 95% CI tells you.

```{r}
# the true population mean of mice bodyweight falls between 28.42 and 29.27 
# 19 out of 20 confidence intervals will contain the true population mean
```
...
ci(clean_mice_pheno$Bodyweight)
```
...
summary_stats <-  clean_mice_pheno %>% 
    group_by(Sex, Diet) %>% 
    summarise(mean_weight = ci(Bodyweight)[1],
              SEM = ci(Bodyweight)[4],
              CI_lower = ci(Bodyweight)[2],
              CI_upper = ci(Bodyweight)[3])


summary_stats[,3:6] <- round(summary_stats[,3:6], digits = 2)
summary_stats
```
...
fc <- summary_stats %>% filter(Sex == "F" & Diet == "chow") %>% select(mean_weight) %>% pull()
fh <- summary_stats %>% filter(Sex == "F" & Diet == "hf") %>% select(mean_weight) %>% pull()
  
mc <- summary_stats %>% filter(Sex == "M" & Diet == "chow") %>% select(mean_weight) %>% pull()
mh <- summary_stats %>% filter(Sex == "M" & Diet == "hf") %>% select(mean_weight) %>% pull() 
...
```{r}
# It seems that both sexes do increase weight by around 10%, but more specifically the males increase weight by about 3% more. It is hard to say if this is significant (no tests are run) but t does seem like the sexes gain weight differently
```


# II. Random sampling

What if we only measured weight on different diets for 12 male mice? Since the original dataset is pretty big, let's now treat it as the "population" and see how our data look when we draw random samples of 12 mice from it.

### II.1 Subset the data

+ First, create two subsets of the data containing just the bodyweights for males on each diet.

```{r}
# subset the data
chow <- subset(clean_mice_pheno, subset = Diet == "chow" & Sex == "M")
hf   <- subset(clean_mice_pheno, subset = Diet == "hf" & Sex == "M")
...
```{r}
# The differences in samples vary, however they tend to stay around the difference from the full dataset. Once in a while there is an extreme larger/smaller difference.
```
...
resample = data.frame(sample.mean =  
                        replicate(  n.samples ,                    # number of samples
                                  mean(sample( chow$Bodyweight , sample.size ))) )  # sample and take the mean
str(resample)
head(resample)
...
ggplot(resample, aes(sample.mean)) + 
  geom_histogram(fill = "thistle", binwidth = 1) + # pick a color scheme, set binwidth
  
  # map `sample.mean` to the vertical line (copy syntax from previous histograms above)
  geom_vline(aes(xintercept=mean(sample.mean)), linetype = "dashed")
...
replicate( 10 , sem(chow$Bodyweight))  # compute sem for 10 samples from "chow" population
...
sem_dist = data.frame(sem = replicate(1000, sem(chow$Bodyweight)))
...
sample.size <- 12
ctl.sample <- sample(chow$Bodyweight, size = sample.size)

# compute SEM and 95% CI
sem_by_hand = sd(ctl.sample)/(sqrt(sample.size))
ci_up = mean(ctl.sample) + 1.96 * sem_by_hand
ci_low = mean(ctl.sample) - 1.96 * sem_by_hand
...
```{r}
# around 5%
# often, 4-6 samples do not contain the mean
# once I got as low as 2 and as high as 11, so it can vary but tends to stay around 5%
```


How much does the width of the 95% CI's seem to vary?
```{r}
# Varies a lot, sometimes they are narrow and fit in between the two means without containing either, and other times it is long enough to contain both means
```

About how often do your CI's contain the mean for the high-fat diet population? What do you think this might mean?
```{r}
# 10-15% of the time, this may mean that the difference in bodyweight between food type is not significant
```
