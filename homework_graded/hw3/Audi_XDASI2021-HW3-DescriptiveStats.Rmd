...
mice = read.csv("data/mice_pheno.csv")
...
#use groupby to make a tbl 
mice_sum = mice %>% 
  group_by(Sex, Diet) %>%  
  summarise(n_animals = n()) #summarizes the tbl
...
#use tidyr to remove nas 
library(tidyr)
mice.pheno = mice %>% drop_na()

str(mice.pheno) #5 less obs, 5 na's dropped 
...
```{r}
# your answer here
#5  data points were removed 1 from M, chow and 4 from M, hf 
```
...
ggplot(data=mice.pheno, aes(Diet, Bodyweight)) + 
  geom_violin(aes(fill=Diet)) +# violin 
  geom_boxplot(width=0.5)+ # box
...
```{r}
# mean bodyweight
mice_mean = mean(mice.pheno$Bodyweight)
#28.84

# sem
se<- function(x) sd(x) / sqrt(length(x)) # Create own function
SE = se(mice.pheno$Bodyweight)
#0.2158

# 95% CI
q = qnorm(.975)
ci  = c(mice_mean - q*SE, mice_mean + q*SE)
ci
#ci = 28.42394- 29.27016
```
...
```{r}
# your answer here
#If the bodywieght of 100 mice were taken, 95 should weight 28.4 and 29.3 units. We have 95% certainty that the range 28.4-28.3 contains the true population mean. 
```
...
summary_stats = mice.pheno  %>% 
  group_by(Sex, Diet) %>%  
  summarise(mean_weight=ci(Bodyweight)[1], SEM= ci(Bodyweight)[4], CI_lower=ci(Bodyweight)[2], CI_upper=ci(Bodyweight)[3]) #don't need to specify here bc groupby is already telling it whatto work on 
...
fc = filter(summary_stats, Sex == 'F',
            Diet == "chow")  

fh = filter(summary_stats, Sex == 'F',
            Diet == "hf")            

mc = filter(summary_stats, Sex == 'M',
            Diet == "chow") 

mh = filter(summary_stats, Sex == 'M',
            Diet == "hf") 


# average weight gain by sex
f_avgwg = fh$mean_weight - fc$mean_weight
f_avgwg

m_avgwg = mh$mean_weight - mc$mean_weight
m_avgwg


# percent weight gain
f_pw = (f_avgwg/fc$mean_weight) * 100
f_pw #9.9%

m_pw = (m_avgwg/mc$mean_weight) * 100
m_pw #12.5% 
...
```{r}
# your answer here
#Males gained more weight than females did on average but we can not say from these results if they are statistically significant 
```
...
```{r}
# your answer here
#the sample set varies ALOT! Sometime the control has higher averaage than treatment. Sometime the difference is less than 1 someimes its closer to 2. Highly variable. 
```
...
ggplot(data =resample, aes(x=sample.mean)) + 
  ...
  # map `sample.mean` to the vertical line (copy syntax from previous histograms above)
   ...
   geom_vline(data = resample, aes(xintercept = mean(sample.mean)),color = "red", linetype = "dashed") #couldn't understand what you were asking... think this is what you want 

# ============================================================================ #
# print some summary stats

pop.mean = round(mean(chow$Bodyweight),2) #added $bodyweight so it worked 
...
sem = function(pop.data, sample.size=12){
  ...
  random.sample = sample(mice.pheno$Bodyweight, 12)
...
chow2 = chow$Bodyweight
print("SEM of individual 'chow' samples:")
replicate( 10 , sem(chow2))  # compute sem for 10 samples from "chow" population
...
sem_dist = data.frame(sem = replicate(1000   ,                    
                                  mean(sample(chow2,12))))
...
```{r}
# your answer here
#less than 5%, doesn't vary too much 
```
