...
```{r}
# load the dataset
mice.pheno = read.csv("./data/mice_pheno.csv")
...
mice.pheno_nsex = mice.pheno %>% group_by(Sex, Diet) %>% summarize(n_animals = n())
mice.pheno_nsex
```
...
which(is.na(mice.pheno$Bodyweight))
...
mice.pheno = subset(mice.pheno, complete.cases(mice.pheno))
...
```{r}
# Five data points were removed. One from male-chow, and four more from male-high fat
...
```{r}
# mean bodyweight
mean_bodyweight = mean(mice.pheno$Bodyweight)
mean_bodyweight

# sem
sem_bodyweight = sd(mice.pheno$Bodyweight) / sqrt(length(mice.pheno$Bodyweight))
sem_bodyweight

# 95% CI
ci_bodyweight = c(mean_bodyweight - 2*sem_bodyweight, mean_bodyweight + 2*sem_bodyweight)
ci_bodyweight
...
```{r}
# 95% of the time, a mean calculated from a population sample of this size will fall within this range.
```
...
ci(mice.pheno$Bodyweight)
ci(mice.pheno$Bodyweight)["Estimate"]
ci(mice.pheno$Bodyweight)[1]
...
summary_stats = mice.pheno  %>% group_by(Sex, Diet) %>%
  summarize(mean_weight = ci(Bodyweight)[1],
                  SEM = ci(Bodyweight)[4],
                  CI_lower = ci(Bodyweight)[2],
                  CI_upper = ci(Bodyweight)[3])
...
fc = summary_stats[[which(summary_stats$Sex == "F" & summary_stats$Diet == "chow"),
                    "mean_weight"]]
fh = summary_stats[[which(summary_stats$Sex == "F" & summary_stats$Diet == "hf"),
                    "mean_weight"]]
  
mc = summary_stats[[which(summary_stats$Sex == "M" & summary_stats$Diet == "chow"),
                    "mean_weight"]]
mh = summary_stats[[which(summary_stats$Sex == "M" & summary_stats$Diet == "hf"),
                    "mean_weight"]]

# average weight gain by sex
male_weight_gain = mh - mc  # Subtract the chow-diet weight average from high-fat diet

female_weight_gain = fh - fc

# percent weight gain
male_percent_weight_gain = 100 * (male_weight_gain / mc)
male_percent_weight_gain

female_percent_weight_gain = 100 * (female_weight_gain / fc)
female_percent_weight_gain
...
```{r}
# On average, males put on about 12.54% of body weight, and females put on about 9.94% of their body weight.
# Male mice gain a significantly higher proportion of their body weight on a high-fat diet.
```
...
hf   = mice.pheno[which(mice.pheno$Sex == "M" & mice.pheno$Diet == "hf"), "Bodyweight"]
```
...
s.ctl =   sample(chow, 12) # control = "chow" group
s.trt =   sample(hf, 12) # treatment = "hf" group
...
obs.diff.full = mean(hf) - mean(chow)
obs.diff.full
...
```{r}
# The differences vary quite drastically. With the small sample size, I would
# sometimes get results that the difference in mean weights was negative (indicating
# that the high fat group lost weight). Other times, I would get a highly positive difference
# (indicating a significant weight gain).
...
mice.pheno.chow = mice.pheno[which(mice.pheno$Diet == "chow"),]
replicate( 6 , mean( sample( mice.pheno.chow$Bodyweight  ,  12 )))
...
resample = data.frame(sample.mean =  
                        replicate( n.samples  ,                    # number of samples
                                  mean(sample( chow , sample.size ))) )  # sample and take the mean
str(resample)
head(resample)
...
ggplot( data = resample,
        mapping = aes(x = sample.mean)) + 
  geom_histogram(color = "black",
                 fill = "light green",
                 binwidth = 0.2) + # pick a color scheme, set binwidth
  # map `sample.mean` to the vertical line (copy syntax from previous histograms above)
  geom_vline(aes(xintercept=mean(chow)),
             linetype="dashed")
...
replicate( 10 , sem(chow))  # compute sem for 10 samples from "chow" population
...
sem_dist = data.frame(sem = replicate(1000, sem(chow)))
...
sample.size = 12
ctl.sample = sample(chow, sample.size)

# compute SEM and 95% CI
sem = sd(ctl.sample) / sqrt(sample.size)
ci = c(ci(ctl.sample)[2], ci(ctl.sample)[3])
...
```{r}
# About 5 / 100 of the confidence intervals do not contain the true population mean. This number seems
# to fluctuate by +- 1 confidence interval when the code is rerun.
```


How much does the width of the 95% CI's seem to vary?
```{r}
# They are usually about the same width, with some CI's being much larger or smaller than others. The
# shortest width I have seen is an interval of ~2 weight units. The longest width I've seen is an interval
# of ~7 weight units.
```

About how often do your CI's contain the mean for the high-fat diet population? What do you think this might mean?
```{r}
# I would estimate that 10 - 15 of the CI's that I've generated contain the mean of the high-fat diet population.
# This may have to do with the p-value, where 15% of the time, we are unable to reject the null
# hypothesis with our results due to random variation.
```
