...
```{r, message=FALSE}
# normalize the data
scaled_data = as.data.frame(scale(cancer_data[, 2:11]))

first = cancer_data %>%
  select(,1)

scaled_data <- cbind(first, scaled_data)
...
scaled_data_melt = melt(scaled_data, value.name = "value",
...
#separating data into M and B outcomes
m_scaled <- scaled_data_melt %>%
  filter(diagnosis == "M")
str(m_scaled)

b_scaled <- scaled_data_melt %>%
  filter(diagnosis == "B")
str(b_scaled)

# make multiple boxplots for M and B outcomes, faceted by features
ggplot(data = scaled_data_melt, aes(diagnosis, value)) +
  geom_boxplot() +
  facet_grid(~ variable)
...
```{r eval=FALSE}
# Perimeter, area, concavity, concave point, and radius look like they have the most predictive power for the diagnosis outcome. Fractal dimension looks very uninformative.
```
...
# p-value for observed difference between B and M outcomes for each feature
  ...
  m_scaled <- scaled_data %>%
  filter(diagnosis == "M")
  
  b_scaled <- scaled_data %>%
  filter(diagnosis == "B")
  
print("radius wilcox test")
wilcox.test(m_scaled$radius, b_scaled$radius)

print("texture wilcox test")
wilcox.test(m_scaled$texture, b_scaled$texture)

print("perimeter wilcox test")
wilcox.test(m_scaled$perimeter, b_scaled$perimeter)

print("area wilcox test")
wilcox.test(m_scaled$area, b_scaled$area)

print("smoothness wilcox test")
wilcox.test(m_scaled$smoothness, b_scaled$smoothness)

print("compactness wilcox test")
wilcox.test(m_scaled$compactness, b_scaled$compactness)

print("concavity wilcox test")
wilcox.test(m_scaled$concavity, b_scaled$concavity)

print("concave.points wilcox test")
wilcox.test(m_scaled$concave.points, b_scaled$concave.points)

print("symmetry wilcox test")
wilcox.test(m_scaled$symmetry, b_scaled$symmetry)

print("fractal_dimension wilcox test")
wilcox.test(m_scaled$fractal_dimension, b_scaled$fractal_dimension)

# fractal dimension measurement is not significant
...
```{r eval=FALSE}
# Radius and perimeter, radius and area, perimeter and area, and concavity and concave points.
```
...
```{r eval=FALSE}
# Texture, smoothness, and symmetry seems to be least correlated with other variables.
```
...
```{r eval=FALSE}
# There are a lot of variables here and it'd be good to narrow down the variables of interest.
```
...
```{r eval=FALSE}
# if we wanna look at how each independent variable impacts the response variable, we should use a logistic model and not a PCA because a PCA won't consider the response variable. 
```
...
B_all$diagnosis = sub("B", 0, B_all$diagnosis)
str(B_all)

M_all$diagnosis = sub("M", 1, M_all$diagnosis)
str(M_all)
```
...
```{r eval=FALSE}
# the reduced model's intercept is significant now that the area variable has been removed. Also reduced significance overall.
```
...
glm_nonredundant = glm(diagnosis ~
                radius +
                texture +
                smoothness + 
                compactness +
...
```{r eval=FALSE}
#  radius, texture, and concave points seem to be most significant. these results suggest that the contributions of the redundant features obscure the effects of some of the unique features. 
```
...
```{r eval=FALSE}
# Our model is decent at predicting cancer because a tumor can either be benign or malignant so you shouldn't have too many values in between.
```
...
```{r eval=FALSE}
# False negatives increase and false positives decrease as the threshold gets higher.
```
...
# texture
type = "Texture"
roc_result = roc_result = roc(cancer_pred_texture,  # predictions on test set
                 BM_test$diagnosis)  # observed diagnoses from test set
...
## combined predictors ======================================================= #
type = "Nonredundant"
roc_result = roc(cancer_pred_nonredundant,
                 BM_test$diagnosis)
plot(roc_result, main=paste0(type, ": AUC = ", signif(auc(roc_result),3)))
```
...
```{r eval=FALSE}
# The nonredundant model is best and the concave points model and the radius model are the second and third best performing models. These variables must be the most impactful variables. 
```
...
# model
glm_top.features = glm(diagnosis ~ concave.points + radius + texture, 
                       data = BM_train,
                       family = binomial, 
                       maxit = 100)
...
```{r eval=FALSE}
# The models look basically the same, but the non redundant one's AUC is a tiiiiny bit better (0.984 vs 0.983). So nonredundant model might be better so we're not fitting data redundantly and misrepresenting it.
...
```{r eval=FALSE}
# The nonredudandant model seems best because it has the lowest level of deviance.
```
...
```{r eval=FALSE}
# The nonredundant model seems better. I'd still choose the nonredundant model.
```
...
```{r eval=FALSE}
# nonredundant because the AIC is lower
```
...
```{r eval=FALSE}
# I would use the step-wise reduced model.
```
