...
```{r}
# load the dataset
cancer_data = read.csv("/NYU/Statistics_course/week_15/cancer_data.csv", header=T , stringsAsFactors = F)
...
cancer_data_small = cancer_data[,c(2:12)]

# check it
str(cancer_data_small)
# remove trailing "_mean" from the column names

colnames(cancer_data_small) <- sub("\\_mean$", "", colnames(cancer_data_small))
...
```{r, message=FALSE}
# normalize the data
scaled_data= scale(cancer_data_small[-1])
scaled_data = cbind(cancer_data_small[1],
                    scaled_data)
...
levels(scaled_data_melt$variable)
...
```{r eval=FALSE}
#by comparing between the means of malignant and benign across different features' boxplots we can say that the last feature which is the fractal dimension has the least difference between B and M means ( meaning B and M are almost overlapping )
```
...
```{r}
scaled_data_sig = as.data.frame(scaled_data[,c(1:10)])
str(scaled_data_sig)
```
...
correlation <- cor(scaled_data_sig[,c(2:10)] , method = "pearson")


# correlogram plot
#install.packages("GGally")  # only needed once
library(GGally)
ggpairs(scaled_data_sig, columns = 2:10, title = "Tumor Feature Correlations",
...
```{r eval=FALSE}
#Radius and perimeter, radius and area, perimeter and area look highly correlated with each other
```
...
```{r eval=FALSE}
#symmetry and texture look least correlated with each other with cor = 0.071, they also seemed uninformative in the boxplot since there was no big difference in their means between benign and malignant but they still showed significant p-value in the statistical test
```
...
```{r eval=FALSE}
#using dimensionality reduction like PCA will allow easier analysis and visualization. as we can see from the pairwise correlation graph that there are some variables highly correlated to each other so they are almost redundant 
```
...
```{r eval=FALSE}
#As PCA will lead to the removal of the correlated features, so we will not be able to interpret the PCAs as they might be composed of combination of mixed correlated variable in just one PCA.so we will not be able to interpret the effects of individual features
...
B_all = scaled_data_sig %>% filter(diagnosis == "B")
M_all = scaled_data_sig %>% filter(diagnosis == "M")
...
BM_test = rbind(M_sample , B_sample)
BM_test

# combine the training data into a single data.frame
# (hint: use negation of test data sets)
M_B_all  = rbind(M_all , B_all)
BM_train = M_B_all %>% anti_join(BM_test)
...
```{r eval=FALSE}
#after removing the area, the contribution of the different variable become less outsized ( estimates are less ), also standard error decreased
```
...
```{r eval=FALSE}
#most of the p values and coefficients decreased after the removal of the redundant variables. concave point and texture are still significantly contributing to the non redundant model like their contribution to the full model. Radius showed significant contribution to the non redundant model but not to the full model. Smoothness was contributing significantly to the full model but in the non redundant model showed no significant contribution.
```
...
cancer_pred_nonredundant = predict(glm_nonredundant, type = "response" , newdata = BM_test)
...
```{r eval=FALSE}
#the plot portray the probability of the responses between 0 and 1. As 0 is Benign and 1 is malignant. we see here that we have higher probability of getting benign tumour (0) than getting malignant tumour (1). In the benign the counts ( frequency is almost 45), in the malignant it is 30.
```
...
#As we increase the threshold, the no. of False positive also increases and the no. of false negative decreases
```
...
# smoothness
type = "Smoothness"
roc_result = roc(cancer_pred_smoothness, BM_test$diagnosis)
plot(roc_result, main=paste0(type, ": AUC = ", signif(auc(roc_result),3)))

# compactness
type = "Compactness"
roc_result = roc(cancer_pred_compactness, BM_test$diagnosis)
plot(roc_result, main=paste0(type, ": AUC = ", signif(auc(roc_result),3)))
...
```{r eval=FALSE}
#The full model that has all combined predictors ( all non redundant features) is the best since it gave the highest AUC equivalent to 0.984. In case of individual predictors, we see that radius and concave point gave the second highest AUC after the combined model. That means we can rely on radius and concave points in predicting the diagnosis, but still the combined model is the optimum since other features have little contribution in enhancing the overall AUC
```
...
```{r eval=FALSE}
#Both models yielded exactly the same AUC equal to 0.984, the roc curves are almost similar. I would rather choose the model with fewer predictors since this will be less computationally expensive and also faster, the trade off here is that we might lose some of the information in case we did not include the rest of non redundant features, but since that both models have equivalent AUC so the lost data should not be significant 
```
...
```{r eval=FALSE}
#The individual features model are significantly different from the non redundant model. The top 3 feature model is also significantly different from the non redundant model. That means the full non redundant model is the best as the other models might have incomplete information.
```
...
```{r eval=FALSE}
#My view point still the same, the full non redundant model is the best, as it has lower aic which means that less information is lost, which is better
```
...
```{r eval=FALSE}
#the best model with the lowest aic of 134.52 only had
...
lr.fold = glm(formula = diagnosis ~ radius + texture + smoothness + compactness + concave.points + symmetry ,     
...
```{r eval=FALSE}
#according to this analysis, the accuracy is almost the same between both models. the full non redundant model accuracy is 0.947 and the stepwise-reduced model accuracy is 0.942 ( slightly smaller ). I would rather use the stepwise-reduced model, much simpler and yields almost same AUC ( accuracy ) 
```
