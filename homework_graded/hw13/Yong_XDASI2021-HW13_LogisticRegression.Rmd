...
```{r}
# load the dataset
cancer_data = read.csv("/Users/lwyong/Desktop/LW Yong NYU/Biostat 2021 Fall/Week 14/XDASI2021-HW13_LogisticRegression/cancer_data.csv", stringsAsFactors = FALSE)
...
library(dplyr)
cancer_data_tidy = cancer_data %>% select(diagnosis, ends_with("_mean"))

# check it
head(cancer_data_tidy)

# remove trailing "_mean" from the column names
colnames(cancer_data_tidy) <- sub("_mean","",colnames(cancer_data_tidy))
...
```{r, message=FALSE}
# normalize the data
scaled_data = scale(cancer_data_tidy[-1])
scaled_data = cbind(cancer_data_tidy[1],scaled_data)
...
```{r eval=FALSE}
# I consider the "features" where the M & B have non overlapping IQRs to have predictive power. With this critirea, texture, smoothness, symmetry and fractal dimensions is least informative. If the IQR are overlapping, this means the feature does not "seperate" the two kinds of tumor. 
```
...
```{r eval=FALSE}
# radius-perimeter, radius-area, perimeter-area. Concavity and concave.point comes very close too. 
```
...
```{r eval=FALSE}
# texture (vs almost all other feature) and symmetry (vs radius, texture, perimeter, area). These dont look best based on the boxplots but the statistical test (the wilcox test above) is still significant. 
```
...
```{r eval=FALSE}
# Since some of the feature are correlated, collapsing them to one "explaining factor"/PCA can still give us enough information to "seperate" the datapoints during PCA analysis. 
```
...
```{r eval=FALSE}
# We loose information by performing PCA. With logistic model, we will want to test each individual feature first to see if we can remove the feature without affecting the model. 
```
...
B_all$diagnosis = as.numeric(gsub("B","0",B_all$diagnosis))
M_all$diagnosis = as.numeric(gsub("M","1",M_all$diagnosis))
```
...
glm_all = glm(diagnosis ~ radius + texture + perimeter + area + smoothness + compactness + concavity + concave.points + symmetry, data = BM_train, family = "binomial", maxit = 100)
...
```{r}
# reduced model (-area)
glm_reduced = glm(diagnosis ~ radius + texture + perimeter + smoothness + compactness + concavity + concave.points + symmetry, data = BM_train, family = "binomial", maxit = 100)
...
```{r eval=FALSE}
# The contribution of each "feature" is more even now. Unlike the previous model, there is no feature such as "area" that disproportionately contributes to the model. 
```
...
```{r eval=FALSE}
# The coefficient is more or less similar and still even spread for all features but the p values are smaller and the features that SIGNIFICANTLY contribute to the model has increased from just texture (in the previous model) to radius, texture and concave points. 
```
...
```{r eval=FALSE}
# This plot shows number of time hte predicted data is 1(malignant) or 0(benign). 
# The times it predicted 0 is more than the times it predicted 1. 
```
...
## combined predictors ======================================================= #
type = "Nonredundant"
roc_result = roc_result = roc(cancer_pred_nonredundant,  # predictions on test set
                 BM_test$diagnosis ) 
plot(roc_result, main=paste0(type, ": AUC = ", signif(auc(roc_result),3)))
```
...
```{r eval=FALSE}
# The Non redundant model performs the best as it has the greatest area under the curve. Concave_points and concave points also does very well but not as well as the non-redundant model. 
```
...
```{r eval=FALSE}
# Both are similar. I will choose the model with more information (non-redundant) as i personally think that having less "feature" may become an issue if we somehow gather data from more diverse kinds of breast cancer tumor or patient. Although this comes at a cost of having higher computational needs. 
```
...
```{r eval=FALSE}
# The full (non-redundant) model is still the best. Chisq compared the models with less features vs the full features and all these results are significantly different (p values less than 0.05). With the full(non-redundant)model, you will not loose as much information. 
```
...
```{r eval=FALSE}
# I would choose the full non-nonredunant model as the AIC is lower and thus less information is loss. I havent changed my viewpoint. 
```
...
```{r eval=FALSE}
# This model: diagnosis ~ radius + texture + smoothness + concave.points (AIC is 134.52, the least)
```
...
```{r eval=FALSE}
# Previous analysis seems to show that the stepwise reduced model is better. I will use the full non redundant model as it has a higher internal estimate of accuracy (higher AUC) for this analysis square test. 
```
