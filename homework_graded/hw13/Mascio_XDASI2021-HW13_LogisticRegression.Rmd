...
cancer_data_subset = cancer_data[,2:12]

# check it
str(cancer_data_subset)
# remove trailing "_mean" from the column names
colnames(cancer_data_subset) <- sub("_mean","",colnames(cancer_data_subset))
...
```{r, message=FALSE}
# normalize the data
scaled_data = cancer_data_subset
scaled_data[,2:11] <- scale(cancer_data_subset[,2:11])
...
scaled_data_melt = melt(scaled_data, variable.name = "variable")
  
# check it
head(scaled_data_melt)

# make multiple boxplots for M and B outcomes, faceted by features
ggplot(scaled_data_melt, aes(x = diagnosis, y = value, fill = diagnosis)) +
  geom_boxplot() +
...
```{r eval=FALSE}
# Radius, perimeter, area, texture, smoothness, compactness, concavity, concave.points - there is a visual difference in the boxplots. the other have a small or no differences between groups
```
...
for (i in 2:ncol(scaled_data)) {
  p <- wilcox.test(scaled_data[,i] ~ diagnosis, data = scaled_data)$p.value
  cat("the p-value for",colnames(scaled_data)[i],"is",p,"\n")
}
...
```{r eval=FALSE}
# radius, area, and perimeter. concavity and concave.points
```
...
```{r eval=FALSE}
# texture and smoothness, not the best but some of the best
```
...
```{r eval=FALSE}
# the features themselves don't seem to be the most descriptive outright so we narrow down important variables
```
...
```{r eval=FALSE}
# its almost like fishing for what is significant to use in the logistic regression
```
...
B_all = scaled_data[which(scaled_data$diagnosis == "B"),]
M_all = scaled_data[which(scaled_data$diagnosis == "M"),]
...
B_sample_indices <- sample(1:nrow(B_all),floor(0.2*nrow(B_all)))
B_sample = B_all[B_sample_indices,]

set.seed(20211204)  # set a seed for reproducibility
M_sample_indices <- sample(1:nrow(M_all),floor(0.2*nrow(M_all)))
M_sample = M_all[M_sample_indices,]
...
BM_train = rbind(B_all[-B_sample_indices,],M_all[-M_sample_indices,])
...
```{r eval=FALSE}
# reduced amount of significance and no warning. AIC increased
```
...
```{r eval=FALSE}
# p-values increase and estimates changed slightly. some features contribute much more than others
```
...
```{r eval=FALSE}
# the counts for each probability between 0 (BENIGN) and 1 (MALIGNANT)
```
...
```{r eval=FALSE}
# False negatives slowly increase and false positives greatly decrease
```
...
```{r eval=FALSE}
# nonredundant model is best. Probably because it takes into account multiple features instead of just one
```
...
summary(glm_all)
# model
glm_top.features = glm(diagnosis ~ texture + radius + concave.points, 
                 data = BM_train, family = "binomial", maxit = 100)
...
```{r eval=FALSE}
# they are very similar. I would choose the non-redendant because even though it is more time consuming, I am including as much information as possible
```
...
```{r eval=FALSE}
# the top 3 and non-redundant models have a smaller p value, and since nonredundant has higher auc, it is showing that this model is better
```
...
```{r eval=FALSE}
# nonredundant loses less information
```
...
```{r eval=FALSE}
# they are so close and so I would use the non-redundant model to retain the most info with the most accuracy
```
