...
cancer_data = cancer_data[, -c(13:ncol(cancer_data))]

# check it
head(cancer_data)

# remove trailing "_mean" from the column names
names(cancer_data) <- sub("_mean", "",
                          names(cancer_data))
...
scaled_data[3:ncol(scaled_data)] <- scale(scaled_data[3:ncol(scaled_data)])
...
scaled_data_melt = melt(data = scaled_data,
                        id.vars = "diagnosis",
                        measure.vars = names(scaled_data)[3:ncol(scaled_data)])
...
ggplot(scaled_data_melt,
       mapping = aes(x = diagnosis,
                     y = value)) +
  geom_boxplot() +
  facet_wrap(scaled_data_melt$variable)
...
```{r eval=FALSE}
# Measures related to tumor size, like radius, area, and perimeter, look particularly
# promising for predicting malignancy. Features like fractal dimension or symmetry
# look a little less powerful. I am making inferences based on the differences in
...
measures <- unique(scaled_data_melt$variable)
for (measure in measures) {
  benign_vals <- scaled_data_melt %>% filter(diagnosis == "B" &
                                             variable == measure)
  malignant_vals <- scaled_data_melt %>% filter(diagnosis == "M" &
                                             variable == measure)
  p_val <- wilcox.test(benign_vals$value, malignant_vals$value)$p.value
  print(paste("Measure:", measure, "|", "p-value:", p_val))
}
...
```{r}
scaled_data = scaled_data %>% select(-c("fractal_dimension"))
```
...
var_corelations <- cor(scaled_data[,3:10])
...
```{r eval=FALSE}
# Measurements of size, like perimeter, radius, and area are all highly
# correlated (as expected). concave points and concavity are highly correlated
# with each other, and pretty highly correlated with size measurements.
```
...
```{r eval=FALSE}
# Smoothness and texture are least correlated with other variables. These
# variables had low p-values from the wilcox test of difference
# between benign and malignant samples, but had higher p-values than the other
# variables, so may be less predictive of malignancy.
```
...
```{r eval=FALSE}
# A PCA could help reveal which variable may be contributing most to observed
# differences. I would imagine that the data would primarily be separated
# by benign or malignancy, and certain variables may be most correlated with
# that on the PCA plot.
```
...
```{r eval=FALSE}
# You may want to just include all of the features in your logistic model
# and perform tests between full and reduced models to determine which
# feature best predicts data.
```
...
B_sample = sample(B_all$id,
                  floor(0.2*nrow(B_all)))
B_sample = B_all %>% filter(id %in% B_sample)

set.seed( 20211204 )  # set a seed for reproducibility
M_sample = sample( M_all$id ,
                  floor(0.2*nrow(M_all)))
M_sample = M_all %>% filter(id %in% M_sample)
...
BM_train = BM_all %>% filter(!(id %in% BM_test$id))
...
glm_all = glm(diagnosis ~ radius + texture + perimeter + area + smoothness + compactness +
...
```{r}
# reduced model (-area)
glm_reduced = glm(diagnosis ~ radius + texture + perimeter + smoothness + compactness +
                concavity + concave.points + symmetry, 
                 data = BM_train, family= "binomial", maxit = 100)
...
```{r eval=FALSE}
# The estimate for intercept is now significant, and many of the estimates
# for each coefficent are different than before.
```
...
glm_nonredundant = glm(diagnosis ~ radius + texture + smoothness + compactness +
...
```{r eval=FALSE}
# The coefficients for the non-redundant model all changed slightly (typically they
# show a lower magnitude). Their p-values decreased as well. This suggests that
# redundancy of features in the full model may spread out contributions, making
# each contribution slightly less significant.
```
...
```{r eval=FALSE}
# The predictions of each point in the test data set given by the non-redundant glm.
```
...
```{r eval=FALSE}
# The number of false positives decrease, but the number of false negatives increase.
```
...
```{r eval=FALSE}
# The number of concave points seemed to perform the best because its area under
# the curve was closest to one.
```
...
roc_result_nonredundant = roc_result
...
```{r eval=FALSE}
# The models perform about the same. I would probably choose the model with
# the least amount of unnecessary information, since it will likely make
# future work a little bit easier and is more reflective of the parameters
# that are actually making predictions.
```
...
```{r eval=FALSE}
# The full nonredundant model seems best, since there is lower residual deviance than
# the top three model.
```
...
```{r eval=FALSE}
# The nonredundant model has a lower aic than the top-three model, indicating
# that it is a little better at fitting the data.
```
...
```{r eval=FALSE}
# The best model includes radius, texture, smoothness and concave points, according
# to the step-wise regression.
```
...
lr.fold = glm(formula = diagnosis ~ radius + texture + smoothness + compactness +
                concave.points + symmetry,   # fill in the formula here
...
lr.fold = glm(formula = diagnosis ~ radius + texture + smoothness +
                concave.points,   # fill in the formula here
              family="binomial", maxit = 100, 
              data=scaled_data)
...
```{r eval=FALSE}
# It looks like the full nonredundant model has a slightly higher internal
# estimate of accuracy and cross-validation estimate of accuracy. In the end,
# I would probably use the full nonredundant model, since it seems to be slightly
# more accurate, and its AIC value is not much higher than the reduced model.
```
