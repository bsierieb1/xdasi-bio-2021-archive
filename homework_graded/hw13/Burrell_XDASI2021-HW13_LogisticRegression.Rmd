...
colnames(cancer_data) = sub(pattern = "_mean",
                  x = colnames(cancer_data),
                  replacement = "")
...
```{r eval=FALSE}
# Many of the features seem like they will be able to provide some predictive power. Most obviously, radius, perimeter, area, compactness, concavity, and concave points, all of which do show no overlap between their respective IQRs comparing malignant and benign tumors. Certain features appear less informative, like fractal dimension and symmetry, which show very little seperation between M and B groups. 
```
...
library(GGally)
ggpairs(scaled_data, columns = 3:11, title = "Tumor Feature Correlations",
...
```{r eval=FALSE}
# symmetry, texture, and smoothness. These are the ones that appeared least significant in prior analyses.
```
...
```{r eval=FALSE}
# Because the variables which appeared most predictive are the most highly correlated (and not to be likely independent), dimensionality reduction will allow me to parse out which variables are in fact most predictive of tumor malignancy. 
```
...
```{r eval=FALSE}
# because PCA transforms correlated variables into uncorrelated variables, and because we will have a lot of PCs for this data set, we might not want to perdorm PCA and just move on to determining the most predictive features more directly.
```
...
B_sample = sample_n( B_all ,
                  floor(0.2*nrow(B_all)))

set.seed(20211204)  # set a seed for reproducibility
M_sample = sample_n( M_all ,
...
glm_all = glm(diagnosis ~ symmetry + concave.points + concavity + compactness + smoothness + area + perimeter + texture + radius, 
                 data = BM_train, family= "binomial", maxit = 100)
...
```{r eval=FALSE}
# the models are very similar, the only major difference is that the intercept becomes significant when we remove area (the predicted value of the diagnosis when all of the variables are 0). 
```
...
```{r eval=FALSE}
# The coefficients remained similar but the p values went way down. This suggests that these variables do not contribute as much to the full model since they are masked by the high degree of correlation with another variables which contributes more. 
```
...
cancer_pred_perimeter = predict(glm_perimeter,
                             newdata = BM_test , type="response")

cancer_pred_area = predict(glm_area,
                             newdata = BM_test , type="response")
...
```{r eval=FALSE}
# this plot shows the frequency of the predicted diagnoses from our nonredundant model, where 0 represents benign and 1 represents cancer and values inbetween represent uncertainty. For instance, a value of 0.25 means the tumor is more likely to be benign than malignant, but the prediction is not as confident.
```
...
```{r eval=FALSE}
# as the threshold increases, the number of false negatives increases, while the number of false positives decreases.
```
...
```{r eval=FALSE}
# the best model here is the nonredundant model, however, concave.points is nearly as good. This is likely because concave.points is a good predictor of tumor malignancy and it is highly correlated with the other best predictors. Therefore, this model has almost the same helpful information as the nonreduntant model.
```
...
```{r eval=FALSE}
# These models are virtually identical in terms of their ability to predict cancer malgnancy. I would choose the model with fewer predictors because it is less likely to overfit and may be better at predicting data from various samples--it is more likely to generalize well. The tradeoffs are between loosing potentially important information and predictive capabilities with making your model easier to understand and preventing overfitting. 
```
...
```{r eval=FALSE}
# the best model in terms of explaining the variance is still the nonredundant model by a slim margin, however, the top features model may still be best for the reasons discusses above.
```
...
```{r eval=FALSE}
# since these models are virtually the same in terms of AIC, I would choose the simpler model as discussed above.
```
...
```{r eval=FALSE}
# A model which uses concave.points + smoothness + texture + radius as predictors. Slightly different from teh top features model.
```
...
```{r eval=FALSE}
# This analysis suggests that the stepwise reduced and nonredundant models are very similar. I would choose the top features model.
```
