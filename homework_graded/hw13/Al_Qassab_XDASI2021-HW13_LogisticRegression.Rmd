...
```{r, message=FALSE}
# normalize the data
scaled_data = data.frame(scale(cancer_data[,-1]))
scaled_data = cbind(cancer_data[,1] ,scaled_data )
colnames(scaled_data) = c("diagnosis", colnames(scaled_data[2:11]))
...
```{r eval=FALSE}
# Measures relating to the size of the tumor seem to be promising in predicting whether or not the tumor is malignant. Specifically radius, area, perimeter.
# Measurements like fractal dimension, and symmetry seem to be less informative,
# I am taking a look at the the plots the show the most difference between the medians, as well as as ones that have the least outlier spread
```
...
for (i in 1:length(measures)) {
  benign_val = scaled_data_melt %>% filter(diagnosis == "B" &
                                             variable == measures[i])
  mal_val = scaled_data_melt %>% filter(diagnosis == "M" &
                                          variable == measures[i])
  
  p_vals[i] = wilcox.test(benign_val$value, mal_val$value)$p.value
  
  print(paste("Measure:", measures[i], "|", "p-value:", p_vals[i]))
}

measures_p_vals = data.frame(measures, p_vals)
measures_p_vals
...
correlations = cor(scaled_data[,2:10])
...
```{r eval=FALSE}
# Measurements of size are highly correlated with one another. Perimeter, radius and area
#Concavity, and concave points are also highly correlated with one another
```
...
```{r eval=FALSE}
# Texture and smoothness seem to be least correlated to other variables. These had low p_values, but higher than the others. And this is consistent with the boxplots
```
...
```{r eval=FALSE}
# Since a bunch of variables are contributing to malignancy, a PCA could be useful in determining the variable that has the most effect. 
```
...
```{r eval=FALSE}
# Because we can compare the full model to the reduced model, and determine which variable has the most effect that way
```
...
B_index = sample( 1:nrow(B_all) ,
                  floor(0.2*nrow(B_all)))
B_sample = B_all[B_index,]

set.seed( 20211204 )  # set a seed for reproducibility
M_index = sample( 1:nrow(M_all) ,
                  floor(0.2*nrow(M_all)))
M_sample = M_all[M_index,]
...
BM_train = rbind(B_all[-B_index, ], M_all[-M_index,])
...
```{r eval=FALSE}
# The estimate for the intercept is not signficant, and the estimates themselves seem to have changed between the two models
```
...
```{r eval=FALSE}
# The estimates slightly changed, and for the most part show a lower magnitude. It also seems that the p-value decreases suggesting that redundant data makes each contribution slightly less significant. 
```
...
```{r eval=FALSE}
# The prediction of each point given by the non redundant glm
```
...
```{r eval=FALSE}
# False positive decreases while false negative increases
```
...
```{r eval=FALSE}
# The best model was the non-redundant, this is because the Area under the curve was closest to 1. Out of all the individual variables though concve_points was the best for the same reason
```
...
```{r eval=FALSE}
# These models function essentially the same. I would choose the model with the least amount of information needed. That is, I would get rid of as much unnecessary data as possible. This is because it will make dealing with the data a little easier, and it should be most reflective of what's actually making the predictions
```
...
```{r eval=FALSE}
# The full non redundnant model is the best. This is because there seems to be less deviance than the top three model
```
...
```{r eval=FALSE}
# Non redundant aic is lower than the other one. This indicates that it fits the data a little bit better.
```
...
```{r eval=FALSE}
# The best model is one that include smoothness, concave points, radius, and texture accordig to this step-wise regression
```
...
```{r eval=FALSE}
# The full non redundant model seems to have a high internal estimate of accuracy but a lower cross-validation estimate of accuracy. I don't think it matters that much, I would choose the full non redundant model though, because of the higher internal estimate of accuracy
```
