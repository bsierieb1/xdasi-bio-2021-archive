...
```{r}
# load the dataset
cancer_data = read.csv("/Users/sicg_labmanager/Documents/NYU BIO/Biostats/XDASI2021-HW13_LogisticRegression/cancer_data.csv", stringsAsFactors = F)
  
head(cancer_data)

# define the factors
cancer_data$diagnosis = factor(cancer_data$diagnosis, c("M", "B"))
...
cancer_data = cancer_data[,c(1:12)]
# check it
str(cancer_data)
...
```{r, message=FALSE}
# normalize the data
scaled_data = as.data.frame(scale(cancer_data[,c(3:12)]))
...
# do we not want these things in the df?! 
#add back in id columns 
scaled_data$diagnosis = cancer_data$diagnosis

#re-order for clarity
scaled_data = scaled_data[,c(11, 1:10)]
head(scaled_data)
...
scaled_data_melt = melt(scaled_data, id.vars = "diagnosis",
                        value.name = "value",
...
#maybe don't need Id column? but leaving it in 
head(scaled_data_melt)

#makes separate dataframes ?!
#radius = scaled_data_melt %>% subset(variable == "radius")


# make multiple boxplots for M and B outcomes, faceted by features
ggplot(scaled_data_melt, aes(x = variable , y= value, col = variable)) +
  geom_boxplot() + 
  geom_jitter() + 
...
```{r eval=FALSE}
# perimeter, area, concavity and concave points could be informative given they have a much smaller range than those in the malignant masses and are also lower in value. Texture, fractal dimension & symmetry look like they may not be as informative as they all have a similar & wide range of values 
```
...
#use scaled_data set for this!!!!! not melt 

head(scaled_data)

#benign data
scaled_data_b = scaled_data %>% 
  subset(diagnosis == "B")

#malignent data 
scaled_data_m = scaled_data %>% 
  subset(diagnosis == "M")

#perform .... IS THIS THE RIGHT TEST? 

wilcox.test(scaled_data_b$symmetry , scaled_data_m$symmetry)
wilcox.test(scaled_data_b$perimeter , scaled_data_m$perimeter) 
wilcox.test(scaled_data_b$radius , scaled_data_m$radius)
wilcox.test(scaled_data_b$texture, scaled_data_m$texture)
wilcox.test(scaled_data_b$area , scaled_data_m$area)
wilcox.test(scaled_data_b$smoothness , scaled_data_m$smoothness)
wilcox.test(scaled_data_b$compactness , scaled_data_m$compactness)
wilcox.test(scaled_data_b$concavity , scaled_data_m$concavity) 
wilcox.test(scaled_data_b$concave.points, scaled_data_m$concave.points) 
wilcox.test(scaled_data_b$fractal_dimension , scaled_data_m$fractal_dimension) # only not sig 
...
scaled_data = scaled_data[, c(1:10)]
str(scaled_data)
...
#remove diagnosis 
scaled_data_edit = scaled_data[,c(2:10)]

cor(scaled_data_edit, method = "pearson")
...
```{r eval=FALSE}
# Radius and perimeter, radius and area, area and perimeter, concave point and concavity, and compactess and concavity 
```
...
```{r eval=FALSE}
#texture, smoothness and symmetry are least correlated with other variables. 
```
...
```{r eval=FALSE}
# To define the most valuable variables to move forward with. There are a lot of variables here and having so many could result in incorrect inferences.  
```
...
```{r eval=FALSE}
# BEcause our response variable is bionomial, we want to look at how each independent variable impacts the response variable. We should use a logistic model  because a PCA won't consider the response variable.
```
...
#Use data set without diagnosis? 
B_all = scaled_data %>% 
  subset(diagnosis == "B")
...
B_all$diagnosis = recode(B_all$diagnosis, "B" = 0)

M_all$diagnosis = recode(M_all$diagnosis, "M" = 1)
...
#make a vector the length of B_all
b = 1:nrow(B_all)
#sample 
B_sample = sample(b, floor(0.2*nrow(B_all))) 
B_test = B_all[B_sample, ]
...
m = 1:nrow(M_all)
#sample 
M_sample = sample(m, floor(0.2*nrow(M_all))) # this isn't working -- does this need to be a 
#subset B_all with B_sample
M_test = M_all[M_sample, ]

# combine the test data into a single data.frame
BM_test = rbind(M_test, B_test)
head(BM_test)
...
generics::intersect(BM_train,BM_test)
...
# etc. for other features ...    # not sure what you want us to do here? 
...
# summary for glm_all
summary(glm_all)
#texture, smoothness, concave points & area 
```
...
glm_nonredundant = glm(diagnosis~ radius + texture + smoothness + compactness + concave.points + symmetry, data = BM_train, family= binomial, maxit = 100)
...
```{r eval=FALSE}
# Radius is now significant and texture and concave points became much more significant. A full model, not accounting for redundancy can misrepresent the impact of certain variables if they are redundant. 
```
...
#glm_radius - is the glm generated from the train dataset 
#new_data = the test data set 

#only non redundant features (glm_nonredundant)
...
cancer_pred_smoothness = predict(glm_smoothness,
                             newdata = BM_test, type="response")
cancer_pred_campactness = predict(glm_compactness,
                             newdata = BM_test, type="response")
...
```{r eval=FALSE}
# Our model is able to predict diagnosis! 
```
...
p20 = table(neg_pos, BM_test$diagnosis) # predictions vs. observed

# 50% probability
neg_pos = as.numeric(cancer_pred_nonredundant >= 0.5)
table(neg_pos)                    # predictions

p50 =table(neg_pos, BM_test$diagnosis) # predictions vs. observed

# 80% probability
neg_pos = as.numeric(cancer_pred_nonredundant >= 0.8)
table(neg_pos)                    # predictions
p80 = table(neg_pos, BM_test$diagnosis) # predictions vs. observed
...
```{r eval=FALSE}
# False negatives decrease and false positives increase as the threshold gets higher.
```
...
#straight line with a slope of 1 = events that would happen just by chance 
#curves that run higher & left will have a higher AUC and greater predictive power
#ideally = ROC curve with larger area underneath than the AUC = 0.5 line 
...
# compactness
type = "Compactness"
roc_result = roc(cancer_pred_campactness,  # predictions on test set
                 BM_test$diagnosis)  # observed diagnoses from test set
plot(roc_result, main=paste0(type, ": AUC = ", signif(auc(roc_result),3)))
...
```{r eval=FALSE}
#Radius, Smoothness, Concave Points are the best single models with the Nonredundant model also performing well (AUC = 0.91). They all have high left curves with AUC higher than 0.5.  
```
...
# model
glm_top.features = glm(diagnosis~ radius + texture + concave.points, data = BM_train, family= binomial, maxit = 100)
...
roc_result_nonredundant = roc(cancer_pred_nonredundant,  # predictions on test set
                 BM_test$diagnosis)  # observed diagnoses from test set
...
```{r eval=FALSE}
# They are very similar! The non-redundant model has a bit better AUC. 
...
```{r eval=FALSE}
# The nonredundant model is better! With a test-statistics of 0.03 we have enough evidence to reject the first model (top features)
```
...
#lower AIC = less information is lost = better 
...
```{r eval=FALSE}
# The nonredundant model still seems better. 
```
...
```{r eval=FALSE}
# diagnosis ~ radius + texture + smoothness + concave.points - has the lowest AIC 
```
...
#Internal estimate of accuracy = 0.947
#Cross-validation estimate of accuracy = 0.931

# stepwise-reduced model
lr.fold_red = glm(diagnosis ~ radius + texture + smoothness + concave.points, data = BM_train, family= binomial, maxit = 100)
  

reduced_10fold = CVbinary(lr.fold_red,
                               rand=NULL,
                               nfolds=10,
                               print.details=TRUE)

#Internal estimate of accuracy = 0.945
#Cross-validation estimate of accuracy = 0.939
```

How does this analysis square with your other tests above? Which model would you use in the end?

```{r eval=FALSE}
# They are very close in accuracy. The non-redundant model appears to be a little bit better. 
```
