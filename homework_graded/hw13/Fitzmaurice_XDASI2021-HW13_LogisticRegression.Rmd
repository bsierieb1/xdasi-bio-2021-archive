...
```{r}
# load the dataset
cancer_data = read.csv("~/R/Biostats/Week_14/cancer_data.csv", stringsAsFactors = FALSE)
...
cancer_data = cancer_data %>% select(-contains("_worst"))
cancer_data = cancer_data %>% select(-contains("_se"))
                                     
# check it
head(cancer_data)

# remove trailing "_mean" from the column names
colnames(cancer_data) <- sub("_mean", "",x = colnames(cancer_data))
...
```{r, message=FALSE}
# normalize the data
scaled_data = data.frame(scale(cancer_data[,3:12], center = FALSE, scale = TRUE))

# check it
scaled_data[,3:12] = scaled_data[,1:10]
scaled_data[,1] = cancer_data[,1]
scaled_data[,2] = cancer_data[,2]

colnames(scaled_data) = colnames(cancer_data)
...
scaled_data_melt = melt(scaled_data, "diagnosis")
  
# check it
head(scaled_data_melt)

# make multiple boxplots for M and B outcomes, faceted by features
ggplot(scaled_data_melt[570:6259,], aes(x = variable, y = value)) + 
  geom_boxplot() + 
  facet_grid(variable ~ diagnosis)
```
...
```{r eval=FALSE}
# your answer here
#I think the features with the greatest differences between malignant and benign would provide the most
#predictive power for diagnosis outcome such as area or concavity point. 

#The features least differentiated between malignant and benign samples would be the least informative 
...
pvalues = c()
for (i in 3:12) {
pvalues[i] = wilcox.test(scaled_data[,i] ~ scaled_data$diagnosis,  alternative = c("two.sided"))$p.value
}
pvalues
```
...
correlations = cor(x=scaled_data[,3:11], method = "pearson")
correlations
...
```{r eval=FALSE}
# your answer here
Texture and smoothness, symmetry. I thought they looked the worst as they look like they're not very different between B and M samples, but they actually have the lowest p-values.
```
...
```{r eval=FALSE}
# your answer here
#A dimensional reduction method like PCA will allow us to project the dimensions 
#which capture the most variation or separate the data into potentially meaningful 
#clusters without losing a lot of the original information. 
```
...
```{r eval=FALSE}
# your answer here
#Because by reducing the dimensionality using PCA we will inherently remove individual 
#features which explain the least variation in the data
```
...
B_all$diagnosis = as.numeric(B_all$diagnosis)-2
M_all$diagnosis = as.numeric(M_all$diagnosis)
...
B_sample_for_train = B_sample
B_sample = B_all[B_sample,]

set.seed(1000)  # set a seed for reproducibility
M_sample = sample(1:nrow(M_all),
                  floor(0.2*nrow(M_all)))
M_sample_for_train = M_sample
M_sample = M_all[M_sample,]
...
Ball_index = 1:nrow(B_all)
Mall_indwx = 1:nrow(M_all)
BM_train =  rbind(B_all[-B_sample_for_train,],M_all[-M_sample_for_train,])
...
glm_symmetry = glm(diagnosis ~ symmetry, 
                 data = BM_train, family= "binomial", maxit = 100)
glm_fractal_dimension = glm(diagnosis ~ fractal_dimension, 
                 data = BM_train, family= "binomial", maxit = 100)

# model with linear combination of all predictors (do not include interaction terms)
glm_all = glm(diagnosis ~ radius + texture + perimeter + area + smoothness + compactness + concavity +concave.points + + symmetry + fractal_dimension, 
              data = BM_train, family= "binomial", maxit = 100)
...
```{r}
# reduced model (-area)
glm_reduced = glm(diagnosis ~ radius + texture + perimeter + smoothness + compactness + concavity +concave.points + symmetry + fractal_dimension, 
              data = BM_train, family= "binomial", maxit = 100)
...
```{r eval=FALSE}
# your answer here
#More of the features now have significant values, and the p-values of the individual 
#features become more significant. 
```
...
glm_nonredundant = glm(diagnosis ~ radius + texture + smoothness + compactness + concave.points + symmetry + fractal_dimension, 
              data = BM_train, family= "binomial", maxit = 100)
...
```{r eval=FALSE}
# your answer here
#Coefficients increase compared to full model and p-values decreases, this suggest
#that redundant features are not just unnessary for the model but actual make the
#model worse. 
```
...
cancer_fractal_dimension = predict(glm_fractal_dimension,
                             newdata = BM_test , type="response")
...
```{r eval=FALSE}
# your answer here
#The distribution of probabilities of being diagnosed with cancer from 0 to 1 based on 
#our nonredundant model.
```
...
```{r eval=FALSE}
# your answer here
#The false positives increase, and false negatives decrease as the threshold in increased.
```
...
```{r eval=FALSE}
# your answer here
#The nonredundant model is the best because it has the highest AUC. I think this makes 
#intuitive sense becuase it is incorporating all the most meaning data by having 
#more nonredundant dimesions.
```
...
## top 3 predictors ======================================================== ##
# radius, compactness, concave points
...
# ROC
roc_result_top.features = roc(cancer_pred_top.features,  # predictions on test set
                 BM_test$diagnosis )  # observed diagnoses from test set
...
```{r eval=FALSE}
# your answer here.
#The all features model performs better since it has a larger AUC. 
#I don't know the tradeoffs.  
```
...
```{r eval=FALSE}
# your answer here
#glm_top.features becuase most predictive with least amount of features.
```
...
```{r eval=FALSE}
# your answer here
#Based on this critierion I would choose the nonredundant model as it has the lowest
#AIC therefore the least information is lost. 
```
...
```{r eval=FALSE}
# your answer here
a model which incorporates radius + texture + compactness + concave.points 
```
...
lr.fold = glm(formula = diagnosis ~ radius + texture + compactness + concave.points + symmetry, # fill in the formula here
...
```{r eval=FALSE}
# your answer here
I would use this model as it predicts the test data almost perfectly with an internal estimate of accuracy of 0.938 and a cross-validation estimate of accuracy = 0.937.
```
