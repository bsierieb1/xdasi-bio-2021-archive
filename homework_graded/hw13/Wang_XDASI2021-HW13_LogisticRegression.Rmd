...
cancer_data = cbind(cancer_data["diagnosis"],cancer_data[,grepl("mean",colnames(cancer_data), fixed = TRUE)])

# check it
head(cancer_data)
# remove trailing "_mean" from the column names
colnames(cancer_data) =colnames(cancer_data) %>% sub(pattern = "_mean",replacement ="")
...
```{r, message=FALSE}
# normalize the data
scaled_data = data.frame(cancer_data[,2:11] %>% scale())
scaled_data$diagnosis=cancer_data[,1]
# check it
scaled_data
```
...
```{r eval=FALSE}
# concave.points because there is large difference the mean of M and B and relatively small std.
```
...
result=c()
for (i in 1:10){
  result=c(result,paste(wilcox.test(scaled_data[scaled_data$diagnosis=="M",i],scaled_data[scaled_data$diagnosis=="B",i])))
}
result
```
...
```{r}
scaled_data = scaled_data[,c(1:9,11)]
```
...
library(GGally)
ggpairs(scaled_data, columns = 1:9, title = "Tumor Feature Correlations",
...
```{r eval=FALSE}
# Variables seem to be reduntant
```
...
```{r eval=FALSE}
# PCA combines each feature for dimension reduction
```
...
B_sample = B_all[sample( 1:nrow(B_all) ,
                  size=floor(0.2*nrow(B_all))),]

#set.seed( ... )  # set a seed for reproducibility
M_sample = M_all[sample( 1:nrow(M_all) ,
...
BM_train = setdiff(rbind(B_all,M_all),BM_test)
# make sure the diagnosis variable in each df is a FACTOR
BM_test$diagnosis = factor(BM_test$diagnosis,levels=c(1,0))
BM_train$diagnosis = factor(BM_train$diagnosis,levels=c(1,0))
...
```{r}
# reduced model (-area)
glm_reduced = glm(formula= diagnosis ~ . -area, 
                 data = BM_train, family= "binomial", maxit = 100)
...
```{r eval=FALSE}
# p-values,weight for the variables got different
```
...
glm_nonredundant = glm(formula= diagnosis ~ . -area - perimeter -concavity, 
                 data = BM_train, family= "binomial", maxit = 100)
...
# p-values for some remaining variables get smaller. Some features are dependent of each other.
```
...
```{r eval=FALSE}
# prediction seems consistent with binary test data
```
...
```{r eval=FALSE}
# non-redundant model because it has the smallest AUC
```
...
# model
glm_top.features =glm(diagnosis ~radius+concave.points+symmetry, 
                 data = BM_train, family= "binomial", maxit = 100)
...
```{r eval=FALSE}
# the model with fewer predictors are more expainable, but less accurate
```
...
```{r eval=FALSE}
# non redundant on regarding smaller residual
```
...
```{r eval=FALSE}
# we should choose non-redundant one 
```
...
lr.fold = glm(formula = diagnosis ~ . -area - perimeter -concavity ,      # fill in the formula here
...
lr.fold = glm(formula = diagnosis ~ radius + texture + smoothness + concave.points ,      # fill in the formula here
              family="binomial", maxit = 100, 
              data=scaled_data) 
