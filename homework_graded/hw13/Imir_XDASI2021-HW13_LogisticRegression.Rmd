...
```{r}
# load the dataset
cancer_data = read.csv("./cancer_data.csv", stringsAsFactors = F)
...
cancer_data = cancer_data[,1:12]

# check it
head(cancer_data)

# remove trailing "_mean" from the column names
colnames(cancer_data) = sub(pattern = "_mean", x = colnames(cancer_data), replacement = "")
...
```{r, message=FALSE}
# normalize the data
scaled_data = cbind(cancer_data[,1:2], scale(cancer_data[,3:12]))
...
melt.data = melt(scaled_data)
  
# check it
head(melt.data)

# make multiple boxplots for M and B outcomes, faceted by features
ggplot(data = melt.data, aes(x = diagnosis, y = value)) +
  geom_boxplot() +
...
```{r eval=FALSE}
# It seems like radius, perimeter, area, compactness, concavity and concave points are all measures that have predictive power for the diagnosis outcome, but most significant difference appears on the area and concave points measures as the data points seem to have the least amount of range intersection and means seem to be most separated in these measures. 
```
...
kruskal.test(formula = scaled_data$radius ~ scaled_data$diagnosis)
kruskal.test(formula = scaled_data$texture ~ scaled_data$diagnosis)
kruskal.test(formula = scaled_data$perimeter ~ scaled_data$diagnosis)
kruskal.test(formula = scaled_data$area ~ scaled_data$diagnosis)
kruskal.test(formula = scaled_data$smoothness ~ scaled_data$diagnosis)
kruskal.test(formula = scaled_data$compactness ~ scaled_data$diagnosis)
kruskal.test(formula = scaled_data$concavity ~ scaled_data$diagnosis)
kruskal.test(formula = scaled_data$concave.points ~ scaled_data$diagnosis)
kruskal.test(formula = scaled_data$symmetry ~ scaled_data$diagnosis)
kruskal.test(formula = scaled_data$fractal_dimension ~ scaled_data$diagnosis)
kruskal.test(formula = scaled_data$id ~ scaled_data$diagnosis)
```

Remove any columns from the scaled data frame that do not show a significant difference between the "B" and "M" samples.

```{r}
scaled_data = scaled_data[,-(12)]
```
...
```{r eval=FALSE}
# Smoothness, texture. Boxplots were not an effective measure in helping us predict which measures are most powerful in predicting.
```
...
```{r eval=FALSE}
# The variables that appear to be the most predictive also appear to be the most predictive. As mentioned, boxplots were not as effective in determining which ones are powerful and which ones are not powerful, but clearly what appeared to be predictive are also correlated and useless, so we should perform PCA to double check. 
```
...
```{r eval=FALSE}
# PCA utilizes two components that are dissimilar which may capture most of the variation in the data, but if we think just by looking at the boxplots that most of the data is highly correlated, we might prefer not to utilize PCA. Looking at individual features or components as predictors does not require that we look at the intersection of two components so PCA is redundant. 
```
...
B_all = filter(scaled_data, diagnosis == "B")
M_all = filter(scaled_data, diagnosis == "M")
...
set.seed(101010)  # set a seed for reproducibility
B_sample = sample_n(B_all, floor(0.2*nrow(B_all)))

set.seed(101010)  # set a seed for reproducibility
M_sample = sample_n(M_all, floor(0.2*nrow(M_all)))
...
BM_train = rbind(B_all, M_all)
BM_train = subset(BM_train, !(id %in% BM_test$id))
...
```{r}
# reduced model (-area)
glm_reduced = glm(diagnosis ~ symmetry + concave.points + concavity + compactness + smoothness + perimeter + texture + radius, 
                 data = BM_train, family= "binomial", maxit = 100)
...
```{r eval=FALSE}
# When area is removed, we observe a significant difference in intercept.
```
...
glm_nonredundant = glm(diagnosis ~ symmetry + concave.points + compactness + smoothness + texture + radius, 
                       data = BM_train, family= "binomial", maxit = 100)
...
```{r eval=FALSE}
# The p-values overall became smaller, meaning the difference within non-redundant variables became more significant. However, different coefficients didn't become significant while not being so early on. This suggests that different features do not contribute too much to the full model as different features didn't create significant differences in the non-redundant variables. 
```
...
cancer_pred_radius = predict(glm_radius, newdata = BM_test , type="response")

cancer_pred_texture = predict(glm_texture, newdata = BM_test , type="response")

cancer_pred_perimeter = predict(glm_perimeter, newdata = BM_test , type="response")

cancer_pred_area = predict(glm_area, newdata = BM_test , type="response")

cancer_pred_smoothness = predict(glm_smoothness, newdata = BM_test , type="response")

cancer_pred_compactness = predict(glm_compactness, newdata = BM_test , type="response")

cancer_pred_concavity = predict(glm_concavity, newdata = BM_test , type="response")

cancer_pred_concave.points = predict(glm_concave.points, newdata = BM_test , type="response")

cancer_pred_symmetry = predict(glm_symmetry, newdata = BM_test , type="response")
...
```{r eval=FALSE}
# On the two extreme ends of the probability distribution for the diagnoses with the non-redundant model, we see two peaks, one where there is 100% and 0% probability of the diagnosis occurrances, 0 being benign and 1 being cancer. The non-zero and non-one values are the probabilities in between resembling uncertainty. 
```
...
```{r eval=FALSE}
# Number of false negatives increase with threshold; number of false positives on the other hand decrease with increasing frequency. 
```
...
```{r eval=FALSE}
# The model that performs the best in predicting diagnosis is the non-redundant model and concave points is the second best performing model. Concave points was also well correlated with the rest of the predictors, so it is understandably a good model for predicting diagnosis.
```
...
summary(glm_all)
# model
glm_top.features = glm(diagnosis ~ concave.points + area + texture, 
                 data = BM_train, family= "binomial", maxit = 100)
...
```{r eval=FALSE}
# The two models yield the same response in prediction. I would likely choose the model with the top features due to the fact that when all features are utilized, it is highly likely to overfit the model and using top top features also provides more generalizability. 
```
...
anova(glm_area, glm_nonredundant)
anova(glm_concave.points, glm_nonredundant)
anova(glm_compactness, glm_nonredundant)
...
```{r eval=FALSE}
# Nonredundant, because of generalizability and less chance of overfitting
```
...
```{r eval=FALSE}
# Top features due to similar reasons
```
...
```{r eval=FALSE}
# Radius + texture + smoothness + concave points predictors is the best model
```
...
lr.fold = glm(formula = diagnosis ~ symmetry + concave.points + compactness + smoothness + texture + radius,      # fill in the formula here
...
lr.fold = glm(formula = diagnosis ~ concave.points + smoothness + texture + radius,
              family="binomial", maxit = 100, 
              data=scaled_data) 
...
```{r eval=FALSE}
# The two models are similar and I would still choose the top features model.
```
...
Convert this file to an R script by executing the following command in the Console: `knitr::purl("XDASI2021-HW13_LogisticRegression.Rmd")` [substitute with the correct filename]. Hand in both this .Rmd file and the .R file you just created.
