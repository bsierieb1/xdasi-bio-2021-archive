...
scaled_data = cbind(cancer_data[1],
                    scaled_data)
# check it
str(scaled_data)
head(scaled_data)
...
scaled_data_melt = melt(data = scaled_data,
                        id.vars = "diagnosis",
                        value.name = "value",
...
```{r eval=FALSE}
# The features whose mean are very different in the malignant vs. benign samples and/or the features which have the least overlap in the data range should provide the most predictive power for the diagnosis outcome. Some of these features are radius, perimeter, area, compactness, concavity, and concave.points. Potentially uninformative features include those that have less difference in mean and data range in malignant vs. benign samples which are texture, smoothness, symmetry, and fractal dimension.
```
...
for (i in 2:ncol(scaled_data)) {
  variable = colnames(scaled_data)[i]
  b_data = scaled_data[scaled_data$diagnosis == "B", i]
  m_data = scaled_data[scaled_data$diagnosis == "M", i]
  print(paste(
    "The p-value for ", variable, " is: ", wilcox.test(b_data, m_data)$p.val))
}
...
```{r eval=FALSE}
# Radius and perimeter, radius and area, and perimeter and area are highly correlated with each other.
```
...
```{r eval=FALSE}
# Texture and symmetry look least correlated with other variables. These are the ones that seemed potentially uninformative in the boxplots since they did not have huge differences in the mean or data range in the benign vs. malignant samples (but still had significant p-values in the statistical tests).
```
...
```{r eval=FALSE}
# Since some of the variables are highly correlated such that they are almost redundant, reducing the dimensionality using PCA would make analysis and visualization simpler and faster, without losing too much information.
```
...
```{r eval=FALSE}
# Since PCA removes correlated features, we cannot interpret the effects of individual features. Rather, we will only be able to interpret the effects of principal components which may be a combination of the correlated variables.
```
...
B_all = scaled_data[scaled_data$diagnosis == "B",]
M_all = scaled_data[scaled_data$diagnosis == "M",]
...
```{r eval=FALSE}
# After removing area from the model, the contributions (especially of radius) have become less outsized and the standard errors have also decreased.
```
...
```{r eval=FALSE}
# Most of the coefficients and p-values have decreased. With the redundant variables removed, texture and concave.points still contribute significantly to the model like they did for the full model. Radius also contributes significantly to the reduced model but not to the full model. So, removing the redundant features has mostly not affected the features that significantly contribute to the full model.
```
...
```{r eval=FALSE}
# This plot shows the probability of getting a '1' response (malignant). We can see that in our test data, the probability of the cancer being benign (0) is greater than that of it being malignant (1), i.e. the histogram shows that the probability of the response being '1' is 0 for a greater number of counts than it is for the probability being 1.
```
...
```{r eval=FALSE}
# As the threshold is increased, the number of false positives also increases and the number of false negatives decreases.
```
...
```{r eval=FALSE}
# The nonredundant full model is the best as it has the greatest area under the curve (AUC). As for the individual models, radius and concave_points are also good. So, radius and concave points might predict the diagnosis better than other features, but some other feature(s) also somewhat contribute making the full model a bit more predictive that the individual models with radius or concave points.
```
...
```{r eval=FALSE}
# The models look very similar and have the same AUC. Since the models look very similar, I would choose the one with fewer predictors since this will make the analysis faster and less complicated. However, the tradeoff is that there may be some loss of information when not including the other features, but in this case since the AUC is the same, the loss of data shouldn't be significant.
```
...
```{r eval=FALSE}
# The individual models are significantly different from the full model as expected but the top 3 feature model is also significantly different from the full nonredundant model. So, the full non redundant model may be the best as the top 3 feature model may cause the loss of some information.
```
...
```{r eval=FALSE}
# I would choose the full model as it has a smaller aic which means that there is less loss of information as compared to the top 3 feature model.
```
...
```{r eval=FALSE}
# The best model is the model with only radius, texture, smoothness, and concave points.
```
...
```{r eval=FALSE}
# According to this analysis too, the auc is smaller for the best stepwise-reduced model as compared to the full nonredundant model. So, I would use the best stepwise-reduced model which is the model with only radius, texture, smoothness, and concave points.
```
