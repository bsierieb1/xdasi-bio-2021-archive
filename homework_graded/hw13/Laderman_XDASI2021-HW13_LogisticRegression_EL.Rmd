...
```{r}
# load the dataset
cancer_data = read.csv('/Users/eladerman/NYU/Classes/Biostatistics/Week14/XDASI2021-HW13_LogisticRegression/cancer_data.csv', stringsAsFactors = FALSE)
...
cancer_data_columns = colnames(cancer_data)
se_columns = cancer_data_columns[which(grepl('se', cancer_data_columns))]
worst_columns = cancer_data_columns[which(grepl('worst', cancer_data_columns))]
best_columns = cancer_data_columns[-which(cancer_data_columns %in% c(se_columns, worst_columns))]
best_columns
cancer_data = cancer_data %>% select(best_columns)
subbed_colnames = sub('_mean', '', colnames(cancer_data))
subbed_colnames
colnames(cancer_data) = subbed_colnames
# check it
...
```{r, message=FALSE}
# normalize the data
columns_to_scale = colnames(cancer_data)[3:length(colnames(cancer_data))]
cancer_data[,columns_to_scale]
scaled_data = data.frame(scale(cancer_data[,columns_to_scale]))
scaled_data = cbind(cancer_data[,c(1:2)], scaled_data)
# check it
scaled_data
```
...
scaled_data_melt = melt(scaled_data, value.name ='value', id = c('id', 'diagnosis'))
  
# check it
scaled_data_melt

# make multiple boxplots for M and B outcomes, faceted by features
ggplot(data = scaled_data_melt, mapping = aes(diagnosis, value)) + geom_boxplot() + facet_grid(~variable)
...
```{r eval=FALSE}
# fractal_dimension is likely to be the least informative and area, concavity, and radius seem to be the best predictors as the show the largest differences between the two diagnoses
```
...
pvalue_table = scaled_data_melt %>% group_by(variable) %>% summarise(pvalue = wilcox.test(value ~ diagnosis)$p.value)
pvalue_table
```
...
scaled_data = scaled_data %>% select(colnames(scaled_data)[1:11])
```
...
```{r eval=FALSE}
# Concavity and concavity points | perimeter area and radius are all correlated with eachother
```
...
```{r eval=FALSE}
# texture looks least correlated with the other variables 
```
...
```{r eval=FALSE}
# This might allow you to group the tumors based on a combination of all of the variables
```
...
```{r eval=FALSE}
# This will mush all of the variables into combined variables which we aren't interested in as we can't measure these
```
...
B_all = scaled_data %>% filter(diagnosis == 'B')
M_all = scaled_data %>% filter(diagnosis == 'M')

# recode the diagnoses as binary outcomes
B_all$diagnosis = as.integer(B_all$diagnosis == 'M')
M_all$diagnosis = as.integer(M_all$diagnosis == 'M')
```
...
B_sample = sample(1:nrow(B_all), floor(0.2*nrow(B_all)))
...
glm_radius = glm(diagnosis ~ radius, data = BM_train, family= binomial, maxit = 100)

# etc. for other features ...
glm_texture = glm(diagnosis ~ texture, data = BM_train, family= binomial, maxit = 100)
glm_perimeter = glm(diagnosis ~ perimeter, data = BM_train, family= binomial, maxit = 100)
glm_area = glm(diagnosis ~ area, data = BM_train, family= binomial, maxit = 100)
glm_smoothness = glm(diagnosis ~ smoothness, data = BM_train, family= binomial, maxit = 100)
glm_compactness = glm(diagnosis ~ compactness, data = BM_train, family= binomial, maxit = 100)
glm_concavity = glm(diagnosis ~ concavity, data = BM_train, family= binomial, maxit = 100)
glm_concave.points = glm(diagnosis ~ concave.points, data = BM_train, family= binomial, maxit = 100)
glm_symmetry = glm(diagnosis ~ symmetry, data = BM_train, family= binomial, maxit = 100)



# model with linear combination of all predictors (do not include interaction terms)
glm_all = glm(diagnosis~radius + perimeter + area + smoothness + compactness + concavity + concave.points + symmetry + texture, data = BM_train, family= binomial, maxit = 100)
...
```{r}
# reduced model (-area)
glm_reduced = glm(diagnosis~radius + perimeter +smoothness + compactness + concavity + concave.points + symmetry + texture, data = BM_train, family= binomial, maxit = 100)
...
```{r eval=FALSE}
# The intercept is now significant and the AIC increased. 
```
...
glm_nonredundant = glm(diagnosis~radius +smoothness + compactness + concave.points + symmetry + texture, data = BM_train, family= binomial, maxit = 100)
...
# It lowered them this suggests that they do contribute their impact is just drowned out by other variables
...
```{r eval=FALSE}
# Generally it classifies samples as benign or malignant with some examples of samples it has a harder time predicting and puts in the middle
```
...
```{r eval=FALSE}
# As the threshold is increased we have fewer false positives and more false negatives
```
...
```{r eval=FALSE}
# The nonredundant model because it considers the most parameters
```
...
```{r eval=FALSE}
# I would choose the non redundant model because it has a slightly higher AUC
```
...
```{r eval=FALSE}
# The non redundant model seems better because it performs much better than either of the other models
```
...
```{r eval=FALSE}
# I would chooose the non redundant model as it has a lower aic
```
...
```{r eval=FALSE}
# Just radius and concave points
```
...
lr.fold = glm(formula = diagnosis ~ radius +smoothness + compactness + concave.points + symmetry +texture,      # fill in the formula here
...
lr.fold = glm(formula = diagnosis ~ radius + smoothness + concave.points + texture,      # fill in the formula here
              family="binomial", maxit = 100, 
              data=scaled_data) 
...
```{r eval=FALSE}
# It shows that they are similar but the nonredundant model performs slighly better
```
