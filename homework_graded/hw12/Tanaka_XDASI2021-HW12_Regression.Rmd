...
fluid.all = read.table("Fluidigm_1691207281.csv", sep=",", header = T, stringsAsFactors = F)
fluid.all
head(fluid.all)
str(fluid.all)
summary(fluid.all)
...
subset(fluid.all, SampleName == "RX QC" | SampleName == "NT QC")
neg.ctls = filter(fluid.all, SampleName == "RX QC" | SampleName == "NT QC")
neg.ctls = select(neg.ctls, ID, SampleName, rConc, AssayName, Value, Call, Plate)
neg.ctls


# ============================================================================ #
# standards ("CoV 50", "CoV 500", "CoV 5000")
subset(fluid.all,  SampleName == "CoV 50" | SampleName == "CoV 500" | SampleName == "CoV 5000")
std.ctls = filter(fluid.all,  SampleName == "CoV 50" | SampleName == "CoV 500" | SampleName == "CoV 5000")
std.ctls = select(std.ctls, ID, SampleName, rConc, AssayName, Value, Call, Plate)
std.ctls
...
ggplot(neg.ctls, aes(x = SampleName, y = Value, color = SampleName)) +
  geom_jitter() +
  geom_boxplot() +
  geom_violin() +
...
```{r eval=FALSE}
# Your answer here
"Since the value of negative controls are not 999 for Plate 2 RX QC, that is not okay. 
The possible reason why the value of negative controls are not 999 for Plate 2 RX QC is a contamination of samples. The reason why I thought there was a possibility of contamination in the experiment was because there is more contamination in the 96 well plate experiment than in other well experiments, so I thought contamination might have occurred. However, since the value of negative controls except for Plate 2 RX QC is 999, that is okay for these samples."
```
...
```{r eval=FALSE}
# your answer here
"It is okay to proceed with the analysis by using the Plate 1. Since the value of negative controls are not 999 for Plate 2 RX QC, it is not good to proceed with Plate 2. "
```
...
ggplot(std.ctls, aes(x = SampleName, y = Value, color = SampleName)) +
  geom_jitter() +
  geom_boxplot() +
  geom_violin() +
  facet_grid(Plate ~ AssayName) +
  ggtitle("Standard Control") +
  theme_classic()
...
std.ctls.n1.n2 = filter(std.ctls,  AssayName == "N1" | AssayName == "N2")
std.ctls.n1.n2

# ggplot for just N1 and N2
ggplot(std.ctls.n1.n2, aes(x = SampleName, y = Value, color = SampleName)) +
  geom_jitter() +
  geom_boxplot() +
  geom_violin() +
  facet_grid(Plate ~ AssayName) +
  ggtitle("Standard Control N1 and N2") +
  theme_classic()
...
```{r eval=FALSE}
# Your answer here
"Since the value of standard control of Plate 2 CoV50 N2 is slightly above 20, that is not okay. Also, since the value of standard control of Plate 2 CoV50 N1 is slightly below 20, that is okay but not desirable. Furthermore, since the value of standard control except Plate 2 CoV50 N2 is below 20, that is okay for these samples. "
```
...
```{r eval=FALSE}
# your answer here
"It is okay to proceed with the analysis by using the Plate 1. Since the value of standard control of Plate 2 CoV50 N2 is slightly above 20, it is not good to proceed with Plate 2. Furthermore, since the value of standard control of Plate 2 CoV50 N1 is slightly below 20, that is okay but not desirable. Therefore, by considering the above reasons, it is not good to proceed with Plate 2."
```
...
samples = filter(fluid.all, SampleName != "RX QC" & SampleName != "NT QC" & 
                   SampleName != "CoV 50" & SampleName != "CoV 500" & SampleName != "CoV 5000")
samples

# Import PHD results from .csv file
phd.calls = read.table("PHD_results.csv", sep = ",", header = T, strip.white = F)
phd.calls

# Add a column to the Fluidigm file containing the PHD results
# use the inner_join() function to do this
samples = inner_join( samples , phd.calls , 
                     by.x = c("SampleName"), 
...
samples = samples %>% rename("PHD_Class" = "Class") %>% 
          mutate(NYUAD_Class = NA)
samples
head(samples)
```
...
std.ctls.n1.n2
str(std.ctls.n1.n2)
summary(std.ctls.n1.n2)
# side-by-side graphs for N1 and N2 on Plate 1 and Plate 2
# using subsetted N1 and N2 assays from standard control set (already done above)
# ggplot for just N1 and N2
#ggplot(std.ctls.n1.n2, aes(x = SampleName, y = Value, color = AssayName)) +
#ggplot(std.ctls.n1.n2, aes(x = AssayName, y = Value, color = AssayName)) +
ggplot(std.ctls.n1.n2, aes(x = rConc, y = Value, color = AssayName)) +
  #geom_point() +
  geom_jitter() +
  scale_x_continuous(label = function(x){return(round(10^x,1))}) +
  ...
  facet_grid(~Plate) +
  ggtitle("Standard Control N1 and N2 with a simple linear regression") +
  theme_classic()
```
...
```{r eval=FALSE}
# Your answer here
"The slope of the standard regression lines are downward trends for both Plates. Although there are differences in terms of the level of slopes between both Plates, they share the same tendencies of downward trends. Also, the plots of N1 and N2 are both seems to be consistent based on the positions of these plots."
```
...
std.ctls.n1.n2.p1 = filter(std.ctls.n1.n2, Plate == "Plate1")
std.ctls.n1.n2.p1

std.ctls.n1.n2.p2 = filter(std.ctls.n1.n2, Plate == "Plate2")
std.ctls.n1.n2.p2

p1_model <- lm(formula = log(std.ctls.n1.n2.p1$Value) ~ std.ctls.n1.n2.p1$rConc)

#p1_model <- lm(formula = log(Value) ~ rConc, data = std.ctls.n1.n2.p1)
summary(p1_model)
coeff <- p1_model$coefficients
coeff
slope <- coeff[2]
slope
#plot(p1_model)

p2_model <- lm(formula = log(std.ctls.n1.n2.p2$Value) ~ std.ctls.n1.n2.p2$rConc)
summary(p2_model)
coeff <- p2_model$coefficients
coeff
slope <- coeff[2]
slope
#plot(p2_model)
...
```{r eval=FALSE}
# Your answer here
  std.ctls.n1.n2.p1$rConc 
           -6.83224e-05   
           
  std.ctls.n1.n2.p2$rConc 
          -0.0001325007  
  
"The slope of the Plate 1 is -6.83224e-05 and the slope of the Plate 2 is -0.0001325007. From the data so far, it can be concluded that the data from Plate 2 is not good, so it is better to use the slope from Plate 1."
```
...
std.ctls.n1.p1 = filter(std.ctls, AssayName == "N1" & Plate == "Plate1")
std.ctls.n1.p1
std.ctls.n2.p1 = filter(std.ctls, AssayName == "N2" & Plate == "Plate1")
std.ctls.n2.p1
std.ctls.n1.p2 = filter(std.ctls, AssayName == "N1" & Plate == "Plate2")
std.ctls.n1.p2
std.ctls.n2.p2 = filter(std.ctls, AssayName == "N2" & Plate == "Plate2")
std.ctls.n2.p2

#lm.n1.p1 = lm(formula = std.ctls.n1.p1$Value ~ std.ctls.n1.p1$rConc)
#lm.n2.p1 = lm(formula = std.ctls.n2.p1$Value ~ std.ctls.n2.p1$rConc)
#lm.n1.p2 = lm(formula = std.ctls.n1.p2$Value ~ std.ctls.n1.p2$rConc)
#lm.n2.p2 = lm(formula = std.ctls.n2.p2$Value ~ std.ctls.n2.p2$rConc)

lm.n1.p1 = lm(formula = log(std.ctls.n1.p1$Value) ~ std.ctls.n1.p1$rConc)
lm.n2.p1 = lm(formula = log(std.ctls.n2.p1$Value) ~ std.ctls.n2.p1$rConc)
lm.n1.p2 = lm(formula = log(std.ctls.n1.p2$Value) ~ std.ctls.n1.p2$rConc)
lm.n2.p2 = lm(formula = log(std.ctls.n2.p2$Value) ~ std.ctls.n2.p2$rConc)


# look at the summaries of the models
summary(lm.n1.p1)
coeff <- lm.n1.p1$coefficients
slope <- coeff[2]
slope
#plot(lm.n1.p1)
summary(lm.n2.p1)
...
```{r eval=FALSE}
# Your answer here
"I will choose N1-P1 because they have the largest R-squared value of 0.8206883 which is the largest R-squared value compared with other ones."
```
...
data = filter(samples, AssayName == "RP")
data

ggplot(data, aes(x = PHD_Class, y = Value, color = PHD_Class)) +
  geom_jitter() +
  #geom_boxplot() +
  geom_violin() +
  facet_grid(~Plate) +
  ggtitle("Sample data for RP") +
  theme_classic()
...
```{r eval=FALSE}
# Your answer here
"We should expect to see that the values for the RP assays in the clinical samples should close to 0. However, the values of Plate 2 is not close to 0, so that is not okay."
```
...
```{r eval=FALSE}
samples
#good_quality <- filter(samples, NYUAD_Class != "Inconclusive")
good_quality <- filter(samples, is.na(NYUAD_Class))
good_quality
nrow(good_quality)

# Your answer here
"There are 4104 samples left."
```
...
#samples.rp = good_quality
samples.rp = filter(samples, is.na(NYUAD_Class))

# violin plot of Ct values for valid assays, filtered for RP gene, by PHD diagnosis
ggplot(samples.rp, aes(x = PHD_Class, y = Value, color = PHD_Class)) +
  geom_jitter() +
  #geom_boxplot() +
  geom_violin() +
  facet_grid(~Plate) +
  ggtitle("Good sample data for RP") +
  theme_classic()
```
...
head(rp.p2)
# Plate 1
ggqqplot(rp.p1, x = "Value")
# Plate 2
ggqqplot(rp.p2, x = "Value")

## Test for normality
st.p1 = shapiro.test(rp.p1$Value)
st.p1
st.p2 = shapiro.test(rp.p2$Value)
st.p2
...
```{r eval=FALSE}
# Your answer here
"The p-value of the normality test for Plate1 and Plate2 is e-57, so both plates can be used."
```
...
```{r eval=FALSE}
# Your answer here
nyuad.pos.sample.ids
nyuad.pos.samples
samples
nrow(nyuad.pos.samples)
"The number of positive samples that were cleaned up is 1755."
```
...
ggplot(nyuad.pos.samples, aes(x = AssayName, y = Value, color = AssayName)) +
  geom_jitter() +
  #geom_boxplot() +
  geom_violin() +
  facet_wrap(~Plate) + # wrap by Plate
  ggtitle("the N1 and N2 genes faceted by plate") +
  theme_classic()
```
...
# by PHD diagnosis
# by PHD diagnosis
ggplot(nyuad.pos.samples, aes(x = PHD_Class, y = Value, color = PHD_Class)) +
  geom_jitter() +
  #geom_boxplot() +
  geom_violin() +
  facet_grid(Plate ~ AssayName) +
  ggtitle("diagnostic assays performed at PHD") +
  theme_classic()
...
```{r eval=FALSE}
# your answer here
"I found the distribution of N1 Negative samples to be interesting because they indicates that they have more depth in their data."
```
...
```{r}
head(sample.clean.var)
head(nyuad_pos.clean.var)

## Add SEM(Ct) column to each dataset
sample.clean.var = mutate(sample.clean.var, Ct.sem = Ct.sd / sqrt(n))
head(sample.clean.var)

nyuad_pos.clean.var = mutate(nyuad_pos.clean.var, Ct.sem = Ct.sd / sqrt(n))
head(nyuad_pos.clean.var)
```
...
p1 = ggplot(sample.clean.var, aes(x = seq(SampleName), y = Ct.sem)) +
  geom_point() +
  geom_hline(yintercept = 0.5) +
  ggtitle("sample.clean.var sem plot") +
  theme_classic()
#p1

# Cleaned data
p2 = ggplot(nyuad_pos.clean.var, aes(x = seq(SampleName), y = Ct.sem)) +
  geom_point() +
  geom_hline(yintercept = 0.5) +
  ggtitle("nyuad_pos.clean.var sem plot") +
  theme_classic()
#p2
...
sample.classes = samples %>% 
  group_by(SampleName) %>%  
  summarize(SampleName = SampleName[1], PHD_Class = PHD_Class[1], NYUAD_Class = NYUAD_Class[1])
sample.classes
head(sample.classes)
...
```{r eval=FALSE}
# Your answer here
"There are two cases where PHD is positive and NYUAD is negative."
```
...
```{r eval=FALSE}
# Your answer here
"There are 17 cases where PHD is negative and NYUAD is positive."
```
...
std.ctls.p1 = std.ctls.n1.n2.p1
std.ctls.p1

filter(std.ctls.p1, rConc >= 50)

# linear model for rConc based on combined N1 and N2 Ct from "good" plate

#lm.std.rConc = lm(formula = log(Value) ~ rConc, std.ctls.p1)
#lm.std.rConc = lm(formula = Value ~ rConc, std.ctls.p1)
#lm.std.rConc = lm(formula = rConc ~ Value, std.ctls.p1)
lm.std.rConc = lm(formula = log(rConc) ~ Value, std.ctls.p1)
...
ggplot(std.ctls.p1, aes(x = log10(rConc), y = Value, color=AssayName)) +
  #geom_point() +
  geom_jitter() +
  scale_x_continuous(label = function(x){return(round(10^x,1))}) +
  #geom_smooth(method="lm") +
  ...
  #facet_grid(~Plate) +
  facet_wrap(~Plate) +
  ggtitle("Standard samples with a simple linear regression for plate1") +
  theme_classic()
...
nyuad.pos.sample.means1 = nyuad_pos.clean.var %>% 
                #filter(Plate == "Plate1") %>% 
                group_by(SampleName) %>% 
                summarise(PHD_Class, Value = (Ct.mean[1] + Ct.mean[2])/2 )

nyuad.pos.sample.means = left_join(sample.classes, select(nyuad.pos.sample.means1, "Value"))

pred.sample.rConc = predict.lm(lm.std.rConc,
                              newdata = nyuad.pos.sample.means,
                              interval = "prediction")  # "none", "confidence", "prediction"
#pred.sample.rConc
...
df.pred.sample.rConc
str(df.pred.sample.rConc)
summary(df.pred.sample.rConc)
```
...
```{r eval=FALSE}
# Your answer here
"Based on the above results, it can be conclude that the sensitivity of the NYUAD Fluidigm system is much more sensitive which would reduce the rate of false negatives."
```
...
```{r eval=FALSE}
# Your answer here
"Since there is a correlation between the viral load estimated by NYUAD and the original diagnoses of the PHD clinical samples, it can be said that there is a trend between these above two methods."
```
...
df.pred.sample.rConc = mutate(df.pred.sample.rConc, Copies_per_ul = round(10^conc.fit))

# make a file for the final data report
#data.report = left_join( <df1> , select( <df2> , <selected columns> ))

data.report = left_join(sample.classes , 
                         select( df.pred.sample.rConc , "SampleName", "PHD_Class", "NYUAD_Class",  "Copies_per_ul" ))
data.report

# export data file
write.csv(data.report, file = "PHD_samples.viral_load.KEN_TANAKA.csv")
```
