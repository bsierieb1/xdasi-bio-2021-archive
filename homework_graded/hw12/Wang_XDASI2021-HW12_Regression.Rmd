...
neg.ctls = fluid.all %>% filter(SampleName=="RX QC"|SampleName=="NT QC") %>% select(ID,SampleName,AssayName,Value,Call,Plate,rConc) %>% arrange()


# ============================================================================ #
# standards ("CoV 50", "CoV 500", "CoV 5000")
std.ctls = fluid.all %>% filter(SampleName=="CoV 50"|SampleName=="CoV 500"|SampleName=="CoV 5000") %>% select(ID,SampleName,AssayName,Value,Call,Plate,rConc) %>% arrange()
...
```{r eval=FALSE}
# No. The negative control value should be 999. The RTPCR for samples in plate 2 failed because of possible condemnation. And RNA extraction of RP in plate 2 also failed.
```

Do you think you proceed with the analysis using the data on one of the two plates? Explain.

```{r eval=FALSE}
# Yes because controls in plate 1 all look fine.
```
...
```{r eval=FALSE}
# Yes. All are below 20 and with decreasing cycles when Cov concentration increases.
```

Based on the standard controls, do you think you can proceed with the analysis for at least one of the plates? Explain.

```{r eval=FALSE}
# Yes. Both controls work well in plate 1.
```
...
samples = fluid.all %>% filter(SampleName!="CoV 50"&SampleName!="CoV 500"&SampleName!="CoV 5000"&SampleName!="RX QC"&SampleName!="NT QC") %>% select(ID,SampleName,AssayName,Value,Call,Plate,rConc) %>% arrange()
...
ggplot(std.ctls.n1.n2,
       aes(x=log10(rConc),y=Value,col=AssayName))+
  geom_point()+
...
```{r eval=FALSE}
# The regression line looks consistent with our expectation. They are consistent between N1 and N2. And in plate 2 there is a larger slope.
```
...
```{r eval=FALSE}
# The slope is larger in plate 2. The interaction is significant.
```
...
lm.n2.p1 = lm(Value~log10(rConc),data=filter(std.ctls,AssayName=="N2" & Plate=="Plate1"))
lm.n1.p2 = lm(Value~log10(rConc),data=filter(std.ctls,AssayName=="N1" & Plate=="Plate2"))
lm.n2.p2 = lm(Value~log10(rConc),data=filter(std.ctls,AssayName=="N2" & Plate=="Plate2"))
...
```{r eval=FALSE}
# RP gene should be detected by PCR. In plate 2 there are some undetected points.
```
...
rp.p1 = filter(valid.samples,Plate=="Plate1")$Value
rp.p2 = filter(valid.samples,Plate=="Plate2")$Value
...
t.test(rp.p1,rp.p2,paired=FALSE)
...
```{r eval=FALSE}
# Yes. They are significantly from the same distribution.
```
...
```{r eval=FALSE}
# Yes the positive samples have a wide-spread distribution
```
...
data.report = left_join( <df1> , 
                        select( <df2> , <selected columns> ))
