...
neg.ctls = fluid.all %>%
  ...
  select(ID, Sample, SampleName, AssayName, rConc, Value, Call, Plate)
head(neg.ctls)
...
ggplot(data = neg.ctls, mapping = aes(SampleName, Value)) +
  geom_boxplot() +
  geom_jitter() +
...
```{r eval=FALSE}
# The negative controls in plate 1 look fine (around 0.999), but a lot of the negative controls in plate 2 look like there has been a mistake, especially in the RX_QC controls.
```

Do you think you proceed with the analysis using the data on one of the two plates? Explain.

```{r eval=FALSE}
# I would only proceed with analyzing the data on the first plate since it all looks good and the weirdness of the controls in the second plate puts the rest of the plate into question.
```
...
ggplot(data = std.ctls, mapping = aes(SampleName, Value)) +
  geom_boxplot() +
  geom_jitter() +
  facet_grid(Plate ~ AssayName)
...
ggplot(data = std.ctls.n1.n2, mapping = aes(SampleName, Value)) +
  geom_boxplot() +
  geom_jitter() +
  facet_grid(Plate ~ AssayName)
...
```{r eval=FALSE}
# The N1 and N2 standards look fine in both plates, but the RP Ct values in plate 2 are kind of concerning because they should all be around 999 again, but for CoV50 and CoV500 they are not. 
```

Based on the standard controls, do you think you can proceed with the analysis for at least one of the plates? Explain.

```{r eval=FALSE}
# I would still proceed with analysis for plate 1 only.
```
...
ggplot(data = std.ctls.n1.n2, 
       mapping = aes(log10(rConc), Value, color = AssayName)) +
  geom_point() +
...
```{r eval=FALSE}
# The data fits the model in plate 1 better than the plate 2 data fits the plate 2 model. However, the decreasing trend is the same, although the slopes are different.In both cases, N2 and N1 seem to have the same slope within their plate but N2 has a greater Y intercept than N1.
```
...
lm.n1.p1 = lm(formula = Value ~ log10(rConc),
               data = filter(std.ctls, AssayName == "N1" & Plate == "Plate1"))

lm.n2.p1 = lm(formula = Value ~ log10(rConc),
               data = filter(std.ctls, AssayName == "N2" & Plate == "Plate1"))

lm.n1.p2 = lm(formula = Value ~ log10(rConc),
               data = filter(std.ctls, AssayName == "N1" & Plate == "Plate2"))

lm.n2.p2 = lm(formula = Value ~ log10(rConc),
               data = filter(std.ctls, AssayName == "N2" & Plate == "Plate2"))
...
```{r eval=FALSE}
# Plate 1 because the R squared values are higher. 
```
...
ggplot(data = filter(samples, AssayName == "RP"), mapping = aes(PHD_Class, Value)) +
  geom_violin() +
  facet_grid(~Plate)
...
```{r eval=FALSE}
# Plate2 looks like it has some weird samples in it. I'd expect the values to fall all between 50 and 0. 
```
...
ggplot(data = samples.rp, mapping = aes(PHD_Class, Value)) +
  geom_violin() +
  facet_grid(~Plate)
...
rp.p1 = filter(samples.rp, Plate == "Plate1")$Value

rp.p2 = filter(samples.rp, Plate == "Plate2")$Value
...
```{r eval=FALSE}
# Wilcox test says that they both come from the same distribution so no significant difference between plates. 
```
...
ggplot(data = nyuad.pos.samples, mapping = aes(AssayName, Value)) +
  geom_violin() +
  facet_wrap(~Plate)
```
...
```{r}
# by PHD diagnosis
ggplot(data = nyuad.pos.samples, mapping = aes(PHD_Class, Value)) +
  geom_violin() +
  facet_grid(Plate ~ AssayName)
```
...
```{r eval=FALSE}
# Negative samples across the board have a tighter range and have higher values on average than positive samples, which means they have higher Ct values which means they have a lower viral load.
```
...
p1 = ggplot(data = sample.clean.var, mapping = aes(seq(SampleName), SEM.Ct)) +
  geom_point() +
  geom_hline(yintercept=0.5, color = "hotpink")


# Cleaned data
p2 = ggplot(data = nyuad_pos.clean.var, mapping = aes(seq(SampleName), SEM.Ct)) +
  geom_point() +
  geom_hline(yintercept=0.5, color = "hotpink")
...
sample.classes = samples %>%
  group_by(SampleName) %>% 
  summarize(SampleName = unique(SampleName), PHD_Class = unique(PHD_Class), NYUAD_Class = unique(NYUAD_Class))
...
nyuad.pos.sample.means = nyuad_pos.clean.var %>%
  group_by(SampleName) %>%
  summarize(Value = mean(Ct.mean), PHD_Class = first(PHD_Class), Plate = first(Plate))
...
```{r eval=FALSE}
# Negative samples have higher Ct which tracks because that means it has lower viral load.
```
...
# export data file
write.csv(data.report, "PHD_samples.viral_load.MariShiratori.csv")
