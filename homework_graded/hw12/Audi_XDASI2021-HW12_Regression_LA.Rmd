...
fluid.all = read.csv("/Users/sicg_labmanager/Documents/NYU BIO/Biostats/XDASI2021-HW12_Regression/data/Fluidigm_1691207281.csv")
...
# ============================================================================ #
# standards ("CoV 50", "CoV 50", "CoV 5000")
std.ctls = fluid.all %>% 
...
ggplot(neg.ctls, aes(x = SampleName, y= Value)) +
  geom_boxplot() + 
  geom_jitter() +
...
```{r eval=FALSE}
# The N controls in plate 1 look good, but many of the neg controls in P3 look like there may have been an issue (RX_QC). 
```

Do you think you proceed with the analysis using the data on one of the two plates? Explain.

```{r eval=FALSE}
# I would proceed with the first plate only 
```
...
ggplot(std.ctls, aes(x = SampleName, y= Value)) +
  geom_boxplot() + 
  geom_jitter() +
  facet_grid(Plate ~ AssayName)
```
...
ggplot(std.ctls.n1.n2, aes(x = SampleName, y= Value)) +
  geom_boxplot() +
  geom_jitter() +
  facet_grid(Plate ~ AssayName)
```
...
```{r eval=FALSE}
# N1 and N2 look good on both plates. but the RP Ct values in plate 2 are not at 999 for CoV50 and CoV500.
```
...
samples =  fluid.all %>% 
  filter (SampleName != "CoV 50" & SampleName != "CoV 500" & SampleName != "CoV 5000" & SampleName != "RX QC" & SampleName != "NT QC" ) %>% 
  select(ID, SampleName, AssayName, Value, Call, Plate, rConc)


# Import PHD results from .csv file
phd.calls = read.csv("/Users/sicg_labmanager/Documents/NYU BIO/Biostats/XDASI2021-HW12_Regression/data/PHD_results.csv")

# Add a column to the Fluidigm file containing the PHD results
# use the inner_join() function to do this
samples = inner_join( phd.calls , samples , 
                     by.x = c("SampleName"), 
...
#IS THIS RIGHT??! 
ggplot(std.ctls.n1.n2, aes(x =(log10(rConc)), y = Value , col = AssayName)) +
  geom_point() +
  facet_wrap(~Plate) +
...
```{r eval=FALSE}
# Plate 1 is a better fit than the plate 2. They both have negative slope, though the slope itself is different between plates. N2 and N1 seem to have the same slope within their plate.  N2 has a greater Y intercept than N1.
```
...
# IS THIS WHAT THEY ARE ASKING -- how value is effected by interactiong of R conc and plate 
summary(lm (Value ~ (log10(rConc)) *Plate, data = std.ctls.n1.n2))

#this didn't  work 
#Anova(lm(formula = Value ~ (log10(rConc)) *Plate, data = std.ctls.n1.n2),  type = "III")
...
```{r eval=FALSE}
# The interaction term is significant. Slope on plate 2 is greater than slope on plate 1. 
```
...
lm.n1.p1 = lm(Value ~ (log10(rConc)), data = filter(std.ctls, AssayName == "N1" & Plate == "Plate1"))
lm.n2.p1 = data = lm( Value ~ (log10(rConc)), data = filter(std.ctls, AssayName == "N2" & Plate == "Plate1"))
lm.n1.p2 = data = lm(Value ~ (log10(rConc)), data = filter(std.ctls, AssayName == "N1" & Plate == "Plate2"))
lm.n2.p2 = data = lm(Value ~ (log10(rConc)), data = filter(std.ctls, AssayName == "N2" & Plate == "Plate2"))

# look at the summaries of the models
#R^2 > 0.975
summary(lm.n1.p1) #ok
summary(lm.n2.p1)
summary(lm.n1.p2) # r is low
summary(lm.n2.p2) # r is low 
```
...
```{r eval=FALSE}
# Plate 1 has a higher R squared and a slope that shows a better amplification efficiency compared to plate 2. 
```
...
# i think this is the samples df now ?
#View(samples)

ggplot(data = filter(samples, AssayName == "RP"), aes(x = PHD_Class, y= Value)) +
  geom_violin() +
  facet_grid(~Plate)
...
```{r eval=FALSE}
# Your answer here
#Plate 2 does not look good, has flagged samples in both neg and pos. 
```
...
ggplot(samples.rp, aes(x = PHD_Class, y= Value)) +
  geom_violin(aes(fill = PHD_Class)) + 
  facet_grid(~Plate)
...
bartlett.test(list(rp.p1$Value  , rp.p2$Value))
##not sig difference in variance between the two plates
var.test(rp.p1$Value  , rp.p2$Value)
#variation is the same between groups 
...
```{r eval=FALSE}
# I think so because even thoough both are not normally distrbuted, they have similar variances. 
```
...
ggplot(nyuad.pos.samples, aes(x = AssayName, y= Value)) +
  geom_violin(aes(fill = AssayName)) + 
  facet_grid(~Plate)
...
```{r}
# by PHD diagnosis
ggplot(nyuad.pos.samples, aes(x = PHD_Class, y= Value)) +
  geom_violin(aes(fill = PHD_Class)) + 
  facet_wrap(Plate ~ AssayName)
...
```{r eval=FALSE}
#Negative CT values generally have higher Ct values, whereas Positive have much large distribution of Ct values. 
```
...
ggplot(sample.clean.var, aes(x = seq(SampleName), y= SEM)) +
  geom_point() + 
 geom_hline(yintercept = 0.5)

#everyone is less than 0.5 here 
ggplot(nyuad_pos.clean.var, aes(x = seq(SampleName), y= SEM)) +
  geom_point() + 
 geom_hline(yintercept = 0.5)


# All samples with N1/N2 detected
p1 = ggplot(sample.clean.var, aes(x = seq(SampleName), y= SEM)) +
  geom_point() + 
 geom_hline(yintercept = 0.5)

# Cleaned data
p2 = ggplot(nyuad_pos.clean.var, aes(x = seq(SampleName), y= SEM)) +
  geom_point() + 
 geom_hline(yintercept = 0.5)
...
sample.classes = samples %>%
  group_by(SampleName)%>% 
  summarize(PHD_Class,NYUAD_Class )
head(sample.classes)
...
nyuad.pos.sample.means = nyuad_pos.clean.var %>%
  group_by(SampleName) %>%
  summarize(Value = mean(Ct.mean), PHD_Class, Ct.sd, n, SEM)

head(nyuad.pos.sample.means)
...
```{r eval=FALSE}
```{r eval=FALSE}
# Negative samples have higher Ct (lower viral load) and vice versa for pos! Wooo! 
```
...
df.pred.sample.rConc = df.pred.sample.rConc %>%
  mutate(Copies_per_ul = signif(10^conc.fit, digits = 2))
head(df.pred.sample.rConc)
...
# export data file
write.csv(data.report, "PHD_samples.viral_load.LA.csv")
```
