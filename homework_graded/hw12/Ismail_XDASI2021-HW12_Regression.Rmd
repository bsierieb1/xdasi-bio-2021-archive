...
fluid.all <- read.csv("Fluidigm_1691207281.csv", header=T)
head(fluid.all)
class(fluid.all)
#View(fluid.all)
```
...
neg.ctls = fluid.all %>% filter(SampleName ==  "RX QC" | SampleName == "NT QC" )  %>% select(-X, -Sample, -Assay, -Type, -rConc, -Quality, -Threshold)

#View(neg.ctls)
# ============================================================================ #
# standards ("CoV 50", "CoV 500", "CoV 5000")
std.ctls = fluid.all %>% filter(SampleName ==  "CoV 50" | SampleName == "CoV 500" | SampleName == "CoV 5000") %>% select(-X, -Sample, -Assay, -Type, -Quality, -Threshold)
...
neg_plot <- ggplot(neg.ctls, mapping = aes(x = SampleName,
                                           y = Value)) + geom_boxplot()


neg_plot + facet_grid(Plate ~ AssayName)
```
...
```{r eval=FALSE}
#Not all the negative controls looks okay, plate 2 has 4 controls that showed very low Ct values ( almost near zero), the ideal negative control should have around 999 ct values, this might be attributed to pipetting errors or cross contamination that happened by mistake from other positive samples 
```

Do you think you proceed with the analysis using the data on one of the two plates? Explain.

```{r eval=FALSE}
#we should proceed with plate 1 , since plate 2 is any way biased and we can not rely on any data coming from it. The other wells in plate 2 might also be contaminated by mistake, that means there is a chance of having some false positive samples
```
...
std_plot <- ggplot(std.ctls, mapping = aes(x = SampleName,
                                           y = Value)) + geom_boxplot()


std_plot + facet_grid(Plate ~ AssayName)
...
std.ctls.n1.n2 = std.ctls %>% 
  filter(AssayName == "N1" | AssayName == "N2")

# ggplot for just N1 and N2

n1_n2_plot <- ggplot(std.ctls.n1.n2, mapping = aes(x = SampleName,
                                           y = Value)) + geom_boxplot()


n1_n2_plot + facet_grid(Plate ~ AssayName)
...
```{r eval=FALSE}
#Not all of them are okay, plate 2 has problem because it has one RP that is = to 20 in cov50 (N2) and another possible one that is just lower than 20 (N1) ,, so this is considered as positive, as the normal range should be within the 999 range
```

Based on the standard controls, do you think you can proceed with the analysis for at least one of the plates? Explain.

```{r eval=FALSE}
#yes we can proceed with plat 1, as the controls look fine
```
...
samples = fluid.all %>% 
  filter(SampleName != "RX QC" & SampleName != "NT QC" & SampleName != "CoV 50" & SampleName != "CoV 500" & SampleName != "CoV 5000") %>% 
  select(-X, -Sample, -Assay, -Type, -rConc, -Quality, -Threshold)

#View(samples)


# Import PHD results from .csv file
phd.calls = read.csv("D:/NYU/Statistics_course/week_13/PHD_results.csv" , header = T)
...
samples$NYUAD_class <- rep(NA)
names(samples)[names(samples) == 'Class'] <- 'PHD_Class'
head(samples)
```
...
quality_control <- ggplot(std.ctls.n1.n2, mapping = aes(x = log10(rConc),
                                           y = Value,
                                           color = AssayName )) + 
 geom_point() + facet_wrap(~Plate) + scale_x_continuous(label = function(x){return(round(10^x,1))})  +  geom_smooth(method="lm") 
...
```{r eval=FALSE}
#The regression line in plate 1 is fitting most of the points and N1 and N2 looks similar. IN case of plate 2, the regression line is not fitting all of the points and  N1 and N2 of plate 2 are having higher values than in plate 1
```
...
summary(lm(formula = Value ~ log10(rConc) * Plate,
           data = std.ctls.n1.n2))
```
...
```{r eval=FALSE}
#In case of plate 2, the slope difference is around 2.7 less than that of plate 1, The interaction term is significant of p-value less than 2e-16
```
...
lm.n1.p1 = lm(formula = Value ~ log10(rConc),
           data = filter(std.ctls.n1.n2, AssayName == "N1" & Plate == "Plate1"))

lm.n2.p1 = lm(formula = Value ~ log10(rConc),
           data = filter(std.ctls.n1.n2, AssayName == "N2" & Plate == "Plate1"))
lm.n1.p2 = lm(formula = Value ~ log10(rConc),
           data = filter(std.ctls.n1.n2, AssayName == "N1" & Plate == "Plate2"))
lm.n2.p2 = lm(formula = Value ~ log10(rConc),
           data = filter(std.ctls.n1.n2, AssayName == "N2" & Plate == "Plate2"))
...
```{r eval=FALSE}
#Plate 1 is better , because the efficiency is higher and the slope is closer to the normal values which ranges from -3.1 to -3.6. 
```
...
```{r eval=FALSE}
#RP assays in the two plates looks a bit similar as they are all human samples, which is good, that means that RP gene is positive and consistent across all samples. But Plate 2 showed some extreme CT values of 999, which means that we have something wrong in plate 2 as the RP detection was failed in these extreme samples.
```
...
ggplot(samples.rp , aes(x= PHD_Class,
                        y = Value)) +
  geom_violin() +
  facet_wrap(~Plate)
...
```{r eval=FALSE}
# Yes it is okay, because the variance of RP genes looks the same ( they both come from same distribution ) as per the results of wilcoxon test. Although shapiro test showed that they both are non normal but looks like we can use the standards from only one of the plates to quantify the viral load in all the samples 
```
...
ggplot(nyuad.pos.samples, aes( x = AssayName,
                               y = Value)) +
...
ggplot(nyuad.pos.samples, aes( x = PHD_Class,
                               y = Value,
                               col = AssayName)) +
...
```{r eval=FALSE}
#the negative samples in the PHD test are considered as positive in late PCR cycles in the NYUAD assay, that might be attributed that the negative samples had very small viral load to be detected in the PHD test, but in case of NYUAD assay, the system showed high sensitivity to detect it 
```
...
sample.clean.var = sample.clean.var %>% mutate(SEM_CT = Ct.sd / sqrt(n))
nyuad_pos.clean.var = nyuad_pos.clean.var %>% mutate(SEM_CT = Ct.sd / sqrt(n))
```
...
p1 = ggplot(sample.clean.var , aes( x = seq(SampleName),
                                    y = SEM_CT)) +
  geom_jitter() +
  geom_hline(yintercept = 0.5 , col = "blue")

# Cleaned data
p2 = ggplot(nyuad_pos.clean.var , aes( x = seq(SampleName),
                                    y = SEM_CT)) +
  geom_jitter() +
  geom_hline(yintercept = 0.5 , col = "blue")
...
sample.classes = samples %>%group_by(SampleName, PHD_Class, NYUAD_Class)%>% summarize()
...
std.ctls.p1 = std.ctls %>% filter(Plate == "Plate1"& AssayName == c("N1" , "N2"))
#View(std.ctls.p1)

# linear model for rConc based on combined N1 and N2 Ct from "good" plate
lm.std.rConc = lm(log10(rConc) ~ Value, std.ctls.p1)
```
...
ggplot(std.ctls.p1, aes(x=log10(rConc), y=Value)) +
    geom_smooth(method="lm", level=0.95) + geom_point() +
  scale_x_continuous(label = function(x){return(round(10^x,1))}) +
  xlab("conc viral RNA") + ylab("Ct Value")
...
nyuad.pos.sample.means = nyuad.pos.samples %>% group_by( SampleName, PHD_Class) %>% summarize(Value = mean(Value))
...
```{r eval=FALSE}
# The NYUAD system is very sensitive as it can detect very minute viral load
```
...
```{r eval=FALSE}
# The NYUAD system is more sensitive than the PHD as it was able to detect very minute viral load. The general trend is that the PHD assay can detect viral loads with >100 copies, while NYUAD can detect less than <100 copies
```
...
df.pred.sample.rConc$Copies_per_ul  = signif((10 ^ df.pred.sample.rConc$conc.fit),2)
View(df.pred.sample.rConc)

# export data file

data.report = left_join(sample.classes,
                        select(df.pred.sample.rConc, SampleName, PHD_Class, Copies_per_ul))
#View(sample.classes)
#View(data.report)



write.csv(data.report,"D:/NYU/Statistics_course/week_13/PHD_samples.viral_Noha_Ismail.csv", row.names = FALSE)
