...
fluid.all = read.csv('/Users/eladerman/NYU/Classes/Biostatistics/Week13/XDASI2021-HW12_Regression/data/Fluidigm_1691207281.csv')
tail(fluid.all)
```
...
neg.ctls = fluid.all[which(fluid.all$SampleName %in% c('RX QC', 'NT QC')), c('SampleName', 'AssayName', 'Value', 'Call', 'Plate')]

neg.ctls
# ============================================================================ #
# standards ("CoV 50", "CoV 500", "CoV 5000")
std.ctls = fluid.all[which(fluid.all$SampleName %in% c("CoV 50", "CoV 500", "CoV 5000")), c('SampleName', 'AssayName', 'Value', 'Call', 'Plate', 'rConc')]
...
ggplot(data = neg.ctls, mapping = aes(x = SampleName, y = Value)) + geom_boxplot() + facet_grid(Plate ~ AssayName)
...
```{r eval=FALSE}
# Some negative controls on plate 2 are not okay. For the RX control on plate 2 it seems to be contaminated with a mix of COVID and Human RNA as all primer sets show amplification. The NT control is only contaminated with human DNA as only the RP primers show amplification.
```

Do you think you proceed with the analysis using the data on one of the two plates? Explain.

```{r eval=FALSE}
# I think I should not use plate two in the subsequent analysis as the negative controls are not reliable.
```
...
ggplot(data = std.ctls, mapping = aes(x = SampleName, y = Value)) + geom_boxplot() + facet_grid(Plate ~ AssayName)
...
std.ctls.n1.n2 = std.ctls %>% filter(AssayName != 'RP')

# ggplot for just N1 and N2
ggplot(data = std.ctls.n1.n2, mapping = aes(x = SampleName, y = Value)) + geom_boxplot() + facet_grid(Plate ~ AssayName)
...
```{r eval=FALSE}
# No once again plate 2 is messed up for the RP primer set as some standard controls are showing human contamination.
```

Based on the standard controls, do you think you can proceed with the analysis for at least one of the plates? Explain.

```{r eval=FALSE}
# Once again I think that I should discard plate 2
```
...
# ============================================================================ #
# samples
sample_names = setdiff(unique(fluid.all$SampleName), c('RX QC', 'NT QC', "CoV 50", "CoV 500", "CoV 5000"))

# filter fluid.all to keep just the samples and none of the controls
samples = fluid.all %>% filter(SampleName %in% sample_names) %>% select(SampleName, AssayName, Value, Call, Plate)

samples

# Import PHD results from .csv file
phd.calls = read.csv('/Users/eladerman/NYU/Classes/Biostatistics/Week13/XDASI2021-HW12_Regression/data/PHD_results.csv')
...
ggplot(data = std.ctls.n1.n2, mapping = aes(x = log10(rConc), y = Value, col=AssayName)) + geom_point() + geom_smooth(method="lm") + facet_wrap(~Plate)
...
```{r eval=FALSE}
# The data look quite different between the two plates and moderately different between N1 and N2 on the first plate
```
...
model = lm(Value ~ log10(rConc) * Plate, data = std.ctls.n1.n2)
summary(model)
```
...
```{r eval=FALSE}
# The slope on plate 2 is greater than that on plate 1 and the interaction term is significant.
```
...
lm.n1.p1 = lm(Value ~ log10(rConc), data = std.ctls.n1.n2[std.ctls.n1.n2$Plate == 'Plate1' & std.ctls.n1.n2$AssayName == 'N1',])
lm.n2.p1 = lm(Value ~ log10(rConc), data = std.ctls.n1.n2[std.ctls.n1.n2$Plate == 'Plate1' & std.ctls.n1.n2$AssayName == 'N2',])
lm.n1.p2 = lm(Value ~ log10(rConc), data = std.ctls.n1.n2[std.ctls.n1.n2$Plate == 'Plate2' & std.ctls.n1.n2$AssayName == 'N1',])
lm.n2.p2 = lm(Value ~ log10(rConc), data = std.ctls.n1.n2[std.ctls.n1.n2$Plate == 'Plate2' & std.ctls.n1.n2$AssayName == 'N2',])
...
```{r eval=FALSE}
# Plate1 because the slopes are closer to what we consider good, the R-squared is higher, and the eff is much higher
```
...
ggplot(data = samples %>% filter(AssayName == 'RP'), 
       mapping = aes(x = PHD_Class, y = Value)) +
  geom_violin() +
  facet_wrap(~Plate)
...
```{r eval=FALSE}
# All of these should be below 999. Some of the plate2 samples are wonky
```
...
valid.samples = merge(samples, valid.sample.ids, by = "SampleName") %>% 
  arrange(SampleName) %>% select(-TotalPass)
valid.samples
# ============================================================================ #
...
```{r eval=FALSE}
length(unique(valid.samples$SampleName))
```
...
samples.rp = valid.samples %>% filter(AssayName == 'RP') %>% select(Plate, Value, PHD_Class)

# violin plot of Ct values for valid assays, filtered for RP gene, by PHD diagnosis
ggplot(data = samples.rp, mapping = aes(x = PHD_Class, y = Value)) + geom_violin() + facet_wrap(~Plate)
...
rp.p1 = (valid.samples %>% filter(AssayName == 'RP' & Plate == 'Plate1') %>% select(Value))$Value
rp.p2 = (valid.samples %>% filter(AssayName == 'RP' & Plate == 'Plate2') %>% select(Value))$Value
...
```{r eval=FALSE}
# Yes because the wilcox test is not significant
```
...
nyuad.pos.samples = merge(valid.samples, nyuad.pos.sample.ids, by = "SampleName") %>% 
  ...
  select(SampleName,PHD_Class,AssayName,Value,Plate) %>% 
  arrange(Plate, SampleName, AssayName)
...
```{r eval=FALSE}
# Your answer here
length(unique(nyuad.pos.samples$SampleName))
```
...
nyuad.pos.samples.n1.n2 = nyuad.pos.samples %>% filter(AssayName == 'N1' | AssayName == 'N2') %>% select(Plate, Value, PHD_Class, AssayName)

ggplot(data = nyuad.pos.samples.n1.n2, mapping = aes(x = AssayName, y = Value)) + geom_violin() + facet_wrap(~Plate)
```
...
ggplot(data = nyuad.pos.samples.n1.n2, mapping = aes(x = PHD_Class, y = Value)) + geom_violin() + facet_wrap(Plate ~AssayName)
...
```{r eval=FALSE}
# The ones that are negative have a tighter distribution and have a higher average ct value indicating they have lower levels of the virus
```
...
sample.clean.var = sample.clean.var %>% mutate(SEM = Ct.sd / n)
nyuad_pos.clean.var = nyuad_pos.clean.var %>% mutate(SEM = Ct.sd / n)
```
...
ggplot(data = sample.clean.var, mapping = aes(seq(SampleName), SEM)) + geom_point()
ggplot(data = nyuad_pos.clean.var, mapping = aes(seq(SampleName), SEM)) + geom_point()
# All samples with N1/N2 detected
p1 = ggplot(data = nyuad_pos.clean.var, mapping = aes(seq(SampleName), SEM)) + geom_point()

# Cleaned data
p2 = ggplot(data = sample.clean.var, mapping = aes(seq(SampleName), SEM)) + geom_point()
...
sample.classes = samples %>% group_by(SampleName) %>% summarise(SampleName = unique(SampleName)[1], PHD_Class = unique(PHD_Class)[1], NYUAD_Class = unique(NYUAD_Class)[1])
head(sample.classes)
...
lm.std.rConc = lm(log10(rConc) ~ Value, data = std.ctls.p1)
```
...
ggplot(data = std.ctls.p1, mapping = aes(x = log10(rConc), y = Value)) + geom_point() + geom_smooth(method="lm")
...
nyuad_pos.clean.var
nyuad.pos.sample.means = nyuad_pos.clean.var %>% group_by(SampleName) %>% summarise(Plate = unique(Plate)[1], Value = mean(Ct.mean), PHD_Class = unique(PHD_Class)[1])

nyuad_guide = samples[samples$SampleName %in% nyuad.pos.sample.means$SampleName,c('SampleName', 'NYUAD_Class')] %>% group_by(SampleName) %>% summarise(NYUAD_Class = unique(NYUAD_Class)[1])

nyuad.pos.sample.means = merge(nyuad.pos.sample.means, nyuad_guide, by='SampleName')
nyuad.pos.sample.means
...
```{r eval=FALSE}
# The NYUAD fluidigm is more sensitive than the other method
```
...
```{r eval=FALSE}
# The negative samples tend to have a lower viral load and the fluidigm can detect this
```
...
df.pred.sample.rConc = df.pred.sample.rConc %>% mutate(Copies_per_ul = 10^(conc.fit))
sample.classes
...
data.report
# export data file
write.csv(data.report, file = 'PHD_samples.viral_load.ERICLADERMAN.csv')
