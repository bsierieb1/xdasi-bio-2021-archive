...
author: "Rob Tibshirani [Luok Wen Yong]"
...
fluid.all = read.csv("/Users/lwyong/Desktop/LW Yong NYU/Biostat 2021 Fall/Week 13/XDASI2021-HW12_Regression/data/Fluidigm_1691207281.csv", stringsAsFactors = TRUE)
head(fluid.all)
str(fluid.all)
```
...
neg.ctls = fluid.all %>% filter(SampleName %in% c("RX QC","NT QC")) %>% select(ID,SampleName,AssayName,Value, Call, Plate, rConc) %>% arrange(ID)


# ============================================================================ #
# standards ("CoV 50", "CoV 500", "CoV 5000")
std.ctls = fluid.all %>% filter(SampleName %in% c("CoV 50", "CoV 500", "CoV 5000")) %>% select(ID,SampleName,AssayName,Value, Call, Plate, rConc) %>% arrange(ID)
...
g.neg<-ggplot (neg.ctls, mapping = aes(x = SampleName,
                                y = Value)) + geom_boxplot()

g.neg + facet_grid(Plate ~ AssayName)
...
```{r eval=FALSE}
# NO! The negative controls in plate 2 does not look okay. Negative controls should have a Ct value of 999 but in plate 2, there are certain negative controls that have Ct values other than 999, which indicates some sort of contamination. 
```

Do you think you proceed with the analysis using the data on one of the two plates? Explain.

```{r eval=FALSE}
# Yes, I will proceed with analyzing the results with only plate 1. The negative controls in this plate are fine - ie nothing(almost nothing) is amplified. 
```
...
+ The positive controls should amplify the viral N gene using both sets of primers (N1 and N2), but not the human RP gene. [copied from above]
...
g.pos<-ggplot (std.ctls, mapping = aes(x = SampleName,
                                y = Value)) + geom_boxplot()

g.pos + facet_grid(Plate ~ AssayName)
...
g.pos2<-ggplot (std.ctls.n1.n2, mapping = aes(x = SampleName,
                                y = Value)) + geom_boxplot()

g.pos2 + facet_grid(Plate ~ AssayName)
...
```{r eval=FALSE}
# Not all looks okay, the standard control for plate 2 at a lower "plasmid" dilution (CoV50) is at Ct Value 20.0. 
```

Based on the standard controls, do you think you can proceed with the analysis for at least one of the plates? Explain.

```{r eval=FALSE}
# Yes, we can analyse the results of plate 1. With plate 1, all the standards have Ct values of below 20.0(within acceptable range). The Ct values also decrease with increasing concentration of control plasmids. 
```
...
samples = fluid.all %>% filter(!SampleName %in% c("RX QC","NT QC","CoV 50","CoV 500","CoV 5000")) %>%
select(ID,SampleName,AssayName,Value, Call, Plate, rConc) %>% arrange(SampleName)


# Import PHD results from .csv file
phd.calls = read.csv("/Users/lwyong/Desktop/LW Yong NYU/Biostat 2021 Fall/Week 13/XDASI2021-HW12_Regression/data/PHD_results.csv")
...
colnames(samples)[which(names(samples) == "Class")] <- "PHD_Class" 
head(samples)
...
ggplot(std.ctls.n1.n2, aes(x = log10(rConc),
                           y = Value,
                           col = AssayName)) +
  facet_wrap(~Plate)+
  geom_jitter()+
...
```{r eval=FALSE}
# The regression line looks like the run parallel to each other for both graphs. The changes in the standards is reflected with corresponding Ct value changes as well. N1 and N2 assay (for each plate) seems okay, the regression line fits the data well (in plate 1 the dots is kinda evenly spread from the regression line). Plate 2 seems relatively fine but has a more slanted slope, and the dots is not as evenly spread across the regression line.  
```
...
```{r eval=FALSE}
#The slope for plate 1 vs plate 2 has a 2.7787 unit difference. 
#the interaction term is significant as p-value is lower than 0.05.
```
...
```{r eval=FALSE}
# We will choose plate 1. Because the efficiency is better in plate 1 (the slope is around -2 for P1, which nearer to the ideal -3.1 to -3.6 range). plate 2's efficiency is much more off (approx -5).
```
...
```{r eval=FALSE}
# The RP assays should be consistent between Negative and positive cases. Not everything looks okay, in plate 2, some of the RP Ct values are very distant/far away from the main "log10(1)" region where the majority are, which means some of the positive controls are not working well. 
```
...
samples.rp = valid.samples %>% filter(AssayName == "RP")
...
rp.p1 = valid.samples %>% filter(Plate == "Plate1" & AssayName == "RP") %>% pull(Value)
rp.p2 = valid.samples %>% filter(Plate == "Plate2" & AssayName == "RP") %>% pull(Value)
...
## Test for normality #very NOT normal
shapiro.test(rp.p1)
shapiro.test(rp.p2)
...
```{r eval=FALSE}
# Yes. Althought both are not normally distributed, the kind of come from the same distribution (according to the wilcox.test, p value is greated than 0.05 hence null is not rejected). this means you can use the standards in plate 1 as a stand in for the real data in plate 2. 
```
...
```{r eval=FALSE}
# For the negative, the Ct value is consistently high above twenty-ish. Interestignly, some of the positives are also above 20. 
```
...
sample.clean.var = sample.clean.var %>% mutate(SEM.ct = Ct.sd/sqrt(n))
nyuad_pos.clean.var = nyuad_pos.clean.var %>% mutate(SEM.ct = Ct.sd/sqrt(n))
```
...
sample.classes = 
  samples %>% group_by(SampleName, PHD_Class, NYUAD_Class) %>% summarise()

#SampleName, PHD_Class, NYUAD_Class
head(sample.classes)
...
```{r eval=FALSE}
# 2 (did not count the inconclusive ones)
```
...
ggplot(std.ctls.p1,
  ...
  geom_smooth(method = 'lm') +
  scale_x_continuous(label = function(x){return(round(10^x,1))})+
  xlab("Known concentration of viral RNA (copies / ul)") + ylab("Ct Value") 
...
nyuad.pos.sample.means = 
  nyuad.pos.samples %>% group_by(SampleName, PHD_Class) %>% summarise(Value = mean(Value))
...
```{r eval=FALSE}
# It is very sensitive. You can have positives are 1 viral copy per uL concentration. 
```
...
```{r eval=FALSE}
# NYUAD Fluidign system is very sensitive compared to the PHD system. 
```
...
# export data file
write.csv(data.report,"/Users/lwyong/Desktop//PHD_samples.viral_load.LUOKWENYONG.csv", row.names = FALSE)
```
