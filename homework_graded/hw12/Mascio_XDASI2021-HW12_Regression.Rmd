...
fluid.all <- fluid.all %>% select(c("ID", "SampleName", "rConc", "AssayName", "Value", "Call", "Plate"))
head(fluid.all)
```
...
neg.ctls <- fluid.all %>% filter(SampleName == "RX QC" | SampleName == "NT QC") %>% select(-c("rConc"))
...
```{r eval=FALSE}
# No, the negative controls should not amplify anything. The NT QC is for PCR, so there may have been some contamination since there should be no PCR present. The RX QC should not extract RNA since only buffer in used. Possibly RNA could have been left in the samples or contamination happened that got replicated in PCR. It looks like Plate 1 has all negatives (999 cycles).
```

Do you think you proceed with the analysis using the data on one of the two plates? Explain.

```{r eval=FALSE}
# Yes, use only plate 1 results for the controls and data. We know the controls are ok so we can be confident in comparing our data to them
```
...
```{r eval=FALSE}
# standards look ok. In plate 2 the human control is showing up in plate 2 but not plate 1, again suggesting plate 2 had contamination.
```

Based on the standard controls, do you think you can proceed with the analysis for at least one of the plates? Explain.

```{r eval=FALSE}
# Yes, plate 1 is acceptable, not plate 2
```
...
samples <- fluid.all %>% filter(SampleName != "RX QC" & SampleName != "NT QC" & SampleName != "CoV 50" & SampleName != "CoV 500" & SampleName != "CoV 5000") %>% select(-"rConc")
...
```{r eval=FALSE}
# Concentrations increase and Ct gets lower. Things are consistent between N1 and N2, but the plates differ. The ribbons for plate 2 seem to overlap.
```
...
```{r eval=FALSE}
# 7.89 difference. interaction term is significant
```
...
lm.n1.p1 = lm(formula = Value ~ log10(rConc), data = filter(std.ctls, AssayName == "N1" & Plate == "Plate1"))
lm.n2.p1 = lm(formula = Value ~ log10(rConc), data = filter(std.ctls, AssayName == "N2" & Plate == "Plate1"))
lm.n1.p2 = lm(formula = Value ~ log10(rConc), data = filter(std.ctls, AssayName == "N1" & Plate == "Plate2"))
lm.n2.p2 = lm(formula = Value ~ log10(rConc), data = filter(std.ctls, AssayName == "N2" & Plate == "Plate2"))
...
```{r eval=FALSE}
# plate 1 because the eff. is closer to 100% and all the controls looked better in it
```
...
```{r eval=FALSE}
# RP should be positive for all clinical samples but late 2 has some negative values
```
...
ggplot(data = samples.rp, aes(x = PHD_Class, y = Value)) +
  geom_violin(aes(fill = PHD_Class)) +
  facet_wrap(~Plate)
```
...
rp.p1 = samples.rp$Value[samples.rp$Plate == "Plate1"]
rp.p2 = samples.rp$Value[samples.rp$Plate == "Plate2"]
...
```{r eval=FALSE}
# yes these samples appear to be from the same distribution
```
...
ggplot(data = nyuad.pos.samples, aes(x = AssayName, y = Value)) +
...
```{r}
# by PHD diagnosis
ggplot(data = nyuad.pos.samples, aes(x = PHD_Class, y = Value)) +
  geom_violin(aes(fill = AssayName)) +
  facet_grid(Plate ~ AssayName)
```
...
```{r eval=FALSE}
# the negative samples have higher Ct values meaning they have lower levels of viral load
```
...
Finally! Now we've gotten to the fun part!!!
...
sample.classes = samples %>% group_by(SampleName) %>% summarise(SampleName = unique(SampleName),PHD_Class = first(PHD_Class), NYUAD_Class = first(NYUAD_Class))
head(sample.classes)
...
nyuad.pos.sample.means <- nyuad_pos.clean.var %>% 
  group_by(SampleName) %>% 
  summarise(PHD_Class = first(PHD_Class), NYUAD_Class = NA, Plate = first(Plate), Value = mean(Ct.mean)) 

NYUAD_Class_cleaned <- merge(samples, nyuad.pos.sample.ids, by = "SampleName") %>% 
  arrange(ID) %>% select(SampleName, NYUAD_Class) %>% group_by(SampleName) %>% summarise(NYUAD_Class = first(NYUAD_Class)) 
nyuad.pos.sample.means$NYUAD_Class <- NYUAD_Class_cleaned$NYUAD_Class
...
```{r eval=FALSE}
# Able to capture smaller viral loads
```
...
# export data file
write.csv(data.report, file = "PHD_samples.viral_load.IsabellaMascio.csv")
```
