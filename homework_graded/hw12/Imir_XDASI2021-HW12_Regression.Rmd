...
involve an RNA extraction step followed by reverse transcription and qPCR
...
fluid.all = read.csv("/Users/ozanimir/Documents/NYU Files/Statistics in Biology/Week 15 Directory/data/Fluidigm_1691207281.csv", header = TRUE)
head(fluid.all)
```
...
negatives = data.frame(subset(fluid.all, fluid.all$SampleName == c("RX QC","NT QC"))) 
neg.ctls = negatives %>% 
...
standards = data.frame(subset(fluid.all, fluid.all$SampleName == c("CoV 50","CoV 500", "CoV 5000"))) 
std.ctls = standards %>% 
  select(ID, SampleName, AssayName, Value, Call, Plate, rConc) %>% 
  arrange(Plate)
...
ggplot(data = neg.ctls, aes(x = SampleName, y = Value, fill = SampleName)) +
  geom_violin() +
...
```{r eval=FALSE}
# The RX QC controls on plate 2 look like they did not work properly as a value of 999 should be observed in a negative control sample.
```

Do you think you proceed with the analysis using the data on one of the two plates? Explain.

```{r eval=FALSE}
# Yes, we should proceed with Plate 1 instead of both of the plates because of the lack of quality in Plate 2. 
```
...
ggplot(data = std.ctls, aes(x = SampleName, y = Value, fill = SampleName)) +
  geom_violin() +
  facet_grid(Plate ~ AssayName)
...
std.ctls.n1.n2 = std.ctls[!(std.ctls$AssayName == "RP"),]

# ggplot for just N1 and N2
ggplot(data = std.ctls.n1.n2, aes(x = SampleName, y = Value, fill = SampleName)) +
  geom_violin() +
  facet_grid(Plate ~ AssayName)
```
...
```{r eval=FALSE}
# All the standard controls for both plates, once RP assay results have been dropped, look great as they are below the standard threshold of 100 for proper standard controls. 
```

Based on the standard controls, do you think you can proceed with the analysis for at least one of the plates? Explain.

```{r eval=FALSE}
# I think we can proceed with these controls as part of the analysis because the standard results for both plates have both yielded proper levels of cycle numbers for a standardized and controlled PCR experiment. 
```
...
sample.names = subset(fluid.all, SampleName!="RX QC" & 
                        SampleName!="NT QC"& 
                        SampleName!="CoV 50"& 
                        SampleName!="CoV 500"& 
                        SampleName!="CoV 5000")
...
phd.calls = read.csv("/Users/ozanimir/Documents/NYU Files/Statistics in Biology/Week 15 Directory/data/PHD_results.csv", header = TRUE)
...
samples$NYUAD_Class = NA
samples$PHD_Class = samples$Class
samples = subset(samples, select = -c(Class))
head(samples)
```
...
ggplot(data = std.ctls.n1.n2, aes(x = log10(rConc),
                                  y = Value,
                                  color = AssayName),
       scale_x_continuous(label = function(x){return(round(10^x,1))})) +
  geom_point() +
...
```{r eval=FALSE}
# The between-plate variation seems to be a bit more different as Plate 2 seems to have higher degree of variation than Plate 1, but judging by the fact that all these samples are supposed to yield results with values lower than 100 cycles, we should be more than ready to move forward with using the standards for our experimentation. If this degree of variation in standards between Plate 1 and Plate is unacceptable (there might be other standards that need to be considered), then we should further investigate the relationship between the plates and the concentrations calculated.
```
...
standards.linear.model = lm(formula = Value ~ Plate * log10(rConc), data = std.ctls.n1.n2)
summary(standards.linear.model)
```
...
```{r eval=FALSE}
# The difference in slopes between different plates are equal to -2.8605 and according to the P value for the interaction term, there seems to be a significant interaction between the plates and the concentration. 
```
...
```{r}
# I prefer to create extra data frames for each, because it is easier to keep track, 
# and sometimes it comes in handy during later stages of the work I perform on selective groups.

# make 4 separate linear models for N1 and N2 on Plate 1 or Plate 2
n1.p1 = data.frame(subset(std.ctls.n1.n2, std.ctls.n1.n2$Plate == "Plate1" & std.ctls.n1.n2$AssayName == "N1"))
n2.p1 = data.frame(subset(std.ctls.n1.n2, std.ctls.n1.n2$Plate == "Plate1" & std.ctls.n1.n2$AssayName == "N2"))
n1.p2 = data.frame(subset(std.ctls.n1.n2, std.ctls.n1.n2$Plate == "Plate2" & std.ctls.n1.n2$AssayName == "N1"))
n2.p2 = data.frame(subset(std.ctls.n1.n2, std.ctls.n1.n2$Plate == "Plate2" & std.ctls.n1.n2$AssayName == "N2"))
...
```{r eval=FALSE}
# The R squared for the assays done in plate one seem to be higher and the slope differences seem much lower than in Plate 2, so I would prefer to use Plate 1. 
```
...
rp.all = data.frame(subset(samples, samples$AssayName == "RP" & samples$Plate == "Plate1"))
ggplot(data = rp.all, aes(x = PHD_Class, y = Value, fill = PHD_Class)) +
  geom_violin() +
...
```{r eval=FALSE}
# We expect to have RP assays to yield positive or negative PHD Class results when their respective PCR cycle values are distributed around the standard threshold but the mean values for invalid values seem to be skewing the results and making it impossible to view the positive and negative results. These sampling distributions don't look good. 
```
...
samples.rp = valid.samples %>% 
  group_by(SampleName, PHD_Class.x) %>% 
  filter(AssayName == "RP")

# violin plot of Ct values for valid assays, filtered for RP gene, by PHD diagnosis
ggplot(data = samples.rp, aes(x = PHD_Class.x, y = Value, fill = PHD_Class.x)) +
  geom_violin() +
  geom_boxplot(width = 0.1,
               fill = "white") +
  facet_grid(Plate ~ AssayName)
...
rp.p1 = subset(valid.samples, AssayName == "RP" & Plate == "Plate1") %>% 
  summarize(Value)
rp.p2 = subset(valid.samples, AssayName == "RP" & Plate == "Plate2") %>% 
  summarize(Value)
  
## QQ plots

# Plate 1
qqnorm(rp.p1$Value, pch = 1, frame = FALSE)
qqline(rp.p1$Value, col = "steelblue", lwd = 2)
# Plate 2
qqnorm(rp.p2$Value, pch = 1, frame = FALSE)
qqline(rp.p2$Value, col = "steelblue", lwd = 2)
...
```{r eval=FALSE}
# I don't think, unless the data is cleaned of the non-normal data points, can be used plate-by-plate in the further predictions. Either plate is not normally distributed and the t-test basically told us that there is a significant difference between the sampling distributions between the two samples, meaning they did not originate from the same sample population. This means the two plates yield different results, so they cannot individually be trusted as control measures. 
```
...
nyuad.pos.samples = merge(valid.samples, nyuad.pos.sample.ids, by = "SampleName") %>%
  group_by(ID,SampleName, PHD_Class.x.x,AssayName,Value,Plate,Call) %>% 
...
ggplot(data = nyuad.pos.samples, aes(x = AssayName, y = Value, fill = AssayName)) +
  geom_violin() +
  facet_grid(~Plate)
```
...
```{r}
# by PHD diagnosis
ggplot(data = nyuad.pos.samples, aes(x = PHD_Class.x.x, y = Value, fill = PHD_Class.x.x)) +
  geom_violin() +
  facet_grid(~Plate)
```

Do you notice anything interesting about the Ct values for the samples that PHD called "Negative" vs. "Positive"?

```{r eval=FALSE}
# Negative samples are clustered together and yield higher N1 and N2 Ct values whereas Positive samples tend to be evenly distributed across the 5-20 Ct value range. We would expect these two groups to be more distinct as one test positive and the other test negative for a test in terms of PCR cycle count. 
```
...
nyuad_pos.clean.var = nyuad.pos.samples %>%
  group_by(SampleName, AssayName, PHD_Class.x.x,Plate) %>%
  summarise(Ct.mean = mean(Value), Ct.sd = sd(Value), n = length(Value))
```
...
sample.clean.var$sem = ((sample.clean.var$Ct.sd)/(sqrt(sample.clean.var$n)))
nyuad_pos.clean.var$sem = ((nyuad_pos.clean.var$Ct.sd)/(sqrt(nyuad_pos.clean.var$n)))
```
...
plot(x = sample.clean.var$Ct.mean, y = sample.clean.var$sem)
abline(h = 0.5)
plot(x = nyuad_pos.clean.var$Ct.mean, y = nyuad_pos.clean.var$sem)
abline(h = 0.5)
...
sample.classes = samples %>% group_by(SampleName, PHD_Class, NYUAD_Class) %>% summarize(mean=mean(Value))
head(sample.classes)

# make a contingency table
contingency.table = with(sample.classes, table(PHD_Class, NYUAD_Class, dnn = c("PHD","NYUAD")))
contingency.table
```
...
std.ctls.p1 = rbind(n1.p1, n2.p1)

# linear model for rConc based on combined N1 and N2 Ct from "good" plate
lm.std.rConc = lm(formula = Value ~ log10(rConc), data = std.ctls.p1)
```
...
ggplot(data = std.ctls.p1, aes(x = log10(rConc), y = Value)) + 
  geom_point() +
...
nyuad.pos.sample.means.N1 = nyuad.pos.samples %>% 
  group_by(SampleName) %>% 
  filter(AssayName == c("N1") & Plate == "Plate1") %>% 
  summarize(Value1 = mean(Value))

nyuad.pos.sample.means.N2 = nyuad.pos.samples %>% 
  group_by(SampleName) %>% 
  filter(AssayName == c("N2") & Plate == "Plate1") %>% 
  summarize(Value2 = mean(Value))

nyuad.pos.sample.means = merge(nyuad.pos.sample.means.N1, nyuad.pos.sample.means.N2, by = "SampleName")
nyuad.pos.sample.means$Value = (nyuad.pos.sample.means$Value1+nyuad.pos.sample.means$Value2)/2
nyuad.pos.sample.means$rConc = NA

# use lm for prediction on controls to get CIs for them
pred.sample.rConc = predict.lm(lm.std.rConc,
                               formula = Value ~ SampleName, 
                               newdata  = data.frame(std.ctls.p1) , 
                               interval = "prediction")
...
df.pred.sample.rConc = cbind(std.ctls.p1,
...
p.fit = p.fit +
  geom_point(data = df.pred.sample.rConc, # the data frame containing predicted values
             mapping = aes(x = conc.fit, y = Value, color = valid.samples$PHD_Class), size=0.5)
...
```{r eval=FALSE}
# N1 assay predicts that with increasing viral copies, we would have decreasing Ct and the opposite is predicted for N2, so N2 would appear to be a more accurate predictor of viral copies. NYUAD was accurate in predicting N2 but not N1. 
```
...
```{r eval=FALSE}
# Assays that seem to yield more positive results (N2) tended to be yielding a better prediction. 
```
...
df.pred.sample.rConc$copies = df.pred.sample.rConc$rConc * df.pred.sample.rConc$conc.fit

# make a file for the final data report
data.report = left_join(sample.classes, 
                        select(df.pred.sample.rConc,SampleName,copies))

# export data file
write.csv(data.report, file = "PHD_samples.viral_load.Ozan_Berk_Imir.csv")
```
