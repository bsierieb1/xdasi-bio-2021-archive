...
author: "Rob Tibshirani [Omar Qassab]"
...
neg.ctls = fluid.all[fluid.all$SampleName == "RX QC" | 
                       fluid.all$SampleName == "NT QC", ]


# ============================================================================ #
# standards ("CoV 50", "CoV 500", "CoV 5000")
std.ctls = fluid.all[fluid.all$SampleName == "CoV 50" |
                       fluid.all$SampleName == "CoV 500" |
                       fluid.all$SampleName == "CoV 5000", ]
...
```{r eval=FALSE}
#  Plate 1 looks okay, but plate 2 definetly does not. Since in the negative control we should not detect anything, we expect the value to be 999. Plate 2 should be removed
```

Do you think you proceed with the analysis using the data on one of the two plates? Explain.

```{r eval=FALSE}
# I would use plate 1 for the analysis. There must be some error in the experimental setup of plate 2
```
...
std.ctls.n1.n2 = std.ctls[std.ctls$AssayName=="N1" |
                            std.ctls$AssayName=="N2", ]
...
```{r eval=FALSE}
# Yes the standard controls all look okay, It seems that the count values are all under 20, and that they decrease with increasing concentration of of the control plasmid
```

Based on the standard controls, do you think you can proceed with the analysis for at least one of the plates? Explain.

```{r eval=FALSE}
# Yes, plate 1 can be used to proceed with the analysis. This is because it fits all the criteria we are looking for. The count values for the CoV samples are all below 20 and decrease as you increase the concentration. The RP samples always show a read of 999, meaning they are not detected. 
#If we also factor in the negative control results, all negative controls show a count value of 999 for all samples, which is what we expect
```
...
# You should keep the sample ID and/or SampleName,
# AssayName, Value, Call, and Plate columns. You will also need to keep rConc
# for the Standards.

# filter fluid.all to keep just the samples and none of the controls
samples = fluid.all %>% 
  filter(!SampleName %in% c("RX QC", "NT QC", "CoV 50", "CoV 500", "CoV 5000")) %>% 
...
samples = samples %>% mutate("NYUAD_Class"=NA) %>% rename(PHD_Class=Class)
head(samples)
```
...
```{r eval=FALSE}
# Everything looks consistent between the N1 and N2 Assays, both decrease with increasing concentrations of COVID. Between the plates, the slopes look different though. It seems that for Plate 2, the slope is steeper, than for plate 1. This suggests that there might be an interaction between the plate and the value.
```
...
```{r eval=FALSE}
#summary(lm(Value ~ log10(rConc), data=std.ctls.n1.n2[std.ctls.n1.n2$Plate=="Plate1",]))
#summary(lm(Value ~ log10(rConc), data=std.ctls.n1.n2[std.ctls.n1.n2$Plate=="Plate2",]))

# The slope for the standards on Plate 1 is less steep than that of Plate 2. If you were to isolate the data and calculate the slopes the slopes would be -2.68 and -.5.46 for plate 1 and plate 2 respectively. If you compile the data and test for an interaction between the value at a given concentration and the plate you would notice a significant interaction term between the plate and the concentration.
...
lm.n1.p1 = lm(Value ~ log10(rConc), data=std.ctls.n1.n2[std.ctls.n1.n2$Plate=="Plate1" &
                                                          std.ctls.n1.n2$AssayName=="N1",])
lm.n2.p1 = lm(Value ~ log10(rConc), data=std.ctls.n1.n2[std.ctls.n1.n2$Plate=="Plate1" &
                                                          std.ctls.n1.n2$AssayName=="N2",])
lm.n1.p2 = lm(Value ~ log10(rConc), data=std.ctls.n1.n2[std.ctls.n1.n2$Plate=="Plate2" &
                                                          std.ctls.n1.n2$AssayName=="N1",])
lm.n2.p2 = lm(Value ~ log10(rConc), data=std.ctls.n1.n2[std.ctls.n1.n2$Plate=="Plate2" &
                                                          std.ctls.n1.n2$AssayName=="N2",])
...
```{r eval=FALSE}
# You would use the first plate. The slope is closer to the desired range of -3.1 to -.3.6. The Efficiency for plate 1 is also better than that of plate 2, which is closer to the desired value of 2.
...
```{r eval=FALSE}
# In the clinical samples we expect to see a positive RP value. Opposite to what we see in our control(we would expect it to be negative)
```
...
samples.rp = valid.samples[valid.samples$AssayName=="RP", ]

# violin plot of Ct values for valid assays, filtered for RP gene, by PHD diagnosis
ggplot(data= filter(samples.rp),
       mapping=aes(x=PHD_Class,
                   y= Value,
                   fill=PHD_Class))+
  geom_violin()+
...
rp.p1 = samples.rp[samples.rp$Plate=="Plate1",]
rp.p2 = samples.rp[samples.rp$Plate=="Plate2",]
...
```{r eval=FALSE}
# Based on the data above, it looks like the samples come from the same distribution. So it is probably okay to use the standard from only one of the plates.
```
...
```{r eval=FALSE}
# Negative Ct values are on average higher, and less spread than that of the positive Ct values
```
...
sample.clean.var = sample.clean.var %>%  mutate("SEM" = Ct.sd/sqrt(n))
nyuad_pos.clean.var = nyuad_pos.clean.var %>%  mutate("SEM" = Ct.sd/sqrt(n))
```
...
sample.classes = samples %>% group_by(SampleName) %>%  summarize(SampleName, PHD_Class, NYUAD_Class)
head(sample.classes)
...
```{r eval=FALSE}
# 96 samples were inconclusive, and 48 Positives by PHD were deemed negative by NYUAD
```
...
std.ctls.p1 = std.ctls.n1.n2[std.ctls.n1.n2$Plate=="Plate1",]

# linear model for rConc based on combined N1 and N2 Ct from "good" plate
lm.std.rConc = lm(Value ~ log10(rConc), data=std.ctls.p1)
```
...
nyuad.pos.sample.means = nyuad.pos.samples %>% group_by(SampleName) %>% 
...
```{r eval=FALSE}
# The Fluidigm system is pretty sensitive
```
...
```{r eval=FALSE}
# It looks like the negative data points are over-represented in viral copies with concentration at around 1 copy/uL
```
...
df.pred.sample.rConc$Copies_per_ul = signif(10^(df.pred.sample.rConc$conc.fit),digits=2) 

# make a file for the final data report
data.report = left_join( sample.classes , 
                        select( df.pred.sample.rConc , Copies_per_ul ))

data.report.unique=unique(data.report)

# export data file
write.csv(data.report.unique, file = "PHD_samples.viral_load.omar_qassab.csv" )
```
