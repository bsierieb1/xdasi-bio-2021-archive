...
author: "Rob Tibshirani [Dylan Fitzmaurice]"
...
setwd("~/R/Biostats/Week_14")
fluid.all <- read.csv("~/R/Biostats/Week_14/Fluidigm_1691207281.csv")
head(fluid.all)

#remove cols that can be ignored 
fluid.all <- within(fluid.all, rm(Quality))  
fluid.all <- within(fluid.all, rm(Threshold))
head(fluid.all)
...
neg.ctls = fluid.all %>%  filter(SampleName == "RX QC" | SampleName == "NT QC") 
# ============================================================================ #
# standards ("CoV 50", "CoV 500", "CoV 5000")
std.ctls = fluid.all %>%  filter(SampleName == "CoV 50" | SampleName == "CoV 500" | SampleName == "CoV 5000") 
...
ggplot(data = neg.ctls, mapping = aes(x = SampleName, y=Value)) + 
  geom_boxplot() + facet_grid(Plate ~ AssayName)

ggplot(data = neg.ctls, mapping = aes(x = SampleName, y=Value)) + 
  geom_violin() + facet_grid(Plate ~ AssayName)
...
```{r eval=FALSE}
# Your answer here
#Pretty good, almost all of them nothing was detected, but there were a few with a 
#low value around 15
```
...
```{r eval=FALSE}
# your answer here
#I would use plate 1 as that one was perfect meaning nothing was detected for
...
ggplot(data = std.ctls, mapping = aes(x = SampleName, y=Value)) + 
  geom_boxplot() + facet_grid(Plate ~ AssayName)
...
std.ctls.n1.n2 = std.ctls %>%  filter(AssayName == "N1" | AssayName == "N2")

# ggplot for just N1 and N2
ggplot(data = std.ctls.n1.n2, mapping = aes(x = SampleName, y=Value)) + 
  geom_boxplot() + facet_grid(Plate ~ AssayName)
...
```{r eval=FALSE}
# Your answer here
#Not really because they are not consistent across plates. 
```
...
```{r eval=FALSE}
# your answer here
#Yes, because they are at least self consistent between N1 and N2 for a given plate.
```
...
samples = fluid.all %>% filter(substr(SampleName, 1, 1) == "9" | substr(SampleName, 1, 1) == "0")

# Import PHD results from .csv file
phd.calls = read.csv("~/R/Biostats/Week_14/PHD_results.csv")
...
samples = samples %>% add_column(NYUAD_Class = NA) 
head(samples)

colnames(samples)[12] <- "PHD_Class"
head(samples)
...
ggplot(data = std.ctls.n1.n2, mapping = aes(x = log10(rConc), y=Value)) + 
  geom_point() +
...
```{r eval=FALSE}
# Your answer here
#Doesn't look reproducible as each plate's regression line has a different slope. 
```
...
summary(lm(Value ~ log10(rConc) * Plate, std.ctls.n1.n2 %>%  filter(AssayName == "N1")))

summary(lm(Value ~ log10(rConc) * Plate, std.ctls.n1.n2 %>%  filter(AssayName == "N2")))

summary(lm(Value ~ log10(rConc) * Plate, std.ctls.n1.n2)) #combined
...
```{r eval=FALSE}
# Your answer here
#The interaction term is significant. Though I don't think what I showed above tells the slope of each plate, 
#though the previous graph shows plate 2 has a greater negative slope than plate 1.
```
...
lm.n1.p1 = lm(Value ~ log10(rConc), std.ctls.n1.n2 %>% filter(AssayName == "N1") %>% filter(Plate == "Plate1")) 
lm.n2.p1 = lm(Value ~ log10(rConc), std.ctls.n1.n2 %>% filter(AssayName == "N2") %>% filter(Plate == "Plate1")) 
lm.n1.p2 = lm(Value ~ log10(rConc), std.ctls.n1.n2 %>% filter(AssayName == "N1") %>% filter(Plate == "Plate2")) 
lm.n2.p2 = lm(Value ~ log10(rConc), std.ctls.n1.n2 %>% filter(AssayName == "N2") %>% filter(Plate == "Plate2")) 
...
```{r eval=FALSE}
# Your answer here
#Plate 1, the slope is closer to the -3.1 and -3.6 range for 90-100% efficiency, and has the highest R^2 values. 
```
...
ggplot(data = samples %>% filter(AssayName == "RP"), mapping = aes(x = AssayName, y=Value, fill = PHD_Class)) + 
  geom_violin()
...
```{r eval=FALSE}
# Your answer here
#Not really because RP should be in all clinical samples if the swab was ok, so the 
#data suggest there are a lot of cases where the swab was not ok. 
```
...
ggplot(data = samples.rp,  mapping = aes(x = AssayName, y=Value, fill = PHD_Class)) + 
  geom_violin() + facet_wrap(~Plate)
...
rp.p1 = samples.rp %>% filter(Plate == "Plate1")
rp.p2 = samples.rp %>% filter(Plate == "Plate2")
...
shapiro.test(rp.p1$Value) #Not normal :(
shapiro.test(rp.p2$Value) #Not normal :( 
...
```{r eval=FALSE}
# Your answer here
#I'm confused here because wouldn't they both not be normally distributed, so you shouldn't use either? 
#Though the plate 1 is more normal than plate 2.
```
...
ggplot(data = nyuad.pos.samples,  mapping = aes(x = AssayName, y=Value, fill = AssayName)) + 
  geom_violin() + facet_wrap(~Plate)
```
...
```{r}
# by PHD diagnosis
ggplot(data = nyuad.pos.samples,  mapping = aes(x = AssayName, y=Value, fill = PHD_Class)) + 
  geom_violin() + facet_grid(Plate ~ AssayName)
```
...
```{r eval=FALSE}
# your answer here
#Yes, the count values are higher and have less variation for the negative samples
#than the positive. 
```
...
sample.clean.var = sample.clean.var %>% add_column(Ct.sem = sample.clean.var$Ct.sd/sqrt(sample.clean.var$n)) 
head(sample.clean.var)

nyuad_pos.clean.var = nyuad_pos.clean.var %>% add_column(Ct.sem = nyuad_pos.clean.var$Ct.sd/sqrt(nyuad_pos.clean.var$n))
...
p1 = ggplot(data = sample.clean.var,  mapping = aes(x = seq(SampleName), y=Ct.sem)) + 
  geom_point() + 
  geom_hline(yintercept=0.5)

# Cleaned data
p2 = ggplot(data = nyuad_pos.clean.var,  mapping = aes(x = seq(SampleName), y=Ct.sem)) + 
  geom_point() + 
  geom_hline(yintercept=0.5)
...
sample.classes = samples %>% group_by(SampleName) %>% summarize()

sample.classes$PHD_Class <- rep(NA, nrow(sample.classes))
sample.classes$NYUAD_Class <- rep(NA, nrow(sample.classes))

for(i in 1:nrow(samples))
{
for(j in 1:nrow(sample.classes))
{
if (samples[i,5] == sample.classes[j,1])
{
sample.classes[j,2] <- samples[i,12]
sample.classes[j,3] <- samples[i,13]
}
}
}
...
std.ctls.p1 = std.ctls %>% filter(Plate == "Plate1")  %>% filter(AssayName %in% c("N1","N2"))

# linear model for rConc based on combined N1 and N2 Ct from "good" plate
lm.std.rConc = lm(Value ~ log10(rConc), std.ctls.p1)
...
nyuad.pos.sample.means = nyuad.pos.samples %>% group_by(SampleName) %>% summarise(Value = mean(Value), PHD_Class = PHD_Class)
...
df.pred.sample.rConc = cbind(nyuad.pos.sample.means , df.pred.sample.rConc ,
...
```{r eval=FALSE}
# Your answer here
#Its more sensitve then the clinical test. 
```
...
```{r eval=FALSE}
# Your answer here
#The virial load estimated by NYUAD show the PHD clinical samples labeled as negative have lower viral copies then the #the positively labeled ones, so the intial test was not sensitive enough to catch these positive samples. 
```
...
df.pred.sample.rConc$Copies_per_ul = signif(10^df.pred.sample.rConc$conc.fit)

# make a file for the final data report
data.report = left_join(sample.classes, 
                        select(df.pred.sample.rConc , c("Copies_per_ul")))

# export data file
write.csv(data.report, "PHD_samples.viral_load.DylanFitzmaurice.csv")
```
