...
fluid.all <- fluid.all %>% select(ID, SampleName, AssayName, Value, Call, Plate, rConc)
...
std.ctls = fluid.all %>% filter(SampleName %in% c("CoV 50",
                                                  "CoV 500",
                                                  "CoV 5000"))
...
```{r eval=FALSE}
# The negative controls on plate one look fine. The negative controls
# on plate two did not all show values of 999, indicating that there
# were false positives detected.
```

Do you think you proceed with the analysis using the data on one of the two plates? Explain.

```{r eval=FALSE}
# Yes, I would proceed with the analysis with the first plate.
```
...
```{r eval=FALSE}
# The standard controls all look okay for plate 1. The plate 2 
# RP assays show some amplification when it shouldn't. I would proceed
# with just plate 2.
```
...
```{r eval=FALSE}
# Yes. I would proceed with the analysis for plate 1.
```
...
samples = fluid.all %>% filter(!(SampleName %in% c("CoV 50",
                                                  "CoV 500",
                                                  "CoV 5000",
                                                  "RX QC",
                                                  "NT QC")))
...
samples = samples %>% mutate("NYUAD_Class" = NA)
...
```{r eval=FALSE}
# Within plate 1, there is a distinct difference between N1 and N2,
# with N2 showing consistently higher cycles than N1 for all concentrations. This
# trend is true for plate 2, but there is a larger error for both regression
# lines and their slopes are steeper.
```
...
```{r eval=FALSE}
# The difference in the slopes is 7.89, and is highly significant.
```
...
```{r eval=FALSE}
# For clinical samples, we would expect to see values much lower than
# 999 for both positive and negative detection.
# We see some values equal to 999 in plate 2, which is not expected.
```
...
```{r eval=FALSE}
# Yes, it seems okay to use the standards from one plate to quantify the
# viral load of all samples, since the two plates are similar enough (> 0.05).
```
...
```{r eval=FALSE}
# The Ct values between true positive and true negative overlap at around 15. Choosing
# all Ct values lower than 15 to be positive will remove all false positives but you will
# have to include some false negatives, which is maybe a worse idea.
```
...
sample.clean.var$Ct.sem = sample.clean.var$Ct.sd / sqrt(sample.clean.var$n)
nyuad_pos.clean.var$Ct.sem = nyuad_pos.clean.var$Ct.sd / sqrt(nyuad_pos.clean.var$n)
```
...
sample.classes = samples %>% group_by(SampleName) %>%
  summarize(PHD_Class = PHD_Class, NYUAD_Class = NYUAD_Class) %>% 
  unique()
head(sample.classes)
...
```{r eval=FALSE}
# 4 + 2 = 6 samples failed to detect
```
...
```{r eval=FALSE}
# The NYUAD Fluidigm system is fairly sensitive. It can detect a positive
# result at very low viral copies per microliter.
```
...
```{r eval=FALSE}
# Most PHD clinical samples marked as 'Negative' have a viral load less than
# about 10 viral copies per microliter. Everything predicted to have above 10
# viral copies per microliter is predicted as 'Positive'
```
...
data.report = left_join(sample.classes , 
                        select( df.pred.sample.rConc ,
                                SampleName,
                                PHD_Class,
                                Copies_per_ul))

# export data file
write.csv(df.pred.sample.rConc, "PHD_samples.viral_load.graeme_vissers.csv")
```
