...
fluid.all = read.csv("data/Fluidigm_1691207281.csv")
head(fluid.all)
```
...
neg.ctls = fluid.all %>% 
  filter(SampleName == "RX QC" | SampleName == "NT QC") %>% 
...
std.ctls = fluid.all %>% 
  filter(SampleName == "CoV 50" | SampleName == "CoV 500" | SampleName == "CoV 5000") %>% 
  select(-X, -Sample, -Assay, -Type, -Quality, -Threshold)
...
```{r eval=FALSE}
# No, negative controls in plate 2 do not look right. The Ct values are the number of PCR cycles at which a signal was detected so for negative controls these should be 999 which means that no signal was detected. Some of the negative controls in Plate 2 have values between 0-20, which means that wells in this plate may have been contaminated by positive samples.
```

Do you think you proceed with the analysis using the data on one of the two plates? Explain.

```{r eval=FALSE}
# Yes only proceed with plate 1 because we cannot be sure how the other wells of plate 2 are affected by the contamination. This could result in false positive signals for samples that were originally negative.
```
...
```{r eval=FALSE}
# No, some RP in plate 2 have a positive detection value unlike the expected 999. Also, some N2 in plate 2 have values slightly greater than 20.
```

Based on the standard controls, do you think you can proceed with the analysis for at least one of the plates? Explain.

```{r eval=FALSE}
# Yes, analysis can be continued with plate 1 since all the controls in this plate have the expected range and pattern of values.
```
...
```{r eval=FALSE}
# The regression lines for plate 1 fit the data points well and N1 and N2 assays also look very similar. For plate 2, although N1 and N2 look similar, the regression lines don't fit the data points as well as those in plate 1. Plate 2 standards also have a greater range of values as compared to plate 1.
```
...
lm_std_combined = lm(formula = Value ~ log10(rConc) * Plate,
                            data = std.ctls.n1.n2)
summary(lm_std_combined)

lm_std_n1 = lm(formula = Value ~ log10(rConc) * Plate,
                            data = filter(std.ctls.n1.n2, AssayName == "N1"))
summary(lm_std_n1)

lm_std_n2 = lm(formula = Value ~ log10(rConc) * Plate,
...
```{r eval=FALSE}
# The slope for standards on plate 2 is 2.7787 LESS than that for plate 1 (combined N1/N2 analysis). The interaction term is significant with a p-value less than 2e-16.
```
...
```{r eval=FALSE}
# We should use the series from plate 1 because the slope for this plate is closer to the ideal range of -3.1 and -3.6 as compared to the slope for plate 2 and plate 1 has higher amplification efficiency.
```
...
```{r eval=FALSE}
# Since the samples are all human samples, they are all expected to be positive for the human RP gene if there was no problem with the samples. Plate 1 looks okay because all values are a positive detection value. However, sample 2 has some 999 values which means the RP detection failed in these samples so something is wrong.
```
...
```{r eval=FALSE}
# 171 samples are left that are of good quality.
```
...
```{r eval=FALSE}
# Although the Shapiro-Wilk test shows that both of these data are not normally distributed (p-value < 0.05), Wilcoxon Rank Sum test shows that they come from similar distributions (p-value > 0.05). So, it seems okay to use the standards from only one of the plates to quantify the viral load in all the samples.
```
...
```{r eval=FALSE}
# Yes, the samples that are "Negative" in the PHD tests are detected as positive at a later PCR cycle in the NYUAD assay system as compared to most of the positive ones. This means that the "Negative" samples had a viral load that was too small for the PHD test to detect in 32 cycles but the NYUAD assay system was sensitive enough to detect it.
```
...
sample.clean.var = sample.clean.var %>% 
  mutate(Ct.SEM = Ct.sd / sqrt(n))
nyuad_pos.clean.var = nyuad_pos.clean.var %>% 
  mutate(Ct.SEM = Ct.sd / sqrt(n))
```
...
sample.classes = samples %>% 
  group_by(SampleName) %>% 
  summarize(PHD_Class = unique(PHD_Class), NYUAD_Class = unique(NYUAD_Class))
head(sample.classes)
...
```{r eval=FALSE}
# Out of the samples that were called "Positive" by PHD, NYUAD called 2 of these "Negative" and 4 other were "Inconclusive".
```

How many samples that were called "Negative" by PHD did NYUAD detect as positive?

```{r eval=FALSE}
# Out of the samples that were called "Negative" by PHD, NYUAD detected 17 of these as positive.
```
...
lm.std.rConc = lm(formula = log10(rConc) ~ Value,
                  data = std.ctls.p1)
```
...
nyuad.pos.sample.means = nyuad.pos.samples %>% 
  group_by(SampleName) %>% 
  summarize(PHD_Class = unique(PHD_Class),
            Plate = unique(Plate),
            Value = mean(Value))
...
```{r eval=FALSE}
# The NYUAD Fluidigm system is very sensitive as it can even detect viral loads as small as 1 viral copy/ul.
```
...
```{r eval=FALSE}
# The NYUAD Fluidigm system is more sensitive that the PHD assay as it can even detect small viral loads (<100 copies/ul) that were concluded as Negative by the PHD assay. The general trend is that the PHD assay can detect viral loads with >100 copies/ul accurately, but is imprecise with smaller loads.
```
...
data.report = left_join( sample.classes , 
                        select( df.pred.sample.rConc , c(SampleName, Copies_per_ul) ))

# export data file
write.csv(data.report,
          "PHD_samples.viral_load.Shaili_Regmi.csv")
```
