...
fluid.all = read.csv("/Users/Sofia/Desktop/R/R week 12/hw 12/data/Fluidigm_1691207281.csv")
head(fluid.all)
```
...
neg.ctls = fluid.all %>% filter(SampleName == "RX QC"| SampleName == "NT QC") %>% select(ID, SampleName, AssayName, Value, Call, Plate, rConc) %>% arrange(ID)

head(neg.ctls)
# ============================================================================ #
# standards ("CoV 50", "CoV 500", "CoV 5000")
std.ctls = fluid.all %>% filter(SampleName == "CoV 50" | SampleName == "CoV 500" | SampleName == "CoV 5000") %>% select(ID, SampleName, AssayName, Value, Call, Plate, rConc)
...
ggplot(neg.ctls, mapping=aes(x=SampleName,
                             y=Value))+
  geom_boxplot()+
...
```{r eval=FALSE}
# not all the negative controls look ok, there is an issue in plate 2. Ct value of negative controls should be 999, but here in plate 2 some negative controls have Ct value not 999 (there is amplification in negative control maybe due to contamination)
```

Do you think you proceed with the analysis using the data on one of the two plates? Explain.

```{r eval=FALSE}
# I proceed with the analysis using data just from plate 1, because is the only plate were I can rely on "real" negative controls
```
...
ggplot(std.ctls, mapping=aes(x=SampleName,
                             y=Value))+
  geom_boxplot()+
  facet_grid(Plate ~ AssayName)
...
ggplot(std.ctls.n1.n2, mapping=aes(x=SampleName,
                             y=Value))+
  geom_boxplot()+
  facet_grid(Plate ~ AssayName)
...
```{r eval=FALSE}
#not all the standard controls look ok. the standard control in plate 2 for CoV50 as a Ct value more than 20.0
```

Based on the standard controls, do you think you can proceed with the analysis for at least one of the plates? Explain.

```{r eval=FALSE}
# yes we can proceed with the analysis for plate 1. All the standards have Ct values lower than 20.0
# also Ct values decrease with increasing concentration of control plasmids
```
...
samples = fluid.all %>% filter(SampleName != "RX QC" & SampleName != "NT QC" & SampleName != "CoV 50" & SampleName != "CoV 500" & SampleName != "CoV 5000") %>% select(ID, SampleName, AssayName, Value, Call, Plate, rConc)

head(samples)

# Import PHD results from .csv file
phd.calls = read.csv("/Users/Sofia/Desktop/R/R week 12/hw 12/data/PHD_results.csv")
...
samples = samples %>% 
  mutate(NYUAD_Class = rep(NA, nrow(samples))) %>% 
  rename(PHD_Class = Class)
...
ggplot(std.ctls.n1.n2, mapping=aes(x=log10(rConc),
                                   y=Value,
                               color=AssayName)) +
  geom_point() +
  facet_wrap(~Plate) +
...
```{r eval=FALSE}
# for plate 1 the data points are well fitted into the standard regression line and everything look consistent between the N1 and N2 assays. For plate 2 the data points are not well fitted into the standard regression line, but everything look consistent between the N1 and N2 assays. Also plate 2 has a bigger range of values than plate 1.
```
...
lm_n1 = lm(formula = Value ~ log10(rConc) * Plate,
                            data = filter(std.ctls.n1.n2, AssayName == "N1"))
summary(lm_n1)

lm_n2 = lm(formula = Value ~ log10(rConc) * Plate,
                            data = filter(std.ctls.n1.n2, AssayName == "N2"))
summary(lm_n2)

lm_both = lm(formula = Value ~ log10(rConc) * Plate,
                            data = std.ctls.n1.n2)
summary(lm_both)
...
```{r eval=FALSE}
#the slope for the standards on Plate 2 is -2.7787 vs. Plate 1. The interaction term is significant(p-value <2e-16)
```
...
```{r eval=FALSE}
# I would choose plate's 1 dilution series because it has a better amplification efficiency. Also the slope for plate 1 is closer to the ideal -3.1 and -3.6. 
```
...
```{r eval=FALSE}
# Being human samples I expect to see that they are all positive for human RP gene. In plate 2 some values are 999 so the RP detection failed, there was something wrong in these samples.
```
...
```{r eval=FALSE}
# Wilcoxon Rank Sum Test has a p-value of 0.7 so shows the data come from similar distributions, therefore it seems ok to use the standards from only one of the plates to quantify the viral load in all the samples. (Shapiro-Wilk test shows these data are not normally distributed)
```
...
```{r eval=FALSE}
# "Negative" samples in the PHD are positive in the NYUAD assay in later cycles, compared to the "positive" ones. therefore the NYUAD assay was more sensitive to detect even the too small viral loads for the PHD test.
```
...
```{r eval=FALSE}
# Of the PHD "Positive" samples there were 2 samples that were called "Negative" by NYUAD and 4 other were called "Inconclusive" by NYUAD  
```

How many samples that were called "Negative" by PHD did NYUAD detect as positive?

```{r eval=FALSE}
# Of the PHD "Negative" samples there were 17 samples that were called "Positive" by NYUAD 
```
...
std.ctls.p1 = filter(std.ctls.n1.n2, Plate == "Plate1")
...
ggplot(std.ctls.p1, aes(x=log10(rConc), 
                        y=Value)) +
  
geom_point()+
geom_smooth(method="lm")+
scale_x_continuous(label=function(x){return(round(10^x,1))})+
xlab("rConc") + ylab("Value")
...
nyuad.pos.sample.means = nyuad.pos.samples %>% group_by(SampleName) %>% summarize(PHD_Class = unique(PHD_Class),Plate = unique(Plate),Value = mean(Value))
...
```{r eval=FALSE}
# NYUAD Fluidigm system is very sensitive, it can detect even 1 viral copy/ul
```
...
```{r eval=FALSE}
# the NYUAD Fluidigm system can detect even small viral loads that the PHD assay detected as "Negative". The PHD assay can accurately detect viral loads of more than 100 viral copies/ul but its very imprecise with small viral loads.
```
...
write.csv(data.report, "/Users/Sofia/Desktop/R/R week 12/hw 12//PHD_samples.viral_load.SOFIASESSA.csv", row.names=FALSE)
