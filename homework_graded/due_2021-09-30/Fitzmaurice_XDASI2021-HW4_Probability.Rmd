...
# IQR and boxplot
IQR(smp) # or summary(smp)[5]-summary(smp)[2] 
...
SEM = sd(smp)/sqrt(length(smp)) #SEM
SEM

c(mean(smp)-1.96*SEM,mean(smp)+1.96*SEM) #CI

# Option 2: using gmodels package

# Explanation
# The SEM (standard error of the mean) represents the precision which we are able to 
# estimate the true population mean. 
# The CI or 95% confidence interval captures the true population mean 95% of the time 
# (i.e. in 1 out of 20)
```
...
# histograms
# Option 1: base R
hist(smp, prob="true", col="blanchedalmond", 
     breaks=40,
     xlim=c(0.6,1.4))
abline(v=mean(smp), col="red") # mean
lines(density(smp),col="cyan", lwd=2)  # smoothened curve
#curve(dnorm(x), add=TRUE,              # ideal curve, not sure why not adding correctly? 
#      col="darkblue", lwd=2)
legend(0.7, 3, legend=c("Mean"),
...
```{r}
# your answer here
1 - (pnorm(1.07, mean=smp_mean, sd=smp_sd, lower.tail=FALSE) + pnorm(0.93, mean=smp_mean, sd=smp_sd, lower.tail=TRUE))
...
# your answer here
#<1 std
pnorm(smp_mean+smp_sd, mean=smp_mean, sd=smp_sd)

#>1 std
1-pnorm(smp_mean+smp_sd, mean=smp_mean, sd=smp_sd)
...
qnorm(c(0.1,0.9), mean=smp_mean, sd=smp_sd)
...
```{r}
# your answer here
#Independence: P(A) + P(B) = P(A)*P(B)

# A = HPV, B = Chlamydia
# No, since  P(A) + P(B) != P(A)*P(B)
```
...
#### a. What is the probability that any single allele chosen at random from the population is either A2 or A5? Write out the probability statement as a comment below and then compute the answer.
