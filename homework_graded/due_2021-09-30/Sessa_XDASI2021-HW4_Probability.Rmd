...
sample_size=1000
sample_sem=sd(sample)/sqrt(sample_size)
sample_sem

sample_ci=c(mean(sample)-1.96*sample_sem, mean(sample)+1.96*sample_sem)
sample_ci
...
# Explanation
#sample_sem=0.003156192
#sample_ci= lower is 0.9944535 and upper is 1.0068258
# SEM is computed as the standard deviation of the sample divided by the square root of the sample size, it gives the position of the estimate, how far the mean of the data is likely to be from the true population mean.The 95% CI is a range of values surrounding a sample estimate in which the true population parameter is expected to be found 95% of the time, to compute 95% CI lower limits we do (mean-1.96*SEM), upper limit we do (mean+1.96*SEM).
```
...
ggplot(data=sample.df, 
  ...
  geom_histogram(color="blue",
                 fill="lightblue") +
geom_vline(aes(xintercept=mean(sample)), color="red")
...
```{r}
# your answer here
pnorm(1.07, mean_sample, sd_sample)-pnorm(0.93, mean_sample, sd_sample)
...
```{r}
# your answer here
pnorm(mean_sample-sd_sample, mean_sample, sd_sample, lower.tail=TRUE)+pnorm(mean_sample+sd_sample, mean_sample, sd_sample, lower.tail=FALSE)
```
...
```{r}
# your answer here
qnorm(0.1, mean_sample, sd_sample)
qnorm(0.9, mean_sample, sd_sample)

#0.8727311 for the 10% quantile
#1.128548 for 90% quantile
...
p.hpv.or.chl=p.chl.tot+p.hpv.tot-p.hpv.chl
...
```{r}
# your answer here
# The occurrence of these infections is not independent, because the occurrence of one infection does affect the probability of occurrence of the other infection. 
```
...
Pr$Caa.Faa_Maa = 1   #0.25*4
Pr$Caa.FAa_Maa = 0.5 #0.25*2
Pr$Caa.FAA_Maa = 0
Pr$Caa.Faa_MAa = 0.5  #0.25*2
Pr$Caa.FAa_MAa = 0.25
Pr$Caa.FAA_MAa = 0
