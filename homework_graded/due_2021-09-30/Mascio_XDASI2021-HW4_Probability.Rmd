...
# sample
n = 1000
ctldiameter_sample <- rnorm(n, mean = 1, sd = 0.1)
# mean and sd
mean(ctldiameter_sample)
sd(ctldiameter_sample)
# IQR and boxplot
summary(ctldiameter_sample)

boxplot(ctldiameter_sample)
```
...
ci(ctldiameter_sample)[4] #SEM
ci(ctldiameter_sample)[2] #lower limit
ci(ctldiameter_sample)[3] #upper limit
# Explanation
# The confidence interval is computed by calculating 1.96 standard deviations away from the mean in both directions. It is saying that the true value of the population parameter mean is within these limits. The 95% means that 1 out of 20 CIs will contain the true value of the population parameter.
# The SEM is computed but calculating the standard deviation divided by the square root of the sample size. It is a measure for how variable the mean will be from sample to sample. It is a measure of precision.
```
...
# Option 1: base R
hist(ctldiameter_sample)
abline(v = mean(ctldiameter_sample), col = "red")
abline(v = median(ctldiameter_sample), col = "blue", lty = 2)

#  Option 2: ggplot

# qqplot
qqnorm(ctldiameter_sample)
qqline(ctldiameter_sample)
```
...
```{r}
pnorm(1.01, mean = 1, sd = 0.1, lower.tail = FALSE)
pnorm(1-0.01, mean = 1, sd = 0.1)
# 0.460 is above or below 1 standard deviation (double this number is you want how much fall outside 1 standard deviation in both directions)

pnorm(1.1, mean = 1, sd = 0.1) - pnorm(0.9, mean = 1, sd = 0.1)
...
```{r}
qnorm(0.1, mean = 1, sd = 0.1)
qnorm(0.9, mean = 1, sd = 0.1)
...
```{r}
# No, multiplying the total individual probabilities together doesn't equal the probability of both. So infection of one is likely linked to infection of the other. (probability of both is higher than if we assume independence)
p.chl.tot * p.hpv.tot
p.hpv.chl
```
...
```{r}
# prob of carrying two A1 alleles: P(A1) * P(A1)
A1A1 <- A1*A1
A1A1
# prob of NOT having two A1 alleles: 1 - P(A1 and A1)
notA1A1 <- 1 - A1A1
notA1A1
# P(2 individuals are NOT A1,A1): P(not A1 and A1)*P(not A1 and A1)
notA1A1*notA1A1
```
...
```{r}
# prob of an allele being A3 or A4: P(A3) + P(A4)
A3A4 <- A3 + A4
A3A4
# prob of an allele not being A3 or A4: 1 - P(A3 or A4)
notA3A4 <- 1 - A3A4
notA3A4
# P(neither of 2 alleles is A3 or A4): P(not A3 or A4) * P(not A3 or A4)
noA3orA4 <- notA3A4 * notA3A4
noA3orA4
# P(3 people have no A3 or A4 alleles): P(neither alleles) * P(neither alleles)
noA3orA4 * noA3orA4 * noA3orA4
```
...
```{r collapse=TRUE}
# conditional probability that Aa x Aa -> aa: P(aa | PrAaAa) = P(aa and AaAa)/P(AaAa)
Pr$Caa.FAa_MAa = 0.25
Pr$Caa.FAa_MAa
```
