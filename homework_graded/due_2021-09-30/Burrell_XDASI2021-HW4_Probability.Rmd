...
SEM = sd(sample) / sqrt(1000)
SEM

CI = c(mean(sample) - 1.96 * SEM, mean(sample) + 1.96 * SEM)
CI
...
# Explanation
# the SEM is the standard deviation of the sample mean sampling distribution, and it gives us an idea of how much the sample mean is expected to differ from sample to sample. It is calculated by using the sample standard deviation as an approximation for the population parameter σ and dividing it by square root n. The 95% CI is a more formal measure of the variance of the sample mean across samples and describes a specific range in which we would expect the true population parameter to fall 95% of the time. It is calculated from the sampling distribution by determining the interval within which 95% of the sampling means can be found. 
```
...
```{r}
pnorm(1.07, mean, sd) - pnorm(0.93, mean, sd)
...
```{r}
c(pnorm(mean-sd, mean, sd) + pnorm(mean+sd, mean, sd, lower.tail = FALSE),
...
```{r}
c(qnorm(0.1, mean, sd), qnorm(0.9, mean, sd))
```
...
```{r}
c(p.chl.tot * p.hpv.tot, p.hpv.chl)

# The occurrence of these infections is NOT independent since the product of the probability of the two infections does not equal the probability of simultaneous infection. In other words, the probability of getting hpv and chl is greater than would be expected if the two events were truly independent. 
```
...
Pr$Faa_Maa = Pr$aa^2
Pr$FAa_Maa = 2*Pr$Aa*Pr$aa
Pr$FAA_Maa = Pr$AA*Pr$aa
Pr$Faa_MAa = Pr$aa*2*Pr$Aa
Pr$FAa_MAa = (2*Pr$Aa)^2
Pr$FAA_MAa = Pr$AA*2*Pr$Aa
Pr$Faa_MAA = Pr$aa*Pr$AA
Pr$FAa_MAA = 2*Pr$Aa*Pr$AA
Pr$FAA_MAA = Pr$AA^2
