...
```{r}
# load the dataset
mice_pheno = read.csv("data/mice_pheno.csv")
...
mice_pheno_summary = mice_pheno %>% group_by(Sex, Diet) %>% summarize(n_animals = n())
mice_pheno_summary
...
mice_pheno[which(is.na(mice_pheno$Bodyweight)),]

subset(mice_pheno, is.na(mice_pheno$Bodyweight))
...
head(mice_pheno_clean)
tail(mice_pheno_clean)
```
...
mice_pheno_clean_summary = mice_pheno_clean %>%
  group_by(Sex, Diet) %>% 
  summarize(n_animals = n())
mice_pheno_clean_summary
...
```{r}
# 5 data points were removed in total. 1 was a male from the chow group and 4 were males from the hf group.
```
...
ggplot(data = mice_pheno_clean,
       mapping = aes(x = Diet,
                     y = Bodyweight)) +
   geom_violin(width = 0.5) +  # violin
   geom_boxplot(width = 0.25) +  # box
...
```{r}
# mean bodyweight
mean_fulldataset = mean(mice_pheno_clean$Bodyweight)
mean_fulldataset

# sem
sem_fulldataset = sd(mice_pheno_clean$Bodyweight) / sqrt(nrow(mice_pheno_clean))
sem_fulldataset

# 95% CI
ci95_fulldataset = c(mean_fulldataset - 2 * sem_fulldataset,
                     mean_fulldataset + 2 * sem_fulldataset) ##approximation using 2 as the multiplication factor
ci95_fulldataset
...
```{r}
# The 95% CI tells you that the true population mean will fall within this range 95% of the time.
```
...
stat_summary = ci(mice_pheno_clean$Bodyweight)
stat_summary[1] ## mean
stat_summary["Std. Error"] ## sem
stat_summary[c("CI lower", "CI upper")] ## 95% CI
...
summary_stats = mice_pheno_clean %>% 
  group_by(Sex, Diet) %>%
  summarize(mean_weight = round(ci(Bodyweight)[1], 2),
            SEM = round(ci(Bodyweight)[4], 2),
            CI_lower = round(ci(Bodyweight)["CI lower"], 2),
            CI_upper = round(ci(Bodyweight)["CI upper"], 2))
...
fc = (filter(summary_stats, Sex == "F" & Diet == "chow"))["mean_weight"]
fh = (filter(summary_stats, Sex == "F" & Diet == "hf"))["mean_weight"]
  
mc = (filter(summary_stats, Sex == "M" & Diet == "chow"))["mean_weight"]
mh = (filter(summary_stats, Sex == "M" & Diet == "hf"))["mean_weight"]
...
# percent weight gain
f_gain_percent = pull((f_gain / fc) * 100)
f_gain_percent

m_gain_percent = pull((m_gain / mc) * 100)
m_gain_percent
...
```{r}
## Although males seem to have a higher percent weight gain, there doesn't seem to be a huge difference in weight gain between the two sexes (9% vs. 12%). However, we need to do a statistical test to confidently say whether or not this difference is significant.
```
...
```{r}
# subset the data
chow = filter(mice_pheno_clean, Sex == "M" & Diet == "chow")$Bodyweight
hf   = filter(mice_pheno_clean, Sex == "M" & Diet == "hf")$Bodyweight
```
...
```{r}
# The sample mean differences vary significantly between trials. The values ranged from very low (less than 1) to ~10. Sometimes the value was even negative indicating that the mean for the control group was greater than that for the treatment group.Since they vary so much, sometimes the difference is close to the one from the full dataset and sometimes it is way off.
```
...
resample = data.frame(sample.mean =  
                        replicate( 1000 ,                    # number of samples
                                  mean(sample( chow , 12 ))) )  # sample and take the mean
str(resample)
head(resample)
...
sem = round((sd(ctl.sample) / sqrt(sample.size)), 2)
...
```{r}
# The number varies from trial to trial but in general multiple trials showed 5 to 10 confidence intervals do NOT contain the true population mean.
```


How much does the width of the 95% CI's seem to vary?
```{r}
# The width varies from sample to sample. Some intervals are quite narrow while others are quite large.
```

About how often do your CI's contain the mean for the high-fat diet population? What do you think this might mean?
```{r}
# About 15-25 times the CIs contain the mean for the hf. This means that the difference between the chow and hf means may not be significant.
```
