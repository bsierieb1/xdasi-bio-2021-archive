...
data_mice <- read.csv("mice_pheno.csv")
...
missing_values <- which(is.na(data_mice))
missing_values
```
...
clean_data <- data_mice[complete.cases(data_mice), ] 
```
...
#five data points, as number of rows was 846 and now it is 841. 
# M/hf : 4 lost , M/chow: 1 lost
...
## Violin plot by sex and diet
  ...
  ggplot(data=clean_data,
       mapping=aes(x=clean_data$Diet,
                   y=clean_data$Bodyweight)) +
 geom_violin(aes( fill = clean_data$Diet)) +
  geom_boxplot(width=0.1) +
  facet_wrap(~clean_data$Sex,
              ncol=2,
              nrow=2)
...
Mean <- mean(clean_data$Bodyweight)
Mean

#sem (SEM is SD of the sampling distribution)

SEM <-  sd(clean_data$Bodyweight)/sqrt(length(clean_data$Bodyweight))
SEM

# 95% CI

Q = qnorm(0.975)
CI = c(Mean - Q*SEM, Mean + Q*SEM)
CI
...
```{r}
# your answer here
#the narrow range ( 28.4 t0 29.2) reflects more precise estimate.
#we can say we are 95% certain that this narrow range contains the true mean
...
fc <- summary_stats %>% filter(Sex == "F" & Diet == "chow") %>% pull(3)
fc

fh <- summary_stats %>% filter(Sex == "F" & Diet == "hf") %>% pull(3)
fh
  
mc <- summary_stats %>% filter(Sex == "M" & Diet == "chow") %>% pull(3)
mc

mh <- summary_stats %>% filter(Sex == "M" & Diet == "hf") %>% pull(3)
mh

# average weight gain by sex
average_wt_gain_female <- fh - fc
average_wt_gain_male<- mh - mc

# percent weight gain
wt_per_fem <- average_wt_gain_female /  fc * 100
wt_per_fem

wt_per_male <- average_wt_gain_male / mc * 100
wt_per_male
...
# statistical speaking, 10% or 12%  increase is not that significant, but medical wise speaking, it is yes significant#
# because according to BMI: body mass index calculation, the ideal person should hold maximum of around 30% of his weight
# to be fats, so may be we need to make deeper investigation to know more about the nature of the gained weight,
# is it pure fat or fat plus other things #then we can judge
...
```{r}
# subset the data
chow = clean_data %>% filter(Sex == "M" & Diet == "chow") %>% pull(3)
hf   = clean_data %>% filter(Sex == "M" & Diet == "hf") %>% pull(3)
```
...
#s.ctl =   # control = "chow" group
#s.trt =   # treatment = "hf" group
  
# to use sample we need to work with dataframes 
chow_df <- as.data.frame(chow)
hf_df <- as.data.frame(hf)

s.ctl = sample(chow_df$chow, 12)
#s.ctl

s.trt = sample(hf_df$hf, 12)
#s.trt
...
diff_whole <- mean(hf_df$hf) - mean(chow_df$chow)
diff_whole
...
# since we are randomly sampling each time, the 12 samples withdrawn each time are not fixed
#because here we didnt use set seed, consequently the differences each time differ,
...
replicate(6  , mean( sample(chow,10   )))
```
TenSampleBootstrapNR<-replicate(10, sample(MacUsers1stHalf
...
#without taking the mean for sample.mean it gives horrible plot actually.
...
replicate( 10 , sem( chow_df$chow ))  # compute sem for 10 samples from "chow" population
...
sem_dist = data.frame(sem = replicate(1000, sem(chow_df$chow)   ))
...
sample.size = 12
ctl.sample = sample(chow_df$chow, 12)


# compute SEM and 95% CI
  
sem = ctl.sample/sqrt(sample.size)
...
# 5% but it varies based on the sampling each time. Since each time it is a random sampling. I can see that 
# the red horizontal lines vary but with very little range.
...
#The width is almost the same, since sample size here is fixed, but if we increased sample size, the width will decrease
# in other words become more precise.
...
```{r}
# your answer here
#body weights in case of high fat are skewed, which make since, the probability of finding
#the mean in the CI of high fat diet is very low, thus reflecting imprecise sampling
```
