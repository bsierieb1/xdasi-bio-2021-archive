...
```{r}
# load the dataset
mice.data = read.csv("./data/mice_pheno.csv", stringsAsFactors = TRUE)
...
mice.data.summary = mice.data %>% group_by(Sex, Diet) %>% summarise(n_animals = n())
mice.data.summary
```
...
mice.data.cleaned = na.omit(mice.data)
```
...
mice.data.cleaned.summary = mice.data.cleaned %>% group_by(Sex, Diet) %>% summarise(n_animals = n())
mice.data.cleaned.summary
```

How many data points were removed, and from which group?

```{r}
# 5 data points: 1 from the Male, regular chow group, 4 from the Male, high-fat chow group.
```
...
ggplot(mice.data.cleaned,
       mapping = aes(x = Diet,
                     y = Bodyweight)) +
  geom_violin(width = 0.8, fill = "light blue") + # violin
  geom_boxplot(width = 0.3, alpha = 0.5) +  # box
...
```{r}
# mean bodyweight
mean(mice.data.cleaned$Bodyweight)
# sem
sd(mice.data.cleaned$Bodyweight)/sqrt(sum(mice.data.cleaned.summary$n_animals))

# 95% CI (approx.)
mean(mice.data.cleaned$Bodyweight) - 1.96 * sd(mice.data.cleaned$Bodyweight)/sqrt(sum(mice.data.cleaned.summary$n_animals))

mean(mice.data.cleaned$Bodyweight) + 1.96 * sd(mice.data.cleaned$Bodyweight)/sqrt(sum(mice.data.cleaned.summary$n_animals))

# more precise
mean(mice.data.cleaned$Bodyweight) - qnorm(0.975) * sd(mice.data.cleaned$Bodyweight)/sqrt(sum(mice.data.cleaned.summary$n_animals))
...
```{r}
# We can be 95% confident that the true population mean for mixed M/F lab mice fed high and low fat diets is between 28.42393 and 29.27017.
```
...
ci(mice.data.cleaned$Bodyweight)
...
summary_stats = mice.data.cleaned %>% group_by(Sex, Diet) %>% 
  summarise(mean.weight = mean(Bodyweight),
            SEM = ci(Bodyweight)[4],
            CI.lower = ci(Bodyweight)[2],
            CI.upper = ci(Bodyweight)[3])
...
fc = summary_stats$mean.weight[1]
fh = summary_stats$mean.weight[2]
  
mc = summary_stats$mean.weight[3]
mh = summary_stats$mean.weight[4]
...
```{r}
# as a proportion of their body weights, male vs. female weight gain seems less significantly different than when represented as the total weight gained. Both sexes independently seemed to gain a significant amount of weight (9.9%-12.5%). 
```
...
```{r}
# subset the data
chow = filter(mice.data.cleaned,
              Sex == "M",
              Diet == "chow")[3]

hf   = filter(mice.data.cleaned,
              Sex == "M",
              Diet == "hf")[3]
...
s.ctl = sample(chow$Bodyweight, size = 12)  # control = "chow" group
s.trt = sample(hf$Bodyweight, size = 12)  # treatment = "hf" group
mean(s.ctl)
mean(s.trt)
...
```{r}
# The difference between the sample means vary significantly; they tend to be within 2-3 units from the mean taken from the full data set. 
```
...
ggplot(resample,
  ...
  geom_histogram(binwidth = 0.2, 
                 fill = "light blue")+  # pick a color scheme, set binwidth
  geom_vline(aes(xintercept = mean(resample$sample.mean)))
...
sample.size = 12
ctl.sample = chow$Bodyweight

# compute SEM and 95% CI
sem = sd(sample(ctl.sample, sample.size))/sqrt(sample.size)
ci = c(sd(ctl.sample, sample.size) - sem, sd(ctl.sample, sample.size) + sem)
...
```{r}
# about 5% of C.I.s do not contain the true pop mean. This number does not seem to vary much.
```


How much does the width of the 95% CI's seem to vary?
```{r}
# The width of the C.I. seems to vary a bit; at most it seems that one C.I. might be 2x larger wider than another. 
```

About how often do your CI's contain the mean for the high-fat diet population? What do you think this might mean?
```{r}
# Greater than 10% of the time, the CI contains the high-fat diet pop mean. This may mean that our confidence in the control vs. treatment effect is low using a sample size of 12.
```
