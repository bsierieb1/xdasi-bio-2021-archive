...
author: "Elizabeth Blackburn [LUOK WEN YONG]"
date: "22ND SEPTEMPER 2021"
output:
  html_document:
    toc: yes
    toc_depth: 3
...
list.files (path = "/Users/lwyong/Desktop/LW Yong NYU/Biostat 2021 Fall/Week 4/XDASI-2021_Homework3/HW3_LuokWenYong")
...
```{r}
# load the dataset
mice_pheno<- read.csv("/Users/lwyong/Desktop/LW Yong NYU/Biostat 2021 Fall/Week 4/XDASI-2021_Homework3/HW3_LuokWenYong/data/mice_pheno.csv")
...
#Option 1 
which(is.na(mice_pheno),arr.ind=TRUE)
```
...
mice_pheno2 <- mice_pheno[complete.cases(mice_pheno), ]
```
...
```{r}
# 5 data points removed, all 5 are from male mice. 
```
...
ggplot (data = mice_pheno2, 
        mapping = aes(x = Diet, 
                      y = Bodyweight)) +
   geom_violin(width = 0.5, fill = "pink") +
...
```{r}
# mean bodyweight
mean_BW<-mean(mice_pheno2$Bodyweight)
mean_BW

# sem - found online
sem2 <- function(x) sd(x)/sqrt(length(x))
SEM_BW<-sem2(mice_pheno2$Bodyweight)
SEM_BW

# 95% CI
#online method - t test
qqnorm(mice_pheno2$Bodyweight)
qqline(mice_pheno2$Bodyweight)
t.test(mice_pheno2$Bodyweight)

#class way
Q = qnorm(0.975)
ci95_BW = c(mean_BW - Q*SEM_BW, mean_BW + Q*SEM_BW)
ci95_BW
```
...
```{r}
#The 95% CI is a range of value that contains the real mean 95% of the time when you sample from the group of data.  
```
...
summary_stats <- 
  mice_pheno2  %>% 
   group_by(Sex,Diet) %>% 
  summarize(mean=ci(Bodyweight)[1],
            SEM=ci(Bodyweight)[4],
...
Gain.F <- MEANS[2]-MEANS[1]
Gain.M <- MEANS[4]-MEANS[3]
Gain.F
Gain.M

# percent weight gain
Gain.F_perc <- MEANS[2]/MEANS[1]
Gain.M_perc <- MEANS[4]-MEANS[3]
Gain.F_perc 
Gain.M_perc
```
...
```{r}
# Male gain more weight on a high fat diet but more statistical test is needed to test the significance. 
```
...
```{r}
# subset the data
chow = subset(mice_pheno2$Bodyweight, mice_pheno2$Sex == "M" & mice_pheno2$Diet == "chow")
hf   = subset(mice_pheno2$Bodyweight, mice_pheno2$Sex == "M" & mice_pheno2$Diet == "hf")
```
...
```{r}
# The means weight difference of the sampling differs each time. They are either lower or higher than mean difference of the full dataset. 
```
...
resample = data.frame(sample.mean =  
                      replicate(sample.size,                    # number of samples
                                mean(sample(chow,n.samples,replace = T ))) )  # sample and take the mean
str(resample)
head(resample)
#str(resample)
#head(resample)
...
ggplot(data=resample, aes(sample.mean)) + 
    geom_histogram(color = "orange",
                   fill = "pink",
                  binwidth = 0.05) + # pick a color scheme, set binwidth
     geom_vline(data = resample, aes(xintercept=mean(sample.mean))) # map `sample.mean` to the vertical line (copy syntax from previous histograms above)
...
sem_dist = data.frame(sem = replicate(12, sem(sample(chow, 1000, replace = T))))

# ============================================================================ #
# plot the distribution
ggplot(data = sem_dist, mapping = aes(sem)) + 
...
```{r}
# Around 5% to 8% do not contain the true population mean. It varies by either 2 or 3 CI.
```
...
```{r}
# Most of the time it does. This means our data is has a high porpotion of the true mean. 
```
