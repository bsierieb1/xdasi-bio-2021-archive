...
```{r}
# load the dataset
mice_pheno.df=read.csv("./data/mice_pheno.csv")
# take a look at its structure and contents
summary(mice_pheno.df)
...
mice_pheno.df
mice_pheno.df.group=mice_pheno.df %>% group_by(Sex,Diet) 
mice_pheno.df.group
head(mice_pheno.df.group)
mice.sex.diet=mice_pheno.df.group %>% summarize(n_animals = n(),.groups="drop")
mice.sex.diet
```
...
mice.clear.df=mice_pheno.df[complete.cases(mice_pheno.df),,]
mice.clear.df
```
...
mice.clear.df.group=mice.clear.df %>% group_by(Sex) 
mice.clear.sex=mice.clear.df.group %>% summarize(n_animals = n(),.groups="drop")
mice.clear.sex
```
...
```{r}
# your answer here
"5 data points were removed from Male Group"
```
...
```{r}
# mean bodyweight
mean.mice=mean(mice.clear.df$Bodyweight)

# sem

sd.mice=sd(mice.clear.df$Bodyweight)

# 95% CI
qnorm(c(0.025,0.975),mean=mean.mice,sd = sd.mice)
```
...
```{r}
# your answer here
"The distribution of bodyweight is quite wide, and the majority of the body weight will fall between 16.6 and 41.1."
```
...
mice.sex.diet = mice.clear.df %>% group_by(Sex,Diet)  
summary_stats=mice.sex.diet%>% summarize(mean_weight = mean(Bodyweight),
                                                          SEM=sd(Bodyweight),CI_lower=qnorm(0.025,mean=mean_weight,sd= SEM),CI_upper=qnorm(0.025,mean=mean_weight,sd= SEM),
.groups="drop")
...
fc = filter(summary_stats,
                       Sex == "F", Diet == "chow")["mean_weight"]
fh = filter(summary_stats,
                       Sex == "F", Diet == "hf")["mean_weight"]
  
mc = filter(summary_stats,
                       Sex == "M", Diet == "chow")["mean_weight"]
mh = filter(summary_stats,
                       Sex == "M", Diet == "hf")["mean_weight"]

# average weight gain by sex
aweight=c(pull(fh-fc),pull(mh-mc))
aweight
# percent weight gain
pweight=c(pull(fh-fc)/pull(fc),pull(mh-mc)/pull(mc))
pweight
...
```{r}
# your answer here
"Males have more weight gain from high fat diet that females. I think it's significant because that sample size is big, and the std of each group is not so large."
```
...
```{r}
# subset the data
chow =filter(mice.clear.df,
                       Sex == "M", Diet == "chow")["Bodyweight"]
hf   =filter(mice.clear.df,
                       Sex == "M", Diet == "hf")["Bodyweight"]
```
...
s.ctl =  sample(chow$Bodyweight,12,replace=FALSE) # control = "chow" group
s.trt =  sample(hf$Bodyweight,12,replace=FALSE) # treatment = "hf" group
...
```{r}
# your answer here
"The difference between samples vary quite a lot. Sometimes smaller than the value of the full dataset while sometimes bigger"
```
...
ggplot(resample, aes(x=sample.mean) ) + 
  ...
  geom_histogram(col="black",fill="lightblue", alpha=0.5, 
                 boundary = 0, closed = "left", binwidth = 0.2) + 
  # map `sample.mean` to the vertical line (copy syntax from previous histograms above)
   geom_vline(aes(xintercept = mean(chow$Bodyweight)),
             color="blue")
...
sem.sample = sem(ctl.sample)
ci = c((mean(ctl.sample)-1.96*sem.sample),(mean(ctl.sample)+1.96*sem.sample))
...
```{r}
# your answer here
"5%. This number vary quite a lot between different simulation because sample size is quite low in this case "
```
...
```{r}
# your answer here
"The distribution of width of CI has the same form of the distribution of SEM, which varies a lot"
```

About how often do your CI's contain the mean for the high-fat diet population? What do you think this might mean?
```{r}
# your answer here
"Only a small part of the CI contain the high-fat mean. This means the average weight of control group could be significantly lower that that of high fat group."
```
