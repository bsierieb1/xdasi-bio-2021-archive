...
```{r}
# load the dataset
mice.df <- read.table("data/mice_pheno.csv", header = T, sep = ",", stringsAsFactors = F)
...
n_animals <- summarize(group_by(mice.df, Sex, Diet), n = n())
n_animals
...
mice.df %>% summarize(which(is.na(mice.df$Bodyweight)))
subset(mice.df, is.na(mice.df$Bodyweight))
...
mice.pheno <- mice.df[complete.cases(mice.df),]
summary(mice.pheno)
...
summarize(group_by(mice.pheno, Sex), n = n())
...
```{r}
# your answer here
mice.df[!complete.cases(mice.df),]
```
...
ggplot(data = mice.pheno, aes(x = mice.pheno$Bodyweight, y = mice.pheno$Diet)) +
   geom_violin(scale = "width")+ # violin
   geom_boxplot(fill = "white", width=0.1) + # box
  # I included the width of violin bounding box in the above (scale = "width")
...
```{r}
# mean bodyweight
mean(mice.pheno$Bodyweight)

# sem
n <- length(mice.pheno$Bodyweight)
sd(mice.pheno$Bodyweight) / sqrt(n)


# 95% CI
t.test(mice.pheno$Bodyweight)
t.test(mice.pheno$Bodyweight)$conf.int
...
ci(mice.pheno$Bodyweight)["CI lower"]
ci(mice.pheno$Bodyweight)["CI upper"]
ci(mice.pheno$Bodyweight)["Std. Error"]
...
summary_stats =   mice.pheno %>% group_by(Sex, Diet) %>% 
  summarise( mean_weight = round(ci(Bodyweight)["Estimate"], digits = 2),
             SEM = round(ci(Bodyweight)["Std. Error"], digits = 2),
             CI_lower = round(ci(Bodyweight)["CI lower"], digits = 2),
             CI_upper = round(ci(Bodyweight)["CI upper"], digits = 2))
...
fc = subset(summary_stats, Sex == "F" & Diet == "chow")$mean_weight
fh = subset(summary_stats, Sex == "F" & Diet == "hf")$mean_weight
mc = subset(summary_stats, Sex == "M" & Diet == "chow")$mean_weight
mh = subset(summary_stats, Sex == "M" & Diet == "hf")$mean_weight
...
# percent weight gain
percent_weight_gain = round((mmean_delta / fmean_delta)*100, digits = 0)
percent_weight_gain
...
```{r}
"Mice Male and Mice Female found out to be that Male was 163% more likely to gain weight on a high fat diet."
```
...
```{r}
# subset the data
chow = subset(mice.pheno, Sex == "M" & Diet == "chow")
head(chow)

hf   = subset(mice.pheno, Sex == "M" & Diet == "hf")
head(hf)
```
...
s.ctl = sample(pull(chow), 12) # control = "chow" group
s.ctl


s.trt = sample(pull(hf), 12) # treatment = "hf" group
s.trt
...
random_diff = mmean_delta - obs.diff
random_diff
...
```{r}
"Since the numbers are different each time, there is a blip between positive and negative."
```
...
replicate(6 ,mean(sample(pull(chow), 12)))
```
...
replicate(n.samples ,mean(sample(pull(chow), sample.size))) )
...
# ggplot histogram of sample means from the `resample` data frame
 ggplot(data = resample, aes(y = sample.mean) ) + 
    geom_histogram(fill = "firebrick", col = "black", binwidth = 0.1) + # pick a color scheme, set binwidth
...
# print some summary stats
 pop.mean = round(mean(chow$Bodyweight),2) # # Added for $Bodyweight
...
sem = function(pop.data, sample.size=12){
  ...
  # add formula for the SEM of a random sample here (round off to 2 digits if you want)
   n <- length(random.sample)
  se.mean <- sd(random.sample) / sqrt(n)
  se.mean <- round(se.mean, digits = 2)
  return(se.mean)
}
# ============================================================================ #
```
...
# Since SD of 1000 is what we are looking for, I changed from 10 times to 1000 times
print("SEM of individual 'chow' samples:")
resample = data.frame(sample.mean =  
replicate(1000, sem(sample(pull(chow), 12))) ) # compute sem for 10 samples from "chow" population
...
sem_dist = data.frame(sem = replicate(1000, sem(sample(pull(chow), 12))) )
...
# ctl.sample =
sample.size = 12
ctl.chow = subset(mice.pheno, Diet == "chow")
ctl.sample = sample(pull(ctl.chow), sample.size)
...
mean = mean(ctl.sample)
mean

ci_lower = mean - (1.96 * sem)
ci_upper = mean + (1.96 * sem)
civ = c(ci_lower, ci_upper)
str(civ)
...
chowPop = pull(subset(mice.pheno, Diet == "chow"))
hfPop = pull(subset(mice.pheno, Diet == "hf"))
...
```{r}
"Those that do not fall into the confidence interval are represented by the red line.
I ran it five times, and on average, there were about six red lines out of 100."
```


How much does the width of the 95% CI's seem to vary?
```{r}
"The width of the 95% confidence interval was about 3 for the shortest and about 8 for the longest."
```

About how often do your CI's contain the mean for the high-fat diet population? What do you think this might mean?
```{r}
"I ran it five times, and the average was about 45 times."
```
