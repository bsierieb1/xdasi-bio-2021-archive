...
mice_pheno %>% group_by(Sex, Diet) %>% summarise(n_animals=n())
...
mice_pheno_cleaned = mice_pheno[complete.cases(mice_pheno),]
```
...
mice_pheno_cleaned %>%group_by(Diet,Sex) %>% summarise(n_animals=n())
```

How many data points were removed, and from which group?

```{r}
# your answer here : 4 from male hf and 1 from male Chow
```
...
ggplot(data=mice_pheno_cleaned,
       mapping= aes(x=Diet,
                    y=Bodyweight,
                    )) +
   geom_violin(aes(fill=Diet)) +# violin
   geom_boxplot(width=0.1) + # box
...
```{r}
# mean bodyweight
mean_bodyweight = mean(mice_pheno_cleaned$Bodyweight)

# sem
sem_bodyweight= sd(mice_pheno_cleaned$Bodyweight) / sqrt(length(mice_pheno_cleaned$Bodyweight))

# 95% CI
Q=qnorm(0.975)
ci95 = c(mean_bodyweight - Q*sem_bodyweight, mean_bodyweight + Q*sem_bodyweight)
...
```{r}
# your answer here : a 95% CI is a range of values surrounding a sample estimate in which the true population parameter is expected to be found 95% of the time. The True population mean lies in 95/100 independent samples
```
...
ci(mice_pheno_cleaned$Bodyweight)
```
...
summary_stats = mice_pheno_cleaned  %>% group_by(Sex, Diet) %>% 
  summarise(mean_weight=round(mean(Bodyweight),2),
            SEM=round(ci(Bodyweight)["Std. Error"],2),
...
means_pulled=pull(summary_stats,var=mean_weight)
...
average_gain_m=mh-mc
average_gain_f=fh-fc

# percent weight gain
percent_gain_m = average_gain_m*100/mc
percent_gain_f = average_gain_f*100/fc
...
```{r}
# your answer here: It looks like the percent gain by mass for females and males on a high fat diet is approximately the same. There is no way to draw a conclusion without valid hypothesis testing though
```
...
```{r}
# subset the data
chow = mice_pheno_cleaned[which(mice_pheno_cleaned$Diet=="chow" & mice_pheno_cleaned$Sex=="M"),] 
hf   = mice_pheno_cleaned[which(mice_pheno_cleaned$Diet=="hf" & mice_pheno_cleaned$Sex=="M"),]
```
...
obs.diff = abs(mean(s.trt)-mean(s.ctl))
obs.diff

# mean difference from full dataset
full_diff=abs(mean(hf$Bodyweight)-mean(chow$Bodyweight))
full_diff
```
...
```{r}
# your answer here: The mean difference from the sample is always about the same as the actual difference between the means. There are sometimes when I would get results that would vary considerably, but these have been very rare.
```
...
ggplot( data= resample,                     #Data from samples
        mapping=aes(x=sample.mean)) +       #Plotting sample means  
    geom_histogram(binwidth = 0.1,          #Setting binwidth =0.1
                   fill="firebrick",        #Color of the coloumn 
...
sample.size = 12
ctl.sample = sample(chow$Bodyweight,sample.size)

# compute SEM and 95% CI
sem = sd(ctl.sample)/sqrt(sample.size)
ci = c(mean(ctl.sample) - qnorm(0.975)*sem, mean(ctl.sample) + qnorm(0.975)*sem )
...
```{r}
# your answer here: 5% or 1/20
```


How much does the width of the 95% CI's seem to vary?
```{r}
# your answer here: Does not vary too much.. typically it ranges about 6 units.
```

About how often do your CI's contain the mean for the high-fat diet population? What do you think this might mean?
```{r}
# your answer here: very infrequently.. This might mean that the true mean of the control diet is different than the high fat diet population. This can be proven with hypothesis testing
```
