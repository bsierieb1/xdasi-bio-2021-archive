...
sum_mice= mice_pheno.df%>%group_by(Sex,Diet)%>%summarise(n_animals=n())
sum_mice
...
mice_pheno.df[which(is.na(mice_pheno.df$Bodyweight)),]
```
...
mice.filter = mice_pheno.df[!is.na(mice_pheno.df$Bodyweight),]
mice.filter
str(mice.filter)
...
sum_mice_sex= mice.filter%>%group_by(Sex,Diet)%>%summarise(n_animals=n())
sum_mice_sex
...
```{r}
# your answer here
# 1 data points from the male chow
# 4 data points from the male hf
```
...
ggplot(data=mice.filter,
       mapping=aes(x=Diet,
                   y=Bodyweight)) +
  
   geom_violin(width=0.7)  +   # violin
   geom_boxplot(width=0.3) +  # box
...
```{r}
# mean bodyweight
mice_filter_mean=mean(mice.filter$Bodyweight)
mice_filter_mean
# sem
mice_filter_sem=sd(mice.filter$Bodyweight)/sqrt(nrow(mice.filter))
mice_filter_sem
# 95% CI
mice_filter_qnorm=qnorm(0.975)
mice_filter_qnorm
mice_filter_ci95 = c(mice_filter_mean - mice_filter_qnorm*mice_filter_sem, mice_filter_mean+ mice_filter_qnorm*mice_filter_sem)
mice_filter_ci95
...
```{r}
# your answer here
# the 95% CI is a range of values surrounding a sample estimate in which the true population parameter is expected to be found 95% of the time. 
```
...
summary_stat=ci(mice.filter$Bodyweight)
summary_stat

summary_stat[1] # Estimate
summary_stat[3] #CI upper
...
summary_stats=mice.filter %>%
  group_by(Sex,Diet) %>%
  summarise(mean_weight=round(ci(Bodyweight)[1],2),
...
# percent weight gain
gain_F_percent=(gain_F/fc)*100
gain_F_percent=pull(gain_F_percent)
gain_F_percent

gain_M_percent=(gain_M/mc)*100
gain_M_percent=pull(gain_M_percent)
gain_M_percent
```
...
```{r}
# your answer here
# Males put on more weight than females, but I don't know if that difference is significant.
```
...
```{r}
# subset the data
chow =(filter(mice.filter, Sex== "M" & Diet== "chow"))$Bodyweight
hf   = (filter(mice.filter, Sex== "M" & Diet== "hf"))$Bodyweight
chow
hf
...
```{r}
# your answer here
# depends on the sampling, sometimes is close to the real mean, sometimes is not. It varies between 0 and 10.
```
...
ggplot(resample,
  ...
  geom_histogram(color="blue",
                 fill="red",
                 binwidth = 0.3 ) + # pick a color scheme, set binwidth
...
```{r}
# your answer here
# Different number appear after each trial, it seems that between 4 and 10 CI do not contain the true population mean. 
```
...
```{r}
# your answer here
# The width of the 95% CI varies between trials, sometimes it is small other times it is quite big.
```

About how often do your CI's contain the mean for the high-fat diet population? What do you think this might mean?
```{r}
# your answer here
# My CI's contain the mean for the high-fat diet population about 20 times, it might mean that the difference between chow and hf means may not be significant.
```
