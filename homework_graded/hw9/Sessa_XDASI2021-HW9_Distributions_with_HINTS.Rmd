...
```{r}
frap=read.csv("/Users/Sofia/Desktop/R/R week 9/hw 9/glh_frap.csv",header=T, stringsAsFactors = T)
head(frap)
tail(frap)
str(frap)
```
...
```{r}
# summary table
frap.summary.norm = frap %>% 
  group_by(Time, strain) %>%
  summarise(mean.Intensity_normalized = mean(norm.Intensity) )

head(frap.summary.norm)

View(frap.summary.norm)

# plot

ggplot(frap.summary.norm,
       mapping=aes(x=Time,
                   y=mean.Intensity_normalized,
                 col=strain))+
  geom_line()+
  ggtitle("Fluorescence")
```
...
frap.norm.summary$sd_upper = frap.norm.summary$i.mean + frap.norm.summary$i.sd
frap.norm.summary$sd_lower = frap.norm.summary$i.mean - frap.norm.summary$i.sd

# SE.upper and SE.lower
frap.norm.summary$se_upper = frap.norm.summary$i.mean + frap.norm.summary$i.se
frap.norm.summary$se_lower = frap.norm.summary$i.mean - frap.norm.summary$i.se

# CI.upper and CI.lower
frap.norm.summary$ci_upper = frap.norm.summary$i.mean + 
  qt(0.975, df = (frap.norm.summary$rep - 1)) * frap.norm.summary$i.se
frap.norm.summary$ci_lower = frap.norm.summary$i.mean - 
  qt(0.975, df = (frap.norm.summary$rep - 1)) * frap.norm.summary$i.se
...
```{r}
# SD plot
sd.plot = ggplot(frap.norm.summary, mapping=aes(x=Time,
                                        y=i.mean,
                                        col=strain)) +
geom_line() +
...
# SEM plot
se.plot = ggplot(frap.norm.summary,
                 mapping=aes(x=Time,
                             y=i.mean,
                            col=strain)) +
geom_line() +
...
# 95%CI plot
ci.plot = ggplot(frap.norm.summary, mapping=aes(x=Time,
                                        y=i.mean,
                                        col=strain)) +
geom_line() +
geom_ribbon(mapping=aes(x=Time, 
...
```{r eval=FALSE}
# I would prefer to show the 95% CI because it shows that the means of the two groups don't overlap, so the two groups have significantly different means.
...
```{r}
# load FRAP rates
frap_rates=read.csv("/Users/Sofia/Desktop/R/R week 9/hw 9/rates_glh.csv", header=T, stringsAsFactors = T)
head(frap_rates)
tail(frap_rates)

# make the plot
ggplot(frap_rates, mapping=aes(x=Treatment, 
                               y=Rate, 
                             col=Treatment)) + 
  geom_boxplot() + 
...
#p.value
t.test(mut.rate, wt.rate, alternative = "greater")$p.value
```
...
```{r eval=FALSE}
# the p-value is smaller than 0.05 so the difference between the means is statistically significant. So MIP-1 protein helps anchor GLH-1 in germ granules of C. elegans early embryo.
```
...
```{r eval=FALSE}
# probability of success here is the probability of the ribosome not dissociating and thus producing a functional ATF4 protein, this probability is 0.02 because functional protein is produced 2% of the time 
#probability of failure is the ribosome dissociating, and this is 98% of the time, so probabilty of failure is 0.98.
```
...
```{r eval=FALSE}
# here we are considering the number of failures before one success, so we can use the Geometric distribution that gives the probability that n independent failures happen before the first success.
...
# for r=10 "successes"
new_range=1:1000
atf4.response.activation.pmf <- dnbinom( x = new_range, size = 10, prob = 0.02 )

# Create a data.frame with the data
atf4.response.activation.pmf.df = data.frame(
  PMF = atf4.response.activation.pmf , 
  Ribosome_Bindings = new_range )
...
atf4.response.activation.pmf.df$Time = atf4.response.activation.pmf.df$Ribosome_Bindings*1/6
...
```{r eval=FALSE}
# based on the graph above I do not expect that the cellular stress response program is activated by chance in unstressed conditions because the protein half life is 20 min while the time of activation is about 70 min. the rate of protein degradation is faster that the ability to reach the total number required of 10 proteins to activate the response. If the proteins were very stable I can imagine the activation of the response program can occur by chance because the degradation would be slower than the time needed to reach a total of 10 required proteins.
```
...
up_range=1:1000
stressed_pmf = dnbinom(x = up_range, size = 10, prob = 0.4)


# Create a data.frame with the data for the stressed condition.
# Add columns for Ribosome_Bindings, Time, and Condition.
stressed_pmf_df = data.frame(PMF = stressed_pmf, Ribosome_Bindings = up_range,Time = (up_range)/6,
                             Condition = "Stressed")

# Row bind the unstressed and stressed data to create a full data set.
binded_data = rbind(atf4.response.activation.pmf.df, stressed_pmf_df)
head(binded_data)
tail(binded_data)
...
ggplot(data = binded_data,
       mapping = aes(x = Time,y = PMF,col = Condition)) +
  geom_line()
```
...
```{r eval=FALSE}
#  Yes, now I expect the cellular stress response to be activated. A small number of ribosome binding events are needed to activate the stress response. In the "stressed" condition the time of activation is less than 10 min, a time that is smaller that the protein half life. The cell activates the stress response in 10 minutes, and in this period of time 10 molecules can be translated. Degradation is not fast enough to contrast the activation of the stress response.
```
