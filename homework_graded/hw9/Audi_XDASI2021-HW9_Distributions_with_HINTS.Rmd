...
```{r}
frap <- read.csv("/Users/sicg_labmanager/Documents/NYU BIO/Biostats/XDASI2021-HW9_Distributions/glh_frap.csv")
str(frap)
head(frap)
...
```{r}
# summary table
detach(package:plyr) # this fucks up groupby/summarize so gotta remember to detach!!!!
...
# plot
ggplot(data = frap_sum, mapping = aes(x = Time, y = Intensity_mean, color = strain)) +
  geom_line()
...
norm_f = (frap_sum$Intensity_mean)
```
...
norm_table = frap %>%
  group_by(rep, strain) %>%
  mutate(Norm_Intensity = norm_f(Intensity))
...
# plot
ggplot(data = sum_table, mapping = aes(x = Time, y = Norm_Int_mean, color = strain)) +
  geom_line()
```
...
```{r}
# SD plot
sd.plot = ggplot(data = sum_table_sig, mapping = aes(x = Time, y = Norm_Int_mean, col = strain)) +
...
# SEM plot
sem.plot = ggplot(data = sum_table_sig, mapping = aes(x = Time, y = Norm_Int_mean, col = strain)) +
...
# 95%CI plot
ci.plot = ggplot(data = sum_table_sig, mapping = aes(x = Time, y = Norm_Int_mean, col = strain)) +
...
```{r eval=FALSE}
# I prefer SEM because no overlap and it utilizes the SD. It's more typically to SE than SD plotted. 
```
...
```{r}
# load FRAP rates
rates = read.csv("/Users/sicg_labmanager/Documents/NYU BIO/Biostats/XDASI2021-HW9_Distributions/rates_glh.csv")
head(rates)
...
```{r eval=FALSE}
#MIP-1 has a significant effect on GLH-1 in germ granules compared to the wild type. 
```
...
```{r eval=FALSE}
#0.02 because the ribosome dissociates from the transcript 98% of the time.
```
...
```{r eval=FALSE}
# Geometric - one success or failure option like in NB 
```
...
atf4.response.activation.pmf = dnbinom(c(0:1000),10, 0.02)
...
# Make a density plot 
ggplot(data = atf4.response.activation.pmf.df, mapping = aes(x = Ribosome_Bindings, y = PMF)) +
  geom_line()
```
...
time = c(0:1000)*10/60
atf4.response.activation.pmf.df.t = cbind(atf4.response.activation.pmf.df, time)

head(atf4.response.activation.pmf.df.t)

# Make a density plot of the TIME to activate the response
# (time goes on the x-axis, density goes on the y-axis).

ggplot(atf4.response.activation.pmf.df.t, aes(x = time, y = PMF)) +
  geom_line()
```
...
```{r eval=FALSE}
#  It is unlikely that the cellular stress response program will activate in unstressed condition. If it was very stable we also don't expect to see activation. We see a normal distribution when plotted, thus there is no evidence that anything out of the ordianry is happening with the cell. 
```
...
# "Unstressed"
condition_df =  atf4.response.activation.pmf.df.t %>%
  mutate(Condition = rep("unstressed", nrow(atf4.response.activation.pmf.df.t)))
...
stressed_AF = data.frame(Ribosome_Bindings = c(0:1000), 
                             PMF = pmf_1000, 
                             time = time, 
...
```{r eval=FALSE}
#Yes! Because they are STRESSED AF! Yes, the spike in the PMF shows that the probability of success increases drastically at the very beginning of the time frame after exposure to tunicamycin. 
```
