...
author: "Siméon Denis Poisson [YOUR NAME HERE]"
...
# plot
ggplot(frap.summary,
       aes(x=Time,y=mean.Intensity,col=strain))+
  geom_line()
```
...
frap = frap %>%
  group_by( strain,rep ) %>%
  mutate(norm.Intensity =  int.norm(Intensity) )
head(frap)
# View(frap)
frap
```
...
frap.norm.summary = frap %>% 
  ...
  summarise(i.mean = mean(Intensity) ,
            i.sd   = sd(Intensity) ,
            i.se   = sd(Intensity)/sqrt(max(rep)) ,
            rep    = max(rep) )
...
frap.norm.summary=frap.norm.summary %>%group_by(strain,Time) %>% 
  mutate(SD.upper=i.mean+i.sd,
                                 SD.lower=i.mean-i.sd,
                                 SE.upper=i.mean+i.se,
                                 SE.lower=i.mean-i.se,
CI.upper=i.mean+qt(0.975,df=rep-1)*i.se,
        CI.lower=i.mean+qt(0.025,df=rep-1)*i.se  )
...
```{r}
# SD plot
sd.plot = ggplot(frap.norm.summary,
                 aes(x=Time,
                     y=i.mean,col=strain )) +
    geom_ribbon(aes(ymin=SD.lower,ymax=SD.upper,fill=strain),alpha=0.1)+
  geom_line()+
  theme_classic() +
...
se.plot = ggplot(frap.norm.summary,
                 aes(x=Time,
                     y=i.mean,col=strain )) +
    geom_ribbon(aes(ymin=SE.lower,ymax=SE.upper,fill=strain),alpha=0.1)+
  geom_line()+
  theme_classic() +
  ggtitle("FRAP with SEM")

# 95%CI plot
ci.plot = ggplot(frap.norm.summary,
                 aes(x=Time,
                     y=i.mean,col=strain )) +
    geom_ribbon(aes(ymin=CI.lower,ymax=CI.upper,fill=strain),alpha=0.1)+
  geom_line()+
  theme_classic() +
...
```{r eval=FALSE}
# the graph of CI because it excludes outliers and meanwhile shows the significant variation of the data
```
...
```{r}
# load FRAP rates
rate=read.csv("./rates_glh.csv")
rate=rate %>% group_by(Treatment)
# make the plot
ggplot(rate,
...
wt.rate = rate %>% filter(Treatment=="WT")
wt.rate=wt.rate$Rate
mut.rate =rate %>% filter(Treatment=="mip1uae1")
mut.rate=mut.rate$Rate
...
```{r eval=FALSE}
# the mip1uae1 mutant has increased rate of recovery
```
...
atf4.unstressed.pmf <- dnbinom( x=0:200,size=1,prob=0.02 )
...
ggplot(atf4.unstressed.pmf.df,
       aes(x=Correctly_Dissociated,y=PMF))+
  geom_line()
```
...
```{r eval=FALSE}
# geometric distribution because here we are just expecting 1 success
```
...
# for r=10 "successes"
atf4.response.activation.pmf <- dnbinom(0:1000,size=10,p=0.02)
...
atf4.response.activation.pmf.df=atf4.response.activation.pmf.df %>% mutate(Time=Ribosome_Bindings/6)
...
```{r eval=FALSE}
# No, the peak lies at about 75 minutes for activation, but the activation requires the 10 successes happens within about 20 minutes to make sure the molecules formed early are not degraded.
```
...
stressed.pmf <- dnbinom(0:1000,size=10,p=0.4)
stressed.pmf.df = data.frame(
  PMF = stressed.pmf, 
  Ribosome_Bindings = 0:1000 )
stressed.pmf.df=stressed.pmf.df %>% mutate(Time=Ribosome_Bindings/6)
stressed.pmf.df=stressed.pmf.df %>% mutate(Condition="Stressed")
...
```{r eval=FALSE}
# Yes, because in this condition the most of the curve lies at the time smaller than 10 minutes, and within that short time the molecules synthesized early will not get degraded before activation.
```
