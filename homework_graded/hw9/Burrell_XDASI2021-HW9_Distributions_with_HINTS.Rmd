...
```{r}
FRAP.data = read.csv("glh_frap.csv", stringsAsFactors = F)
head(FRAP.data)
```
...
```{r}
# summary table
frap.summary = FRAP.data %>% 
  group_by(strain) %>% group_by(Time, .add = TRUE) %>% 
  summarise(mean.Intensity = mean(Intensity), .groups = "keep" )
head(frap.summary)
# View(frap.summary)
frap.summary
# plot
...
norm.fun = function(data) {
  new.data = (data-min(data)) / (1-min(data))
  return(new.data)
}
...
FRAP.data.norm = FRAP.data %>%
  group_by( strain) %>%  group_by(rep, .add = T) %>%
  mutate(norm.Intensity =  norm.fun(Intensity) )
head(FRAP.data.norm)
View(FRAP.data.norm)
```
...
```{r}
# summary table
frap.summary.norm = FRAP.data.norm %>% 
  group_by(strain) %>% group_by(Time, .add = TRUE) %>% 
  summarise(mean.norm.Intensity = mean(norm.Intensity), .groups = "keep" )
head(frap.summary)
# View(frap.summary)
frap.summary.norm
# plot
ggplot(data = frap.summary.norm,
       mapping = aes(x=Time,
...
```{r}
frap.summary.norm.new = FRAP.data.norm %>% 
  group_by(strain) %>% group_by(Time, .add = TRUE) %>%
  summarise(i.mean = mean(norm.Intensity) ,
            i.sd   = sd(norm.Intensity) ,
            i.se   = i.sd / sqrt(max(rep)),
            rep    = length(rep) )
head(frap.summary.norm.new)
...
frap.summary.norm.wstats = frap.summary.norm.new %>% mutate( SD.upper =  (i.mean + i.sd),
                                                             SD.lower = (i.mean - i.sd),
                                                             SE.upper = (i.mean + i.se),
                                                             SE.lower = (i.mean - i.se),
                                                             CI.upper = (i.mean + 2*i.se),
                                                             CI.lower = (i.mean - 2*i.se))

head(frap.summary.norm.wstats)
#View(frap.summary.norm.wstats)
```
...
```{r}
# SD plot
sd.plot = ggplot(data = frap.summary.norm.wstats,
                 mapping = aes(x=Time,
                     y=i.mean,
                     col = strain)) +
  geom_line() +
  geom_ribbon(aes(ymin = SD.lower,
                     ymax = SD.upper,
                     fill = strain),
               alpha = 0.5) +
  theme_classic() +
...
# SEM plot
se.plot = ggplot(data = frap.summary.norm.wstats,
                 mapping = aes(x=Time,
                     y=i.mean,
                     col = strain)) +
  geom_line() +
  geom_ribbon(aes(ymin = SE.lower,
                     ymax = SE.upper,
                     fill = strain),
               alpha = 0.5) +
  theme_classic() +
...
# 95%CI plot
ci.plot = ggplot(data = frap.summary.norm.wstats,
                 mapping = aes(x=Time,
                     y=i.mean,
                     col = strain)) +
  geom_line() +
...
```{r eval=FALSE}
# 95% C.I. since the 95% C.I.s do not overlap and we can therefore infer statistical significance between the two lines visually. 
```
...
```{r}
# load FRAP rates
frap.rates = read.csv("rates_glh.csv") %>% group_by(Treatment)
# make the plot
ggplot(data = frap.rates,
...
wt.rate = frap.rates[1:12,4]
mut.rate = frap.rates[13:22,4]
...
```{r eval=FALSE}
# MIP-1 mutants have a significantly faster GLH-1 FRAP rate, suggesting that MIP-1 plays a role in slowing down diffusion of GLH-1. 
```
...
```{r eval=FALSE}
# Geometric. Since we are only interested in the outcome of a single success, the geometric and negative binomial are identical in this circumstance.
```
...
atf4.response.activation.pmf.df = atf4.response.activation.pmf.df %>% mutate(time.minutes = Ribosome_Bindings*10/60)

# Make a density plot of the TIME to activate the response
# (time goes on the x-axis, density goes on the y-axis).

ggplot( data = atf4.response.activation.pmf.df,
        mapping = aes(x = time.minutes,
                      y = PMF)) +
  geom_line()
...
```{r eval=FALSE}
# Since the relative half-life of ATF4 is much lower than the maximum of the PDF for the time to activate its transcription, and since you need 10 ATF4 molecules to accumulate prior to seeing activation of the cellular stress response program, this is not likely to occur by random chance. If, however, the half-life of ATF4 was much greater (10 x greater or more) than the maximum of the PDF for time to activate its transcription, we might expect that the cellular stress response program would be activate at times by random chance. 
```
...
```{r eval=FALSE}
# The cell is much more likely to activate the cellular stress response since the maximum PDF for the time to activate transcription of 10 ATF4 molecules is around 2 minutes, indicating that most probably 10 ATF4 will be transcribed every 2 minutes. Given the half life of ATF4 is 20 minutes, the concentration of ATF4 molecules within cells treated by tunicamycin should greatly exceed 10 molecules, and activation of cellular stress response can be expected to happen fairly regularly. 
