...
# ============================================================================ ## 
# load the file
tigers.df = read.csv(file ="/Users/lwyong/Desktop/LW Yong NYU/Biostat 2021 Fall/Week 3/Homework2/data/Ex2.2A_tigers.csv")
...
tiger.table = sort(table(tigers.df$activity),decreasing=T) 
#create frequency table, sort by descending order

tiger.table.df = as.data.frame(tiger.table)
# creates a data frame out of frequency table
...
# check data structures col
class(tiger.table.df)
head(tiger.table.df)

# ============================================================================ #
# write the table to a file 
write.csv(tiger.table.df,"/Users/lwyong/Desktop/LW Yong NYU/Biostat 2021 Fall/Week 3/Homework2/data/tiger_attack_frequency.csv", row.names = FALSE)
```
...
```{r, error=TRUE}
tiger.barplot<- barplot(height=tiger.table.df$Frequency, col = "firebrick")
```
...
tiger.barplot<- barplot(height=tiger.table.df$Frequency, col = "firebrick", ylim=c(0,50))
...
# 0. open a handle for writing to a PDF file 
#    (skip for part b; we will add this in part c)
pdf("tigers_freq_plot.pdf") #to write PDF
dev.cur()  
...
```{r, collapse=TRUE, error=TRUE}
# load the data
salmonSizeData <- as.data.frame(read.csv(file="/Users/lwyong/Desktop/LW Yong NYU/Biostat 2021 Fall/Week 3/Homework2/data/Fig2.2-5_SalmonBodySize.csv"))
...
hist(salmonSizeData$massKg,
     ...
     xlim = c(1,4),
     ylim = c(0,30), 
     main = "Salmon Size Distribution" #title
    )
...
qplot(x=massKg,
      data=salmonSizeData,
      fill=I("firebrick"),
      color=I("black"),   # here color applies to the borders!
      ) 
...
ggplot (data = salmonSizeData,
        mapping = aes(x = massKg)) + 
  geom_histogram(fill = "firebrick",col ="black", binwidth = 0.1, boundary=0, closed="left") +
         
# add data+aes+geom layers with additions specified above 
...
avianMalaria <- read.csv("/Users/lwyong/Desktop/LW Yong NYU/Biostat 2021 Fall/Week 3/Homework2/data/Ex2.3A_malaria.csv")
str(avianMalaria)
```
...
avianMalaria.table #show results

# ============================================================================ #
# add "marginal" totals
 avianMalaria.table2 = addmargins(avianMalaria.table, margin = c(1,2), FUN = sum, quiet = TRUE)
avianMalaria.table2

#make database from table for further use just in case
avianMalaria.df<-as.data.frame(avianMalaria.table2)
colnames(avianMalaria.df) = c("Response", "Treatment","Frequency")
avianMalaria.df
```
...
# ============================================================================ #
# stacked
barplot(height = avianMalaria.table) #must be matrix or vector?

# ============================================================================ #
# side-by-side
barplot(height = avianMalaria.table,
        beside = T, #make side by side
       ) 
...
barplot(height = avianMalaria.table,
        beside = T, 
        ylim = c(0,30), #added to make graph nicer
        ylab = "Frequency", # some bells and whistles
...
avianMalaria.df2<-as.data.frame(avianMalaria.table)
colnames(avianMalaria.df2) = c("Response", "Treatment","Frequency")
avianMalaria.df2

ggplot(data = avianMalaria, #df is from avianMalaria.table2
      mapping = aes(x = treatment, fill = response)) +
...
# a fancier plot
ggplot(data = avianMalaria, #df is from avianMalaria.table2
      mapping = aes(x = treatment, fill = response)) + # add the data and aesthetics here
...
a. First, make a basic `mosaciplot()` using the malaria table. (SPELLING ERROR DETECTED HERE)

```{r, error=TRUE}
avian.m.mosiac = mosaicplot(avianMalaria.table)
```
...
```{r, error=TRUE}
avian.m.mosaic2 = t(avianMalaria.table) 
avian.m.mosaic2
mosaicplot(avian.m.mosaic2)
...
```{r, error=TRUE}
#read salmon file
salmon.df = read.csv(file="/Users/lwyong/Downloads/Homework2/data/Fig2.2-5_SalmonBodySize.csv")
...
plot(salmon.df$lengthMm, salmon.df$massKg, 
     xlab = "Length(Mm)", ylab = "Mass (Kg)") #added x y labels for fun, not required. 

# ============================================================================ #
# add the best-fit regression line for these data
# (execute the entire code block at once to see the result)
abline(lm(massKg ~ lengthMm, data = salmon.df)) #changed data name cz reloaded it
```
...
ggplot(data = salmon.df,
       mapping = aes(x=lengthMm, y=massKg)) + 
 geom_point(size = 2,    
            col = "blue",
            shape = 1) +
