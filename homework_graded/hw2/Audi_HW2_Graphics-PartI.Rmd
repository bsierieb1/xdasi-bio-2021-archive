...
# ============================================================================ ## 
# load the file
tigers.df = read.csv("/Users/sicg_labmanager/Documents/NYU BIO/Biostats/Homework2/data/Ex2.2A_tigers.csv")
...
#tiger.table= as.data.frame(tiger.table)
# tiger.table.df = sort(tiger.table)

# Option 2: all at once ---------------------------------------------------- #

tiger.table.df = as.data.frame(sort(table(tigers.df$activity)))
tiger.table.df
...
write.csv(tiger.table.df, "tiger.freq.table.csv")
```
...
freq = as.vector(as.numeric(tiger.table.df$Frequency))
freq
barplot(freq, 
        col="firebrick")
```
...
my.ggplot = ggplot(tiger.table.df, 
                   aes(Activity, Frequency)) + 
  geom_bar(fill="firebrick", stat="identity") + # make a bar graph 
...
ggsave("my.ggplot.png") # saves last plot
...
salmonSizeData <- read.csv("/Users/sicg_labmanager/Documents/NYU BIO/Biostats/Homework2/data/Fig2.2-5_SalmonBodySize.csv")
...
qplot(salmonSizeData$massKg)

# ============================================================================ #
# b. body mass histogram with ggplot

ggplot(salmonSizeData, 
       aes(x=massKg)) + 
...
avianMalaria <- read.csv("/Users/sicg_labmanager/Documents/NYU BIO/Biostats/Homework2/data/Ex2.3A_malaria.csv")

avianMalaria$treatment <- as.factor(avianMalaria$treatment)
avianMalaria$response <- as.factor(avianMalaria$response)
...
levels(avianMalaria$treatment) = c("Control", "Egg Removal")
levels(avianMalaria$response) = c("Malaria", "No Malaria")
...
# stacked
avianMalaria.df = as.data.frame(avianMalaria.table, colnames=TRUE)
colnames(avianMalaria.df) = c("response", "treatment", "freq")
avianMalaria.df$response = as.character(avianMalaria.df$response)
str(avianMalaria.df)

avian.gg = ggplot(data = avianMalaria.df, 
       mapping = aes(x=treatment,
                     y=freq,
                     fill = response))  
avian.gg + geom_col()
#dodge stops the bars from stacking on top of one another 

avian.gg + geom_col(position = "dodge") # can use this instead of geom_bar for basic bar plot
```
...
ggplot(data = avianMalaria.df, 
       mapping = aes(x=treatment,
                     y=freq,
                     fill = response)) +

  # some bells and whistles
  #added stat = identity to overide issue with stat_count
...
plot(salmonSizeData$massKg ~ salmonSizeData$lengthMm)
...
ggplot(data= salmonSizeData, 
       aes(x= lengthMm, 
           y=massKg)) +
  geom_point(shape=1, size=2 ,col="green") + 
