...
tiger.table = table(tigers.df$activity) ## If you don't use the activity column and instead use the entire data frame, it will give you a table with 0 inserted where there is no data for every person for every activity.
tiger.table
...
write.csv(tiger.table.df, 
        ...
        row.names = FALSE,
        col.names = c("Activity", "Frequency"))
...
```{r, error=FALSE}
barplot(tiger.table.df$Frequency,
        col = "firebrick")
...
pdf("tigers_freq_plot.pdf") ## writing to pdf by opening a device
...
ggsave("tigers_freq_ggplot.png", my.ggplot)
...
png("tigers_freq_ggplot_save2.png")
...
salmonSizeData = read.csv("data/Fig2.2-5_SalmonBodySize.csv",
                    colClasses = c("numeric","factor","numeric","numeric","numeric"),
                     stringsAsFactors = TRUE) ## setting the colclasses so that the "F"s                  in the "sex" column is not misinterpreted as a logical FALSE
...
flipped_table = t(avianMalaria.table)
mosaicplot(flipped_table)
