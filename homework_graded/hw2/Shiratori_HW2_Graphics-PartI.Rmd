...
# ============================================================================ ## 
# load the file
tigers.df = read.csv("./data/Ex2.2A_tigers.csv", header = TRUE, stringsAsFactors = TRUE)

# ============================================================================ ## 
# take a look at it
# Using head() so I don't have all the lines of data in my final knitted document
head(tigers.df)
```
...
sorted_tiger_table <- sort(tiger.table, decreasing = TRUE)
sorted_tiger_table

tiger.table.df <- as.data.frame(sorted_tiger_table)
tiger.table.df
...
write.csv(tiger.table.df, "./data/tiger_attack_freq.csv", row.names = FALSE)
...
# This is the how we save the following plot into a .pdf. We have to write this in at the beginning of the code chunk for the plot in question and add dev.off() after it.
pdf("tigers_freq_plot.pdf")
dev.off()
#    (skip for part b; we will add this in part c)
barplot(tiger.table.df$Frequency, col = "firebrick", ylim = c(0, 50))
...
# We're specifying that the data is tiger.table.df and the X-axis should be activity and the y-axis should be frequency
my.ggplot = ggplot(data = tiger.table.df, aes(x = Activity, y = Frequency)) +
  geom_bar(fill = "firebrick", stat = "identity") +
  
  
  # bells and whistles
  # This is specifying the labels, the theme, and the angle (45 degrees) that the text should be set to.
...
ggsave("ggplot_tiger_frequency_barplot.png", path = "./data/")
...
png("printed_tiger_plot")
# print the graph object
print(my.ggplot)
# close the device
dev.off()

#This makes the resulting plot much more pixelated.
...
```{r, collapse=TRUE, error=TRUE}
# load the data
salmonSizeData <- read.csv("./data/Fig2.2-5_SalmonBodySize.csv", header = TRUE)
...
hist(salmonSizeData$massKg, col = "firebrick")

# ============================================================================ #
# b. Replot with bin size (interval width) = 0.1
# Setting breaks to 20 because then the interval bin size becomes 0.1. 
# Setting xlim and ylim to 2 argument lists with their respective mins and maxes
# Setting title with the main parameter and the x-axis label with the xlab parameter
hist(salmonSizeData$massKg, col = "firebrick", breaks = 20, xlim = c(1, 4), ylim = c(0, 30), main = "Salmon Size Distribution", xlab = "mass (kg)")
...
# salmon mass plot from the previous code chunk with binwidth specified as well as boundary and closed for bar positioning.
# The resulting plot doesn't have the bars rising up from the X axis like the plot in the book does. Why does this histogram's y axis not start at 0?
ggplot(data = salmonSizeData, 
       aes(massKg)) +
  geom_histogram(fill = "firebrick", 
                 col = "black",
...
avianMalaria <- read.csv("./data/Ex2.3A_malaria.csv", header = TRUE)
head(avianMalaria)
...
# ============================================================================ #
# side-by-side
# In order to set the bars to be next to each other instead of stacked, you need to add in "beside = TRUE"
barplot(height = avianMalaria.table,
        beside = TRUE)
...
# with ggplot, instead of the beside = TRUE parameter like in base R to made side-by-side bars, you can use position = "dodge"
...
# This should be figure 2.3-2, not 2.2-2. Fig 2.2-2 is a pie chart of the tiger data
...
mosaicplot(t(avianMalaria.table),
            ...
            border=FALSE,
            main = "Frequency of Malaria when Eggs are Removed" )
```
