---
title: "XDDAS-2020 Logistic Regression"
...
It can be seen that this equation ranges from 0 to 1, as desired. The probability is very close to 0 when the exponent $(\beta_o + \beta_1X)$ is really big, and is close to 1 when it is very small, since one of the terms becomes negligible in either case.
...
From this we can see that for a debt of \$1500 (nd income being equal), we would predict that non-students will default at a rate of around 10.5%, whereas students would default only around 5.8% of the time. Go students! ;-)
...
"**Success**" in prediction is typically referred to as a "**positive**" outcome, and vice versa. There is always a trade-off between **True positive rate (sensitivity)** and **False-positive rate (1-specificity)**.
...
**Specificity** gives a measure of how **selective** the process is, i.e. the ability to identify a negative when there actually is one: $Specificity = TN/N$. It the same as the **True Negative Rate**: $TNR = \frac{TN}{N}$ (and hence, $FPR = 1 - Specificity = 1 - TPR$, since $TN + FP = N$). 
...
![**Synonyms related to classification** (P* and N* are predicted positives and negatives)](Images/TP_FP_synonyms.png){width=60%}
...
plot(roc(pred.default, Default$default))
```


To really test our model above, we would need to make some predictions and test them. We will do this in the exercise and homework.

## Linear Discriminant Analysis

We can also make predictions for the Default example using another method called **Linear Discriminant Analysis**.

![**ROC Curve: higher area under the curve (AUC) indicates better predictive power**](Images/ROC_curve.png){width=60%}
