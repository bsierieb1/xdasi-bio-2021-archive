---
title: "Discrete and continuous distributions, Part 2"
...
The PDF is defined, for $-\infty < a \le x \le b < \infty$, as:

$$ f(x) = \frac{1}{b-a} $$

The CDF is:

$$ F(x) = \int_a^x \frac{1}{b-a} dt = 1 $$

In geometric terms, the width of a uniform distribution is $(b-a)$ and the height is $1/(b-a)$, so the area under the curve always equals one.

We write $X$ ~ $UNIF(a,b)$. The mean and variance are: $E(X) = \frac{b-a}{2}$ and $V(X) = \frac{(b-a)^2}{12}$.
...
We write $X$ ~ $N(\mu,\sigma^2)$, where $E(X) = \mu$ and $V(X) = \sigma^2$.

The Normal distribution has a characteristic bell shape, the precise dimensions of which are governed by $\mu$ and $\sigma$.

![Normal distributions with different mean and variance](Images/Normal_Distribution_PDF.png){width=60%}
...
There is a general rule of thumb about how much of a distribution falls within some number of standard deviations from the mean: a little over 2/3 of the data fall within 1SD, ~95% fall within 2SD, and ~99% fall within 3SD.

![Empirical rule for the normal distribution](Images/Empirical_Rule.png){width=60%}


If you were to sample any Normal distribution, it might look something like the histogram here.

![Sampling from the normal distribution](Images/Empirical_Rule_Histogram.png){width=50%}
...
**_Width of the curve_**: This is governed by the value of the exponent. The Normal distribution is expressed in terms of the mean, $\mu$, and the variance, $\sigma^2$, of the data. Substituting the exponent with the term ${-\frac{1}{2}\left(\frac{x-\mu}{\sigma}\right)^2}$ gives a curve that is zero-centered around the mean $\mu$ and has a standard deviation of $\sigma$.
...
Example: If Starbucks on the corner of Mercer and Greene sells between 2500 and 5000 cups of coffee per day, what's the probability that on any particular day it sold between 3000 and 3500 cups of coffee?

```{r}
punif(c(3500),min=2500, max=5000) - punif(c(3000),min=2500, max=5000)
```
