...
The diagram below illustrates how to choose the right test depending on your question. As the course progresses, we will examine a variety of **parametric** tests for comparing differences between samples:  $t$-tests, ANOVA, simple linear models, and linear models with mixed effects (i.e. interaction terms). We will also go over **nonparametric tests** and tests for **correlations** and **categorical** data.

We will revisit this framework as we progress through the semester.
...
We will walk through Example 6.10 from Ken Aho's book in class. An annotated version of the accompanying figure (Fig. 6.7) illustrates how the above variables affect the power to detect true differences in sample means. Panels (b-d) show that the following differences in the scenario would increase power:
...
# critical value (z*) for lower-tail test
qnorm(0.05)

# get x-bar and SEM
sem = sigma/sqrt(N)
sem
x.bar = qnorm(0.05)*sem

# power
pnorm(x.bar, effect.size, sem)
```
