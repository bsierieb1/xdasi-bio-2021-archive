...
+ If you take a whole bunch of random samples from a population, then the distribution of the sample means will look pretty much like a bell curve. 

+ The amount of uncertainty in your estimator will shrink as the sample size gets bigger, since the variation in $\bar{X}$ between samples is inversely proportional to $N$.
...
```{r class.source = 'fold-hide', message=FALSE, fig.width=5, fig.asp=.7}
...
These are not exactly the same because they are different random samples, but they are pretty close!
...
To be a little more precise, we expect the true population mean to fall the 95% CI is within 1.96 times the standard error error of the mean:
...
+ Because 95% of a normal distribution is contained within two SD of the mean.
...
Another way to write the formula for the CI is 
