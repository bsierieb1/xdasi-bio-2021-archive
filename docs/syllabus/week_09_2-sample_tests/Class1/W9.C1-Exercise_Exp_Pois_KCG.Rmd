...
pexp(20, rate=0.05, lower.tail=F)

# with Poisson
mu = 0.05*20
ppois(0, mu, lower.tail=T)
dpois(0, mu)
```
...
# using exponential
1-exp(-0.05*20)
pexp(20, rate=0.05, lower.tail=T)

# using poisson
ppois(0, mu, lower.tail=F)
1-dpois(0, mu)
```
...
```{r, collapse = T}
exp(-0.05*10) - exp(-0.05*15) 
pexp(15, rate=0.05) - pexp(10, rate=0.05)
```
