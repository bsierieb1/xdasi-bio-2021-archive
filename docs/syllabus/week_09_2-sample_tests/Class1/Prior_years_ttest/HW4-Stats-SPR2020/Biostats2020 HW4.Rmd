---
title: "Parametric and non-parametric tests for one-sample, paired, and two-sample tests"
output:
  html_document:
    toc: true
    toc_depth: 4
...
t_0.975 = qt(p = ___, df = ___)
paste0("t_crit = ",round(t_0.975,4)) # pretty printing
...
#### Q1.b: Check your manual CI estimates using the $t$-test function

Do this for both the Drug and the Placebo samples, comparing against the simulated population mean. Are these the same as what you calculated above?

```{r}
# confidence intervals
ci1 = t.test(___)$conf.int[1:2]  # placebo
ci2 = t.test(___)$conf.int[1:2]  # drug
c(ci1,ci2)
...
#### Q1.c: Using the rules of thumb given in your textbook, can you conclude by eye that the samples come from different populations? Why or why not?
...
```{r}
# Placebo sample
t.test( )  # against pop mean
t.test( )  # against placebo mean

# Drug sample
t.test( )  # against pop mean
t.test( )  # against placebo sample
```
...
se_diff = sqrt ( var(___)/length(___) + 
                   var(___) /length(___) )
se_diff

# critical t-value
t_crit = abs(qt(___, ___))
t_crit
...
2*pt(q = ___, df = ___, lower.tail = ___)

# perform two versions of a 2-sample test using R
t.test(___)  # standard test
t.test(___)  # Welch's approximation
```
...
t.test(___) # equal var
t.test(___) # unequal var
...
## do t-tests using the raw and transformed data
___  # raw
___  # log
```
...
Now extract the 95% CI's for the log-transformed data (pre and post) using the $t$-test command, comparing against the mean of the data itself (this doesn't really matter since you are just interested in the CI here. Don't forget to back-transform!
...
# get the CI data from the test object using `$conf.int[1:2]`
ci95_post_log = ___
...
+ The sum of the ranks that is higher in magnitude is the test statistic.
