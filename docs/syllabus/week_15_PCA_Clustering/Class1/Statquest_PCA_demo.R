## XDASI In-class exercise 12/6/2021

## Adapted from this Statquest PCA demo:
...
# generate data for all 100 genes
# - for each gene, create 5 control and 5 knockout replicates for each experiment
# - sampled from a Poisson distribution with a different random lambda parameter
for (i in 1:100) {
...
## do PCA - need to transpose the matrix first in order to do PCA on wt vs. ko
pca <- prcomp(t(data.matrix), scale=TRUE)

## plot how experiments distribute along pc1 and pc2
# most of the variation is explained by PC1
...
## alternatively we could do PCA on the genes vs. the samples
# do PCA - use original (not transposed) matrix
pca <- prcomp(data.matrix, scale=TRUE)

## plot how genes distribute among pc1 and pc2
# genes vary all over the place
plot(pca$x[,1], pca$x[,2])
...
# There are 10 PCs whether we use conditions or genes for the PCA
