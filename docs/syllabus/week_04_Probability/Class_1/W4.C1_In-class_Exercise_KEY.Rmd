...
father_range = seq(from = min(father.son$fheight),
                   ...
                   by = 0.1) # arbitrary number - the smaller the spacing, the smoother the curve will look on the plot
...
x = seq(from = min(father.son$fheight),
        to = max(father.son$fheight),
        by = step)
...
# y = the ecdf function applied to new data in sequence x
plot(x, f.ecdf(x), 
     lwd=2, col="red", type="l", 
     ylab="CDF", main="ECDF for father heights")
# add ideal line (x=sequence, y=CDF)
lines(x, pnorm(x,mean=mean(father.son$fheight), 
...
# add ideal line (x=sequence, y=CDF)
lines(x, pnorm(x,mean=mean(father.son$sheight), 
