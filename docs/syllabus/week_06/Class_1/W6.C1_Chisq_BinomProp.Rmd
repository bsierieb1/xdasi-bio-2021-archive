...
We have already used resampling and simulation methods to determine how likely a particular outcome is under a ***null hypothesis*** for several different scenarios involving proportions that can be modeled using the binomial distribution.

Here we will learn about several different methods that can be applied to binomial data or to categorical or discrete data with more than two outcomes.
...
For example, we saw that the probability of observing 14/18 right-handed frogs, given $p=0.5$ was very small, even if we perform a two-sided test for obtaining 

```{r}
# probability of obtaining 4 or less, or 14 or more, right-handed frogs
# given neutral expectation for 18 animals = 18/2 = 9 (p=0.5)

pbinom(13,18,p=0.5, lower.tail=FALSE) * 2  # p(X > 13) * 2
...
The $\chi^{2}$ "Chi square" goodness-of-fit test is simply looking at the difference between experimentally `observed` data values and the `expected` values from a model.

## Chi-squared Test

The **Chi-squared Test** statistic, $\chi^2$, compares individual proportions in each group to the **expected proportion** based on the **population mean estimate**.

The test statistic, $\chi^2$, is:

$$ X^2 = \sum\frac{(Obs-Exp)^2}{Exp}$$

So for each cell, we take the square of the difference between the Observed and Expected and divide by Expected, then sum these to get $\chi^2$. Under $H_0$, $\chi^2$ will have an asymptotic $\chi^2$ distribution with $(r-1)(c-1) = 1$ degree of freedom.
...
Notice that the manual calculation using `pchisq` does not give exactly the same $p$-value as `chisq.test`. This is because the approximation we made by hand does not correct for the fact that $\chi^2$ is a continuous function, and thus underestimates the true $p$-values based on the binomial distribution. In contrast, `chisq.test` uses Yates’ correction by default. 
