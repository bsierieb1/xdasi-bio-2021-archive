...
qplot(x=Sepal.Length,
      y=Sepal.Width,
      data=iris,
      geom=c("point")) # this is the biggest difference from base R
```
...
```{r}
# a function to build the plot piece by piece
ggplot(data=iris, 
...
```{r}
# pass a single value to the geometry function to color *all* points the same way
ggplot(data=iris,
       mapping=aes(x=Sepal.Length,
                   y=Sepal.Width)) +
  geom_point(color="red") # here

# pass the name of the data.frame column by which to color points to the aesthetics
ggplot(data=iris, 
       mapping=aes(x=Sepal.Length,
                   y=Sepal.Width,
                   color=Species)) + # here
...
ggplot(data=iris, 
       mapping=aes(x=Sepal.Length,
                   y=Sepal.Width,
                   color=Species)) +
  geom_point(mapping=aes(color=Species)) # here also works because there is only one geometry on this graph
```
