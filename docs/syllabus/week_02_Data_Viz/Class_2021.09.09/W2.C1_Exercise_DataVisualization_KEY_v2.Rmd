...
Note that R seems to know how to map `palette()` values onto factors automatically. *HOWEVER, if the number of factors exceeds the number of colors in the palette, colors will just get recycled! (You can try this with the iris dataset by removing one of the colors above and remaking the plot.)*

What if instead we just tried to use a random vector of three colors? Points will just get randomly assigned to colors! *(Just like with the palette colors, these colors will get recycled until all of the points are colored; however there will be no relationship between factors and the color of each data point.)*
...
str(species_colors)           # is it still a factor?
...
# we need to convert the factor vector to a character vector
# why? these are not already part of the data frame
