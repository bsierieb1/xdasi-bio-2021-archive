...
For independent samples, the degrees of freedom are: $df = n_1 + n_2 - 2$.
...
Since we expect that we have random samples drawn from a normal distribution, it is valud to use $t$-statistics to test our null hypothesis.  
...
t.test(Drug, Placebo, var.equal = T)
t.test(Drug, Placebo, var.equal = F)
```
...
Do this by hand first, and then using the `t.test()` function to perform a one-sample test that compares the paired difference vector against the expected mean difference.
...
It is important to note that the result from the **one-sample** $t$-test using the **paired differences** between the two samples gives the same result as a **two-sample** $t$-test with the **paired** option! So you don't actually need to take the paired differences to do this test.

Perform a paired $t$-test below to verify that the two methods are equivalent.
