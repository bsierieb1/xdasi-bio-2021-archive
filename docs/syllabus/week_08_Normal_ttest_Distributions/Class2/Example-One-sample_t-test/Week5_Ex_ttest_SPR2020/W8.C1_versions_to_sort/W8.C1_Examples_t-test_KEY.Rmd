...
Perform a one-sample t-test for each of the two samples using the population parameter. What do you conclude?
...
Compare the estimated, simulated, and true population parameters. How do they compare?
...
## Compare results from one-sample $t$-tests
...
# Placebo sample
t.test(Placebo, mu=mean(Placebo_pop))
t.test(Placebo, mu=mean(pop_mean_est))
t.test(Placebo, mu=mean(Placebo))

# Drug sample
t.test(Drug, mu=mean(Placebo_pop))
t.test(Drug, mu=mean(pop_mean_est))
