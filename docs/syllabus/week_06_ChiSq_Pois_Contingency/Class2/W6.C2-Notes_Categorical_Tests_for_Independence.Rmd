...
2. ***Independence***: Are two (or more) categorical variables associated in some way? Do response variables differ between groups?
>>>>>>> Stashed changes
...
+ **Fisher's test** is usually used only for 2x2 contingency tables. 
  + It can, in fact, be applied to larger RxC tables, but its computational efficiency declines for very large sample sizes.
>>>>>>> Stashed changes
