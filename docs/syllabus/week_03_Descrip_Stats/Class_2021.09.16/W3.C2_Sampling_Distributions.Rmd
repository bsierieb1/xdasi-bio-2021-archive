...
+ Therefore, our **random variable, $\bar{X}$,** should be contained within two standard deviations of the true mean around 95% of the time (even though every once in a while it will be rather far off because we are taking random samples). 
...
Technically, the 95% CI specifies that 95% of random intervals $\overline{X} \pm \sim 1.96 * SEM$ will contain the true population mean. 

+ Since $SEM = s_x/\sqrt{N}$, combining these gives us: $95\%\ CI = \overline{X} \pm \sim 1.96s_x/\sqrt{N}$.

So, with the sample mean, SD, and sample size in hand, we are almost ready to find the limits of the 95% CI.

#### The quantile function

There is one additional piece of information that will help us: the precise **z-score** (number of SDs away from the mean) that provides the exact boundary for the central 95% of the data (so we don't have to use an approximation of 1.96). 

We will use the **quantile** function to find this number.
